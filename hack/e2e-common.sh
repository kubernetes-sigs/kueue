...
if [[ -n ${KUBEFLOW_TRAINER_V2_VERSION:-} ]]; then
    export KUBEFLOW_TRAINER_V2_MANIFEST=${ROOT_DIR}/dep-crds/kf-trainer-v2/manifests
    # Extract the Kubeflow Trainer controller manager image version tag (newTag) from the manifest.
    # This is necessary because the image version tag does not follow the usual package versioning convention.
    KF_TRAINER_V2_IMAGE_VERSION=$($YQ '.images[] | select(.name | contains("trainer-controller-manager")) | .newTag' "${KUBEFLOW_TRAINER_V2_MANIFEST}/overlays/manager/kustomization.yaml")
    export KF_TRAINER_V2_IMAGE_VERSION
    export KF_TRAINER_V2_IMAGE=ghcr.io/kubeflow/trainer/trainer-controller-manager:${KF_TRAINER_V2_IMAGE_VERSION}
fi
...
function prepare_docker_images {
    ...
    if [[ -n ${KUBEFLOW_TRAINER_V2_VERSION:-} ]]; then
        docker pull "${KF_TRAINER_V2_IMAGE}"
    fi
...
function install_kubeflow_trainer_v2 {
    cluster_kind_load_image "${1}" "${KF_TRAINER_V2_IMAGE}"
    (
        ...
        cp -r "${KUBEFLOW_TRAINER_V2_MANIFEST}"/* "$manifests_temp_dir/" && chmod -R 777 "$manifests_temp_dir"
        if [[ -n ${JOBSET_VERSION:-} ]]; then
    ...
    kubectl apply --kubeconfig="$2" --server-side -k "${KUBEFLOW_TRAINER_V2_MANIFEST}/overlays/runtimes"
}

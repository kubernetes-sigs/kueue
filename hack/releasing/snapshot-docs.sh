...
for locale in "${locales[@]}"; do
  ...
  # Strip `aliases:` and `menu:` frontmatter. Aliases would redeclare the same
  # redirect as the development page (Hugo errors on duplicate aliases); menu
  # entries would add duplicate versioned items to the site nav.
  find "${dst}" -type f -name '*.md' -exec perl -0777 -i -pe '
    s/^aliases:[ \t]*\n(?:[ \t]*-[ \t].*\n)*//mg;
    s/^menu:[ \t]*\n(?:[ \t]+.*\n)+//mg;
  ' {} +

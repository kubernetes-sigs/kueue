...
KF_TRAINER_V2_ROOT = $(shell $(GO_CMD) list -m -mod=readonly -f "{{.Dir}}" github.com/kubeflow/trainer/v2)
...
.PHONY: kf-trainer-v2-crd ## Runtimes
kf-trainer-v2-crd: ## Copy the CRDs of the kubeflow trainer v2 to the dep-crds directory.
	mkdir -p $(EXTERNAL_CRDS_DIR)/kf-trainer-v2-crds/
	cp -rf $(KF_TRAINER_V2_ROOT)/manifests/base/crds/* $(EXTERNAL_CRDS_DIR)/kf-trainer-v2-crds/

.PHONY: kf-trainer-v2-manifests
kf-trainer-v2-manifests: ## Copy whole manifests folder of the kubeflow trainer v2 to the dep-crds directory.
	if [ -d "$(EXTERNAL_CRDS_DIR)/kf-trainer-v2" ]; then \
		chmod -R u+w "$(EXTERNAL_CRDS_DIR)/kf-trainer-v2" && \
		rm -rf "$(EXTERNAL_CRDS_DIR)/kf-trainer-v2"; \
	fi
	mkdir -p "$(EXTERNAL_CRDS_DIR)/kf-trainer-v2"
	cp -rf "$(KF_TRAINER_V2_ROOT)/manifests" "$(EXTERNAL_CRDS_DIR)/kf-trainer-v2"
...
.PHONY: dep-crds
dep-crds: mpi-operator-crd kf-training-operator-crd kf-trainer-v2-crd ray-operator-crd jobset-operator-crd leaderworkerset-operator-crd cluster-autoscaler-crd appwrapper-crd appwrapper-manifests kf-training-operator-manifests ray-operator-manifests kf-trainer-v2-manifests ## Copy the CRDs from the external operators to the dep-crds directory.

	...
	t.Run("ClusterQueueClasses", func(t *testing.T) {
		for c, cqcSummary := range summary.ClusterQueueClasses {
			t.Run(c, func(t *testing.T) {
				...
				if actual < expected {
					t.Errorf("Usage %.2f%% is less than expected %.2f%%", actual, expected)
				}
				t.Logf("ClusterQueue class %s: usage %.2f%% (min expected: %.2f%%)", c, actual, expected)
			})
		}
	})
	...
	t.Run("WorkloadClasses", func(t *testing.T) {
		for c, wlcSummary := range summary.WorkloadClasses {
			t.Run(c, func(t *testing.T) {
				...
				if !found {
					t.Fatalf("Unexpected workload class: %s", c)
				}
				if wlcSummary.AverageTimeToAdmissionMs > expected {
					t.Errorf("Average time to admission %dms is more than expected %dms", wlcSummary.AverageTimeToAdmissionMs, expected)
				}
				t.Logf("Workload class %s: avg admission time %dms (max expected: %dms)", c, wlcSummary.AverageTimeToAdmissionMs, expected)
				t.Logf("  - Count: %d", wlcSummary.Count)
				t.Logf("  - Avg time to finish: %dms", wlcSummary.AverageTimeToFinishMs)
				t.Logf("  - Total evictions: %d", wlcSummary.TotalEvictions)
			})
		}
	})
}

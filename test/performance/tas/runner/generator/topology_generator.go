...
func GenerateTopologyResource(ctx context.Context, c client.Client, config TopologyConfig) error {
	...
	topologyLevels := make([]kueue.TopologyLevel, len(config.Levels))
	for i, level := range config.Levels {
		topologyLevels[i] = kueue.TopologyLevel{
	...
	topology.Name = config.Name
	topology.Spec.Levels = topologyLevels
...
func GenerateResourceFlavor(ctx context.Context, c client.Client, flavorConfig ResourceFlavorConfig, topologyName string) error {
	...
	rf := utiltestingapi.MakeResourceFlavor(flavorConfig.Name).

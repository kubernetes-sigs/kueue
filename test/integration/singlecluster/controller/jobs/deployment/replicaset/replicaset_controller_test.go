...
var _ = ginkgo.Describe("RS without scheduler", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	ginkgo.When("ElasticJobsViaWorkloadSlices feature is enabled", func() {
		...
		ginkgo.It("should not create a workload for a replicaset without elastic-job annotation and without queue-name label", func() {
			...
			replicaSet = newReplicaSet(namespace.Name, "test", "default", 3)
			replicaSet.Annotations = nil
			replicaSet.Labels = nil
	...
	ginkgo.When("ElasticJobsViaWorkloadSlices feature is disabled", func() {
		...
		ginkgo.It("should not create a workload for a replicaset with elastic-job annotation and queue-name label", func() {
			ginkgo.By("create deployment")
			replicaSet = newReplicaSet(namespace.Name, "test", "default", 1)
...
var _ = ginkgo.Describe("RS with scheduler", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should scale up and down", func() {
		...
		ginkgo.By("Creating 0-pod replicaset", func() {
			util.MustCreate(ctx, k8sClient, rs)
		})

		ginkgo.By("Workload created and admitted", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				key := jobframework.GetWorkloadNameForOwnerWithGVKAndGeneration(rs.Name, rs.UID, replicaset.GroupVersionKind, rs.Generation)
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: key, Namespace: rs.Namespace}, wlOriginal)).To(gomega.Succeed())
				util.ExpectWorkloadsToBeAdmitted(ctx, k8sClient, wlOriginal)
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})

		ginkgo.By("Scaling-up replicaset", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(rs), rs)).To(gomega.Succeed())
				rs.Spec.Replicas = ptr.To(int32(3))
				g.Expect(k8sClient.Update(ctx, rs)).To(gomega.Succeed())
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})
		...
		ginkgo.By("Scaled-up workload created and admitted and original workload is finished", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				...
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: key, Namespace: rs.Namespace}, wlScaledUp)).To(gomega.Succeed())
		...
		ginkgo.By("Scaling-down replicaset", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(rs), rs)).To(gomega.Succeed())
		...
		ginkgo.By("Scaled-up workload is still admitted, and has 1 pod count", func() {
			...
			gomega.Eventually(func(g gomega.Gomega) {
				...
				g.Expect(wlScaledUp.Spec.PodSets[0].Count).To(gomega.BeEquivalentTo(int32(1)))
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})
	...
	ginkgo.It("Should update pending workload", func() {
		rs := newReplicaSet(namespace.Name, "test", localQueue.Name, 10000)
		wl := &kueue.Workload{}

		ginkgo.By("Creating inadmissible replicaset", func() {
			util.MustCreate(ctx, k8sClient, rs)
		})
		...
		ginkgo.By("Workload created but not admitted", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				key := jobframework.GetWorkloadNameForOwnerWithGVKAndGeneration(rs.Name, rs.UID, replicaset.GroupVersionKind, rs.Generation)
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: key, Namespace: rs.Namespace}, wl)).To(gomega.Succeed())
				g.Expect(wl.Spec.PodSets[0].Count).To(gomega.BeEquivalentTo(int32(10000)))
				util.ExpectWorkloadsToBePending(ctx, k8sClient, wl)
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})

		ginkgo.By("Update replicaset to still inadmissible", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(rs), rs)).To(gomega.Succeed())
				rs.Spec.Replicas = ptr.To(int32(1000))
				g.Expect(k8sClient.Update(ctx, rs)).To(gomega.Succeed())
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})
		...
		ginkgo.By("Update replicaset to admissible", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(rs), rs)).To(gomega.Succeed())
				rs.Spec.Replicas = ptr.To(int32(1))
				g.Expect(k8sClient.Update(ctx, rs)).To(gomega.Succeed())
		...
		ginkgo.By("There is only 1 workload", func() {
			gomega.Consistently(func(g gomega.Gomega) {
				wlList := &kueue.WorkloadList{}
				g.Expect(k8sClient.List(ctx, wlList, client.InNamespace(rs.Namespace))).To(gomega.Succeed())
	...
	ginkgo.It("Should preempt", func() {
		...
		ginkgo.By("Creating low-pri replicaset", func() {
		...
		ginkgo.By("Observing low-pri workload created and admitted", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				key := jobframework.GetWorkloadNameForOwnerWithGVKAndGeneration(lowPriRS.Name, lowPriRS.UID, replicaset.GroupVersionKind, lowPriRS.Generation)
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: key, Namespace: lowPriRS.Namespace}, lowPriWl)).To(gomega.Succeed())
				util.ExpectWorkloadsToBeAdmitted(ctx, k8sClient, lowPriWl)
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})

		ginkgo.By("Scale-up low-pri replicaset", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lowPriRS), lowPriRS)).To(gomega.Succeed())
				lowPriRS.Spec.Replicas = ptr.To(int32(2))
				g.Expect(k8sClient.Update(ctx, lowPriRS)).To(gomega.Succeed())
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})
		...
		ginkgo.By("Scaled-up workload created and admitted and original workload is finished", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				...
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: key, Namespace: lowPriRS.Namespace}, lowPriWlSlice)).To(gomega.Succeed())
		...
		highPriRS := newReplicaSet(namespace.Name, "high", localQueue.Name, 2)
		highPriWl := &kueue.Workload{}

		ginkgo.By("Creating high-pri replicaset", func() {
		...
		ginkgo.By("Observing high-pri workload created and admitted", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				key := jobframework.GetWorkloadNameForOwnerWithGVKAndGeneration(highPriRS.Name, highPriRS.UID, replicaset.GroupVersionKind, highPriRS.Generation)
	...
	ginkgo.It("Should preempt pending scale-up", func() {
		lowPriRS := newReplicaSet(namespace.Name, "low", localQueue.Name, 2)
		lowPriWl := &kueue.Workload{}
		...
		ginkgo.By("Scale-up low-pri replicaset to INADMISSIBLE", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lowPriRS), lowPriRS)).To(gomega.Succeed())
				lowPriRS.Spec.Replicas = ptr.To(int32(10))
				g.Expect(k8sClient.Update(ctx, lowPriRS)).To(gomega.Succeed())
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})

		lowPriWlSlice := &kueue.Workload{}
		ginkgo.By("Scaled-up workload created but stays pending while original is still admitted", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				key := jobframework.GetWorkloadNameForOwnerWithGVKAndGeneration(lowPriRS.Name, lowPriRS.UID, replicaset.GroupVersionKind, lowPriRS.Generation)
				g.Expect(key).ToNot(gomega.Equal(lowPriWl.Name))
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: key, Namespace: lowPriRS.Namespace}, lowPriWlSlice)).To(gomega.Succeed())
				util.ExpectWorkloadsToBePending(ctx, k8sClient, lowPriWlSlice)
				util.ExpectWorkloadsToBeAdmitted(ctx, k8sClient, lowPriWl)
			}, util.Timeout, util.Interval).Should(gomega.Succeed())
		})
		...
		ginkgo.By("Observing [new] low-pri workload-slice is still pending", func() {
			util.ExpectWorkloadsToBePending(ctx, k8sClient, lowPriWlSlice)
		})
		ginkgo.By("Observing [old] low-pri workload-slice is preempted", func() {
			util.ExpectWorkloadsToBePreempted(ctx, k8sClient, lowPriWl)
		})
	})
})

...
var _ = ginkgo.Describe("Pod controller Deployment parent suspension", ginkgo.Label("job:pod", "area:jobs"), ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	createDeploymentChain := func(name string) (*appsv1.Deployment, *corev1.Pod) {
		...
		// Re-read to get the server-assigned UID.
		gomega.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(deploy), deploy)).To(gomega.Succeed())
		...
		pod := testingpod.MakePod(name+"-pod", ns.Name).
			...
			Obj()
		// Fix the OwnerReference UID — the wrapper sets name as UID by default.
		for i := range pod.OwnerReferences {
			if pod.OwnerReferences[i].Kind == "ReplicaSet" {
				pod.OwnerReferences[i].UID = rs.UID
			}
		}
		util.MustCreate(ctx, k8sClient, pod)
	...
	ginkgo.It("should not unpause a user-paused Deployment", func() {
		ginkgo.By("creating a user-paused Deployment before any pods exist")
		deploy := &appsv1.Deployment{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "test-deploy",
				Namespace: ns.Name,
			...
			Spec: appsv1.DeploymentSpec{
				Paused:   true,
				Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"app": "test-deploy"}},
				Template: corev1.PodTemplateSpec{
					ObjectMeta: metav1.ObjectMeta{Labels: map[string]string{"app": "test-deploy"}},
					Spec: corev1.PodSpec{
						Containers: []corev1.Container{{
							Name:      "c",
							Image:     "pause",
		...
		pod := testingpod.MakePod("test-deploy-pod", ns.Name).
			Queue(lq.Name).
			ManagedByKueueLabel().
			Request(corev1.ResourceCPU, "1").
			SuspendedByParent("deployment").
		...
		ginkgo.By("verifying the Deployment remains paused without Kueue annotation")
		gomega.Consistently(func(g gomega.Gomega) {
			d := &appsv1.Deployment{}
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(deploy), d)).To(gomega.Succeed())
			g.Expect(d.Spec.Paused).To(gomega.BeTrue(), "User-paused Deployment should remain paused")
			g.Expect(d.Annotations).NotTo(gomega.HaveKey(controllerconstants.PausedByKueueAnnotation))

	...
	ginkgo.When("manageJobsWithoutQueueName is disabled", func() {
		...
		ginkgo.When("Using single pod", func() {
			...
			ginkgo.It("Should preserve custom finalizers when using RemoveFinalizersWithStrictPatch featuregate", func() {
				gomega.Expect(features.SetEnable(features.RemoveFinalizersWithStrictPatch, true)).Should(gomega.Succeed())
				...
				gomega.Expect(createdPod.Finalizers).
					To(gomega.ContainElement(constants.ManagedByKueueLabel), "Pod should have finalizer")

...
// Reproduces https://github.com/kubernetes-sigs/kueue/issues/1789:
// Workloads whose owner Job is deleted should be finished and release quota.
var _ = ginkgo.Describe("Job controller with orphaned workload cleanup", ginkgo.Label("job:batch", "area:jobs"), ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("should finish orphaned workload and release quota when owner Job is deleted", func() {
		ginkgo.By("creating a Job")
		...
		ginkgo.By("asserting the orphaned workload is finished with OwnerNotFound")
		wl := &kueue.Workload{}
		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, wlKey, wl)).To(gomega.Succeed())
			g.Expect(wl.Status.Conditions).Should(utiltesting.HaveConditionStatusTrueAndReason(kueue.WorkloadFinished, kueue.WorkloadFinishedReasonOwnerNotFound))
		}, util.Timeout, util.Interval).Should(gomega.Succeed())
	})
})

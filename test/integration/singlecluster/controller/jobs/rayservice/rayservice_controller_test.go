...
var _ = ginkgo.Describe("RayService controller", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should reconcile RayServices", func() {
		...
		job := testingrayservice.MakeRayService(jobName, ns.Name).
			Suspend(false).
			WithPriorityClassName(priorityClassName).
			Obj()
		util.MustCreate(ctx, k8sClient, job)
		createdJob := &rayv1.RayService{}

		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: jobName, Namespace: ns.Name}, createdJob)).Should(gomega.Succeed())
			g.Expect(createdJob.Spec.RayClusterSpec.Suspend).Should(gomega.Equal(ptr.To(true)))
		}, util.Timeout, util.Interval).Should(gomega.Succeed())

		ginkgo.By("checking the workload is created without queue assigned")
		createdWorkload := &kueue.Workload{}
		wlLookupKey := types.NamespacedName{Name: workloadrayservice.GetWorkloadNameForRayService(job.Name, job.UID), Namespace: ns.Name}
		gomega.Eventually(func(g gomega.Gomega) {
		...
		secondWl := &kueue.Workload{
			ObjectMeta: metav1.ObjectMeta{
				Name:      workloadrayservice.GetWorkloadNameForRayService("second-workload", "test-uid"),
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(createdJob.Spec.RayClusterSpec.Suspend).Should(gomega.Equal(ptr.To(false)))
		}, util.Timeout, util.Interval).Should(gomega.Succeed())
...
var _ = ginkgo.Describe("Job controller RayService for workloads when only jobs with queue are managed", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	ginkgo.BeforeAll(func() {
	...
	ginkgo.It("Should reconcile jobs only when queue is set", func() {
		...
		job := testingrayservice.MakeRayService(jobName, ns.Name).Obj()
		util.MustCreate(ctx, k8sClient, job)
		lookupKey := types.NamespacedName{Name: jobName, Namespace: ns.Name}
		createdJob := &rayv1.RayService{}
	...
	ginkgo.It("Should suspend a cluster if the parent's workload does not exist or is not admitted", func() {
		...
		childCluster := testingrayservice.MakeRayService(jobName, ns.Name).
			Suspend(false).
			Obj()
		...
		gomega.Eventually(func(g gomega.Gomega) {
			...
			g.Expect(childCluster.Spec.RayClusterSpec.Suspend).Should(gomega.Equal(ptr.To(true)))
		}, util.Timeout, util.Interval).Should(gomega.Succeed())
	})
})
...
var _ = ginkgo.Describe("Job controller when waitForPodsReady enabled", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.DescribeTable("Single job at different stages of progress towards completion",
		...
		ginkgo.Entry("Running RayService; PodsReady=False before", podsReadyTestSpec{
			beforeCondition: &metav1.Condition{
				Type:    kueue.WorkloadPodsReady,
				Status:  metav1.ConditionFalse,
				Reason:  kueue.WorkloadWaitForStart,
...
var _ = ginkgo.Describe("RayService Job controller interacting with scheduler", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	ginkgo.BeforeAll(func() {
	...
	ginkgo.It("Should schedule jobs as they fit in their ClusterQueue", func() {
		...
		job := testingrayservice.MakeRayService("dev-job", ns.Name).Queue(localQueue.Name).
		...
		ginkgo.By("checking a second no-fit RayService does not start")
		job2 := testingrayservice.MakeRayService("dev-job2", ns.Name).Queue(localQueue.Name).
		...
		ginkgo.By("deleting the workload", func() {
			wl := &kueue.Workload{}
			wlKey := types.NamespacedName{Name: workloadrayservice.GetWorkloadNameForRayService(job.Name, job.UID), Namespace: job.Namespace}
		...
		ginkgo.By("checking the second RayService starts when the first one was deleted")
		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(job2), createdJob2)).Should(gomega.Succeed())
			g.Expect(createdJob2.Spec.RayClusterSpec.Suspend).Should(gomega.Equal(ptr.To(false)))
		}, util.Timeout, util.Interval).Should(gomega.Succeed())
		gomega.Expect(createdJob2.Spec.RayClusterSpec.HeadGroupSpec.Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(spotUntaintedFlavor.Name))
		gomega.Expect(createdJob2.Spec.RayClusterSpec.WorkerGroupSpecs[0].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(spotUntaintedFlavor.Name))
		util.ExpectPendingWorkloadsMetric(clusterQueue, 0, 0)
		util.ExpectReservingActiveWorkloadsMetric(clusterQueue, 1)
	})
})
...
var _ = ginkgo.Describe("Job controller with preemption enabled", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should preempt lower priority RayServices when resource insufficient", func() {
		ginkgo.By("Create a low priority RayService")
		lowPriorityJob := testingrayservice.MakeRayService("rayservice-with-low-priority", ns.Name).Queue(localQueue.Name).
		...
		ginkgo.By("Create a high priority RayService which will preempt the lower one")
		highPriorityJob := testingrayservice.MakeRayService("rayserivce-with-high-priority", ns.Name).Queue(localQueue.Name).
			RequestHead(corev1.ResourceCPU, "2").
			WithPriorityClassName(priorityClassName).
			RequestWorkerGroup(corev1.ResourceCPU, "2").
			Obj()
		...
		highPriorityLookupKey := types.NamespacedName{Name: workloadrayservice.GetWorkloadNameForRayService(highPriorityJob.Name, highPriorityJob.UID), Namespace: ns.Name}
		...
		createdWorkload := &kueue.Workload{}
		lowPriorityLookupKey := types.NamespacedName{Name: workloadrayservice.GetWorkloadNameForRayService(lowPriorityJob.Name, lowPriorityJob.UID), Namespace: ns.Name}
		...
		ginkgo.By("Low priority RayService should be suspended")
		createdJob = &rayv1.RayService{}
		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lowPriorityJob), createdJob)).Should(gomega.Succeed())
			g.Expect(createdJob.Spec.RayClusterSpec.Suspend).Should(gomega.Equal(ptr.To(true)))
		...
		ginkgo.By("Delete high priority rayservice")
		...
		ginkgo.By("Low priority RayServiceshould be unsuspended")
		createdJob = &rayv1.RayService{}
		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lowPriorityJob), createdJob)).Should(gomega.Succeed())
			g.Expect(createdJob.Spec.RayClusterSpec.Suspend).Should(gomega.Equal(ptr.To(false)))

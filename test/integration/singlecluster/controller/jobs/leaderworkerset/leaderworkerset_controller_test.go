...
var _ = ginkgo.Describe("LeaderWorkerSet controller", ginkgo.Label("job:leaderworkerset", "area:jobs"), func() {
	...
	ginkgo.It("Should propagate and sync GroupPodsReadyMinCountAnnotation from LeaderWorkerSet to its Pods when updated", func() {
		...
		ginkgo.By("Creating a Pod owned by the LeaderWorkerSet's StatefulSet and verifying initial annotation propagation")
		pod := testingjobspod.MakePod(createdLWS.Name+"-0", ns.Name).
			OwnerReference(createdSTS.Name, appsv1.SchemeGroupVersion.WithKind("StatefulSet")).
			Label(leaderworkersetv1.SetNameLabelKey, createdLWS.Name).
			Label(leaderworkersetv1.GroupIndexLabelKey, "0").
		...
		ginkgo.By("Updating GroupPodsReadyMinCountAnnotation on the LeaderWorkerSet to 1 and verifying the Pod annotation is synced")
		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lws), createdLWS)).Should(gomega.Succeed())
			createdLWS.Annotations[constants.GroupPodsReadyMinCountAnnotation] = "1"
		...
		ginkgo.By("Removing GroupPodsReadyMinCountAnnotation from the LeaderWorkerSet and verifying it is removed from the Pod")
		gomega.Eventually(func(g gomega.Gomega) {
			g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(lws), createdLWS)).Should(gomega.Succeed())
			delete(createdLWS.Annotations, constants.GroupPodsReadyMinCountAnnotation)
			g.Expect(k8sClient.Update(ctx, createdLWS)).Should(gomega.Succeed())
		}, util.Timeout, util.Interval).Should(gomega.Succeed())

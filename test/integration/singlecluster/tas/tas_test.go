			...
			ginkgo.It("should update workload TopologyAssignment when node fails", func() {
				...
				ginkgo.By("verify the workload has corrected TopologyAssignment", func() {
					gomega.Eventually(func(g gomega.Gomega) {
						...
						gomega.Expect(workload.GetNodesToReplace(wl1)).NotTo(gomega.ContainElement(nodeName))
					}, util.Timeout, util.Interval).Should(gomega.Succeed())
				})
			})
			ginkgo.It("should remove node from nodesToReplace when the node recovers", func() {
				...
				ginkgo.By("verify the workload eventually gets an entry in nodesToReplace list", func() {
					gomega.Eventually(func(g gomega.Gomega) []string {
						g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(wl1), wl1)).To(gomega.Succeed())
						return workload.GetNodesToReplace(wl1)
			...
			ginkgo.It("should update workload TopologyAssignment after a node recovers", func() {
				...
				ginkgo.By("verify the workload has corrected TopologyAssignment and no node in NodesToReplace", func() {
					gomega.Eventually(func(g gomega.Gomega) {
						...
						g.Expect(workload.GetNodesToReplace(wl1)).NotTo(gomega.ContainElement(nodeName))
					}, util.Timeout, util.Interval).Should(gomega.Succeed())
				})
			})
			...
			ginkgo.It("should evict workload when multiple assigned nodes are deleted", func() {
				...
				ginkgo.By("verify the workload is evicted due to multiple node failures", func() {
					...
					gomega.Eventually(func(g gomega.Gomega) {
						...
						g.Expect(workload.GetNodesToReplace(updatedWl)).To(gomega.BeEmpty(), "NodesToReplace should be cleared after eviction due to multiple node failures")
					}, util.Timeout, util.Interval).Should(gomega.Succeed())
				})
			})
			...
			ginkgo.It("should evict the Workload if no replacement is possible and place it on another rack", func() {
				...
				ginkgo.By("verify the workload is evicted due to no replacement possible", func() {
					...
					gomega.Eventually(func(g gomega.Gomega) {
						updatedWl := &kueue.Workload{}
						g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(wl1), updatedWl)).To(gomega.Succeed())
						g.Expect(workload.GetNodesToReplace(wl1)).To(gomega.BeEmpty())
					}, util.Timeout, util.Interval).Should(gomega.Succeed())
				})

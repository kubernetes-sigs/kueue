...
var _ = ginkgo.Describe("Scheduler", ginkgo.Label("feature:fairsharing"), func() {
	...
	ginkgo.When("Using AdmissionFairSharing with AdmissionChecks", ginkgo.Label("feature:admissionfairsharing"), func() {
		...
		ginkgo.It("should settle the entry penalty at quota reservation when the accounting anchor moves there", func() {
			...
			ginkgo.By("Verifying the entry penalty is settled before the admission check completes")
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(qManager.AfsUsageLedger.HasPendingPenalty(lqKey)).To(gomega.BeFalse(), "entry penalty should settle at quota reservation for lq-a")
			}, util.Timeout, util.Interval).Should(gomega.Succeed())

			ginkgo.By("Verifying the still unadmitted workload's quota, not just its settled penalty, reaches the consumed usage")
			// The penalty alone contributes 2 CPU here; exceeding 2.5 proves that
			// reserved live usage is also being sampled.
			util.ExpectLocalQueueFairSharingUsageToBe(ctx, k8sClient, client.ObjectKeyFromObject(lq), ">", 2_500)
	...
	ginkgo.When("Preemption is enabled in fairsharing and the preemptor exceeds its cohort's nominal quota", func() {
		...
		ginkgo.It("Should not preempt once the preemptor's cohort is past its nominal quota", func() {
			ginkgo.By("Admitting a workload in cqB that borrows root-over's flavor1 quota")
			wlB := createWorkload("b", "1")
			util.ExpectWorkloadsToBeAdmitted(ctx, k8sClient, wlB)

			ginkgo.By("Admitting a workload in cqA that consumes cohort-a's whole nominal flavor1 quota")
			wlA1 := createWorkload("a", "1")
			util.ExpectWorkloadsToBeAdmitted(ctx, k8sClient, wlA1)
			...
			ginkgo.By("Creating a second workload in cqA, which would push cohort-a past its nominal 1 CPU")
			wlA2 := createWorkload("a", "1")

			ginkgo.By("The workload in cqB should be left alone and the new workload should stay pending")
			util.ExpectWorkloadsToBePending(ctx, k8sClient, wlA2)
			gomega.Consistently(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(wlB), wlB)).To(gomega.Succeed())
				cond := meta.FindStatusCondition(wlB.Status.Conditions, kueue.WorkloadEvicted)
				g.Expect(cond == nil || cond.Reason != kueue.WorkloadPreempted).To(gomega.BeTrue(),
					"a preemptor past its cohort's nominal quota must not reclaim on DRS alone")
			}, util.ConsistentDuration, util.ShortInterval).Should(gomega.Succeed())
			util.ExpectAdmittedWorkloadsTotalMetric(cqA, "", 1)
			util.ExpectAdmittedWorkloadsTotalMetric(cqB, "", 1)
		})

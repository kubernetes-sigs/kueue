...
var _ = ginkgo.Describe("Scheduler", func() {
	...
	testSuite := func(parallelizedApiCalls bool) func() {
		return func() {
			ginkgo.BeforeEach(func() {
				features.SetFeatureGateDuringTest(ginkgo.GinkgoTB(), features.SchedulerParallelizedAPICalls, parallelizedApiCalls)
			})
			...
			ginkgo.When("Deleting child Cohort should update borrowable resources for parent Cohort", func() {
				...
				ginkgo.It("Should not admit from stale subtree quota after deleting sibling cohort", func() {
					...
					util.ExpectWorkloadToBeAdmittedAs(
						...
						k8sClient,
						wlBase,
						utiltestingapi.MakeAdmission(kueue.ClusterQueueReference(cqA.Name)).
							PodSets(
	...
	ginkgo.Context("Scheduler API calls are made synchroniously", testSuite(false))
	ginkgo.Context("Scheduler API calls are parallelized", testSuite(true))

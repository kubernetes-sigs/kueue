...
var _ = ginkgo.Describe("Job controller", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.BeforeEach(func() {
		...
		wlLookupKey = types.NamespacedName{Name: workloadmxjob.GetWorkloadNameForMXJob(jobName), Namespace: ns.Name}
	})
	...
	ginkgo.It("Should reconcile MXJobs", func() {
		...
		secondWl := &kueue.Workload{
			ObjectMeta: metav1.ObjectMeta{
				Name:      workloadmxjob.GetWorkloadNameForMXJob("second-workload"),
		...
		gomega.Expect(len(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeScheduler].Template.Spec.NodeSelector)).Should(gomega.Equal(1))
		gomega.Expect(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeScheduler].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(onDemandFlavor.Name))
		gomega.Expect(len(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeServer].Template.Spec.NodeSelector)).Should(gomega.Equal(1))
		gomega.Expect(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeServer].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(spotFlavor.Name))
		gomega.Expect(len(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeWorker].Template.Spec.NodeSelector)).Should(gomega.Equal(1))
		gomega.Expect(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeWorker].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(spotFlavor.Name))
		gomega.Consistently(func() bool {
			if err := k8sClient.Get(ctx, wlLookupKey, createdWorkload); err != nil {
				return false
			}
		...
		parallelism := ptr.Deref(job.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeWorker].Replicas, 1)
		newParallelism := parallelism + 1
		createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeWorker].Replicas = &newParallelism
		gomega.Expect(k8sClient.Update(ctx, createdJob)).Should(gomega.Succeed())
		gomega.Eventually(func() bool {
			if err := k8sClient.Get(ctx, lookupKey, createdJob); err != nil {
				return false
			...
			return createdJob.Spec.RunPolicy.Suspend != nil && *createdJob.Spec.RunPolicy.Suspend &&
				len(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeWorker].Template.Spec.NodeSelector) == 0
		}, util.Timeout, util.Interval).Should(gomega.BeTrue())
		gomega.Eventually(func() bool {
...
var _ = ginkgo.Describe("Job controller when waitForPodsReady enabled", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.DescribeTable("Single job at different stages of progress towards completion",
		...
		ginkgo.Entry("Running MXJob", podsReadyTestSpec{
			jobStatus: kftraining.JobStatus{
		...
		ginkgo.Entry("Running MXJob; PodsReady=False before", podsReadyTestSpec{
			beforeCondition: &metav1.Condition{
				Type:    kueue.WorkloadPodsReady,
				Status:  metav1.ConditionFalse,
				Reason:  "PodsReady",
...
var _ = ginkgo.Describe("Job controller interacting with scheduler", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should schedule jobs as they fit in their ClusterQueue", func() {
		...
		job := testingmxjob.MakeMXJob("dev-job", ns.Name).Queue(localQueue.Name).
		...
		gomega.Expect(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeScheduler].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(spotUntaintedFlavor.Name))
		gomega.Expect(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeServer].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(spotUntaintedFlavor.Name))
		gomega.Expect(createdJob.Spec.MXReplicaSpecs[kftraining.MXJobReplicaTypeWorker].Template.Spec.NodeSelector[instanceKey]).Should(gomega.Equal(onDemandFlavor.Name))

...
var _ = ginkgo.Describe("Job controller", ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.Describe("Job controller for workloads when only jobs with queue are managed", ginkgo.Ordered, func() {
		ginkgo.BeforeAll(func() {
	...
	ginkgo.Describe("Job controller when waitForPodsReady enabled", ginkgo.Ordered, func() {
	...
	ginkgo.Describe("Job controller interacting with scheduler", ginkgo.Ordered, func() {
		var (
			ns                  *corev1.Namespace
		...
		ginkgo.When("The workload's admission is removed", func() {
			ginkgo.It("Should restore the original node selectors", func() {
				...
				ginkgo.By("the node selectors should be updated", func() {
					gomega.Eventually(func() map[kfmpi.MPIReplicaType]map[string]string {
						gomega.Expect(k8sClient.Get(ctx, lookupKey, createdJob)).Should(gomega.Succeed())

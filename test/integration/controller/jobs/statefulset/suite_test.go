...
func managerSetup(configuration *config.Configuration, opts ...jobframework.Option) framework.ManagerSetup {
	...
	return func(ctx context.Context, mgr manager.Manager) {
		...
		statefulsetReconcilerFactory := jobframework.NewNoopReconcilerFactory(
			appsv1.SchemeGroupVersion.WithKind("StatefulSet"),
		)
		statefulsetReconciler := statefulsetReconcilerFactory(
			mgr.GetClient(),
			mgr.GetEventRecorderFor(constants.JobControllerName),
			opts...)
		err = statefulsetReconciler.SetupWithManager(mgr)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())

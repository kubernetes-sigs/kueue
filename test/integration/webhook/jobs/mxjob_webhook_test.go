...
import (
	...
	testingjobsmxjob "sigs.k8s.io/kueue/pkg/util/testingjobs/mxjob"
	"sigs.k8s.io/kueue/test/util"
)

var _ = ginkgo.Describe("MXJob Webhook", ginkgo.Ordered, func() {
	var ns *corev1.Namespace
	ginkgo.BeforeAll(func() {
		fwk.StartManager(ctx, cfg, managerSetup(mxjob.SetupMXJobWebhook))
	})
	...
	ginkgo.It("the creation doesn't succeed if job contain both kueue.x-k8s.io/podset-required-topology and kueue.x-k8s.io/podset-preferred-topology annotations", func() {
		job := testingjobsmxjob.MakeMXJob("job", ns.Name).

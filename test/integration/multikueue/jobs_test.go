...
var _ = ginkgo.Describe("MultiKueue", ginkgo.Label("area:multikueue", "feature:multikueue"), ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("Should handle a pod group admission with an extra non-multikueue admission check defined", func() {
		var testAc *kueue.AdmissionCheck

		ginkgo.By("creating a non-multikueue AC and adding it to the CQ", func() {
		...
		ginkgo.By("checking workload in manager is set up correctly", func() {
			gomega.Eventually(func(g gomega.Gomega) {
				...
				g.Expect(admissioncheck.FindAdmissionCheck(createdWorkload.Status.AdmissionChecks, kueue.AdmissionCheckReference(testAc.Name))).To(gomega.BeComparableTo(&kueue.AdmissionCheckState{
		...
		ginkgo.By("checking the workload creation in the worker clusters didn't happen due to pending non-multikueue AC", func() {
			managerWl := &kueue.Workload{}
			gomega.Expect(managerTestCluster.client.Get(managerTestCluster.ctx, wlLookupKey, managerWl)).To(gomega.Succeed())
			gomega.Eventually(func(g gomega.Gomega) {
		...
		ginkgo.By("Update pending AC to Ready", func() {
			gomega.Eventually(func(g gomega.Gomega) {

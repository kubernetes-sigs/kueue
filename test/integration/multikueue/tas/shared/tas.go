...
func RunTASIntegrationTests(tc *TestContext, managerSetup func(context.Context, manager.Manager)) {
	managerTestCluster := tc.ManagerTestCluster
	worker1TestCluster := tc.Worker1TestCluster
	worker2TestCluster := tc.Worker2TestCluster

	ginkgo.Describe("Multi-cluster Topology Aware Scheduling", ginkgo.Label("area:multikueue", "feature:multikueue", "feature:tas"), ginkgo.Ordered, func() {
		...
		ginkgo.BeforeEach(func() {
			...
			managerMultiKueueSecret1 = utiltesting.MakeSecret("multikueue1", tc.ManagersConfigNamespace.Name).Data(kueue.MultiKueueConfigSecretKey, w1Kubeconfig).Obj()
			gomega.Expect(managerTestCluster.Client.Create(managerTestCluster.Ctx, managerMultiKueueSecret1)).To(gomega.Succeed())

			managerMultiKueueSecret2 = utiltesting.MakeSecret("multikueue2", tc.ManagersConfigNamespace.Name).Data(kueue.MultiKueueConfigSecretKey, w2Kubeconfig).Obj()
		...
		ginkgo.When("Topology has a single level", func() {
			...
			ginkgo.It("should admit workload which fits in a required topology domain", func() {
				...
				wl := &kueue.Workload{}
				wlLookupKey := apitypes.NamespacedName{Name: workloadjob.GetWorkloadNameForJob(job.Name, job.UID), Namespace: managerNs.Name}
					...
					gomega.Eventually(func(g gomega.Gomega) {
						...
						g.Expect(wl.Status.Admission.PodSetAssignments[0].TopologyAssignment).Should(gomega.BeComparableTo(
							utiltas.V1Beta2From(&utiltas.TopologyAssignment{
								...
								Domains: []utiltas.TopologyDomainAssignment{{Count: 1, Values: []string{"host-1"}}},
							}),
						))
					}, util.Timeout, util.Interval).Should(gomega.Succeed())
				})

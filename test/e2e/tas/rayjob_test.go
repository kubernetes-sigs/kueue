...
var _ = ginkgo.Describe("TopologyAwareScheduling for RayJob", func() {
	var (
		ns           *corev1.Namespace
	...
	ginkgo.When("Creating a RayJob", func() {
		ginkgo.It("Should place pods based on the ranks-ordering", func() {
			...
			ginkgo.By("verify the assignment of pods are as expected with rank-based ordering", func() {
				...
				gotAssignment := readRankAssignmentsFromRayJobPods(pods.Items)
				// raycluster pods does not contain the index, thus what we can check is
...
func readRankAssignmentsFromRayJobPods(pods []corev1.Pod) map[string]int {

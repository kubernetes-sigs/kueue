...
func readTopologyAnnotationFromRayJobSubmitterPod(pods []corev1.Pod) (string, string) {
	for _, pod := range pods {
		if role := pod.Labels[kueuealpha.PodSetLabel]; role == "submitter" {
			if v, ok := pod.Annotations[kueuealpha.PodSetRequiredTopologyAnnotation]; ok {
				return kueuealpha.PodSetRequiredTopologyAnnotation, v
			}
			if v, ok := pod.Annotations[kueuealpha.PodSetPreferredTopologyAnnotation]; ok {
				return kueuealpha.PodSetPreferredTopologyAnnotation, v
			}
		}
	}
	return "", ""

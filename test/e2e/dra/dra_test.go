...
var _ = ginkgo.Describe("DRA", func() {
	...
	ginkgo.When("Creating Jobs with Extended Resources backed by DRA", func() {
		...
		ginkgo.It("Should not double-count quota when single job uses both ResourceClaimTemplate and Extended Resource", func() {
			...
			ginkgo.By("Verifying workload is admitted with 3 GPUs total (2 + 1, no double-counting)")
			createdWorkload := &kueue.Workload{}
			gomega.Eventually(func(g gomega.Gomega) {
				g.Expect(k8sClient.Get(ctx, wlLookupKey, createdWorkload)).To(gomega.Succeed())
				g.Expect(workload.HasQuotaReservation(createdWorkload)).To(gomega.BeTrue())

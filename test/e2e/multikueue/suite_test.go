...
var _ = ginkgo.BeforeSuite(func() {
	...
	worker1Kconfig, err := kubeconfigForMultiKueueSA(ctx, k8sWorker1Client, worker1Cfg, kueueNamespace.Name, "mksa", worker1ClusterName)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	gomega.Expect(makeMultiKueueSecret(ctx, k8sManagerClient, kueueNamespace.Name, "multikueue1", worker1Kconfig)).To(gomega.Succeed())

	worker2Kconfig, err := kubeconfigForMultiKueueSA(ctx, k8sWorker2Client, worker2Cfg, kueueNamespace.Name, "mksa", worker2ClusterName)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	gomega.Expect(makeMultiKueueSecret(ctx, k8sManagerClient, kueueNamespace.Name, "multikueue2", worker2Kconfig)).To(gomega.Succeed())

	...
	ginkgo.When("Creating Jobs with multiple ResourceClaimTemplates", func() {
		ginkgo.It("Should correctly account for DRA resources from multiple pods", func() {
			...
			for _, c := range []struct {
				...
				{k8sManagerClient, managerNs.Name},
				{k8sWorker1Client, worker1Ns.Name},
				{k8sWorker2Client, worker2Ns.Name},
			} {
				rct := utiltesting.MakeResourceClaimTemplate("multi-pod-gpu-template", c.ns).
					DeviceRequest("gpu-request", draconsts.DriverName, 1).
					Obj()
				util.MustCreate(ctx, c.client, rct)
			}

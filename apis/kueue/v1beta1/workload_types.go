...
// +kubebuilder:validation:XValidation:rule="(has(oldSelf.status) && has(oldSelf.status.conditions) && oldSelf.status.conditions.exists(c, c.type == 'QuotaReserved' && c.status == 'True')) ? (oldSelf.spec.priorityClassSource == self.spec.priorityClassSource) || (oldSelf.spec.priorityClassSource == 'kueue.x-k8s.io/workloadpriorityclass' && self.spec.priorityClassSource.size() == 0) || (oldSelf.spec.priorityClassSource.size() == 0 && self.spec.priorityClassSource == 'kueue.x-k8s.io/workloadpriorityclass') : true", message="priorityClassSource is immutable while the workload quota is reserved, except for kueue.x-k8s.io/workloadpriorityclass"
// +kubebuilder:validation:XValidation:rule="(has(oldSelf.status) && has(oldSelf.status.conditions) && oldSelf.status.conditions.exists(c, c.type == 'QuotaReserved' && c.status == 'True') && (self.spec.priorityClassSource != 'kueue.x-k8s.io/workloadpriorityclass') && has(oldSelf.spec.priorityClassName) && has(self.spec.priorityClassName)) ? (oldSelf.spec.priorityClassName == self.spec.priorityClassName) : true", message="priorityClassName is immutable while the workload quota is reserved and priorityClassSource is not equal to kueue.x-k8s.io/workloadpriorityclass"

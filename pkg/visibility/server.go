...
func applyVisibilityServerOptions(config *genericapiserver.RecommendedConfig, enableInternalCertManagement bool, tlsOpts *kueuecfg.TLSOptions) error {
	...
	if features.Enabled(features.TLSOptions) && tlsOpts != nil && config.SecureServing != nil {
		minVersion, err := tlsconfig.ConvertTLSMinVersion(tlsOpts.TLSMinVersion)
		if err != nil {
			return fmt.Errorf("error converting TLS min version: %v", err)
		}
		config.SecureServing.MinTLSVersion = minVersion
		cipherSuites, err := tlsconfig.ConvertCipherSuites(tlsOpts.TLSCipherSuites)
		if err != nil {

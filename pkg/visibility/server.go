...
func CreateAndStartVisibilityServer(ctx context.Context, kueueMgr *qcache.Manager, enableInternalCertManagement bool, kubeConfig *rest.Config, visibilityServerFlags string, tlsOpts *tlsconfig.TLS) error {
	config := newVisibilityServerConfig(kubeConfig)
	if err := applyVisibilityServerOptions(config, enableInternalCertManagement, visibilityServerFlags, tlsOpts); err != nil {
...
func applyVisibilityServerOptions(config *genericapiserver.RecommendedConfig, enableInternalCertManagement bool, visibilityServerFlags string, tlsOpts *tlsconfig.TLS) error {

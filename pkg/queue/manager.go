	...
	if features.Enabled(features.LocalQueueMetrics) {
		for _, q := range queues.Items {
			...
			if qImpl != nil {
				...
				m.client.Get(ctx, client.ObjectKey{Name: lqKeySlice[1], Namespace: lqKeySlice[0]}, lqObj)
				if should, _ := metrics.ShouldReportLocalMetrics(ctx, m.client, lqObj); should {

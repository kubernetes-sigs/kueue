...
func TestClusterQueueImpl(t *testing.T) {
	cl := utiltesting.NewFakeClient(
		utiltesting.MakeNamespace("ns1").Label("dep", "eng").Obj(),
		utiltesting.MakeNamespace("ns2").Label("dep", "sales").Obj(),
		utiltesting.MakeNamespace("ns3").Label("dep", "marketing").Obj(),
	)
...
func TestQueueInadmissibleWorkloadsDuringScheduling(t *testing.T) {
	...
	wl := utiltesting.MakeWorkload("workload-1", defaultNamespace).Obj()
	cl := utiltesting.NewFakeClient(wl, utiltesting.MakeDefaultNamespace(defaultNamespace))

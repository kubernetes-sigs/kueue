...
type flavorInformation struct {
	...
	Topologypkg/cache/tas_flavor.goName kueue.TopologyReference
...
type TASFlavorCache struct {
	...
	// of topology levels) which is relevant for TAS-scheduling.
	topology topologyInformation

	// flavor represents the part of the ResourceFlavor specification (e.g. the
	// list of node labels and tolerations) which is relevant for TAS-scheduling.

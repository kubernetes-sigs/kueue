...
func (c *ClusterQueueSnapshot) addTASUsage(wi *workload.Info) {
	c.updateTASUsage(wi, 1)
}

func (c *ClusterQueueSnapshot) removeTASUsage(wi *workload.Info) {
	c.updateTASUsage(wi, -1)
}

func (c *ClusterQueueSnapshot) updateTASUsage(wi *workload.Info, m int) {
		...
		for tasFlavor, tasUsage := range wi.TASUsage() {
			if tasFlvCache := c.TASFlavors[tasFlavor]; tasFlvCache != nil {
				for _, tr := range tasUsage {
					...
					if m == 1 {
						tasFlvCache.addTASUsage(domainID, tr.Requests)
					} else {
						tasFlvCache.removeTASUsage(domainID, tr.Requests)
					}
				}
			}
		}

...
func (s *TASFlavorSnapshot) addNode(node corev1.Node) utiltas.TopologyDomainID {
	...
	capacity := resources.NewResources(node.Status.Allocatable)
...
func (s *TASFlavorSnapshot) addCapacity(domainID utiltas.TopologyDomainID, capacity resources.Resources) {
	if s.leaves[domainID].freeCapacity == nil {
		s.leaves[domainID].freeCapacity = resources.Resources{}
	}
	s.leaves[domainID].freeCapacity.Add(capacity)
}

func (s *TASFlavorSnapshot) addNonTASUsage(domainID utiltas.TopologyDomainID, usage resources.Resources) {
	...
	s.leaves[domainID].freeCapacity.Sub(resources.Resources{corev1.ResourcePods: 1})
}

func (s *TASFlavorSnapshot) updateTASUsage(domainID utiltas.TopologyDomainID, usage resources.Resources, op usageOp, count int32) {
	u := usage.Clone()
	u.Add(resources.Resources{corev1.ResourcePods: int64(count)})
	if op == add {
...
func (s *TASFlavorSnapshot) addTASUsage(domainID utiltas.TopologyDomainID, usage resources.Resources) {
	...
	if s.leaves[domainID].tasUsage == nil {
		s.leaves[domainID].tasUsage = resources.Resources{}
	}
	s.leaves[domainID].tasUsage.Add(usage)
}

func (s *TASFlavorSnapshot) removeTASUsage(domainID utiltas.TopologyDomainID, usage resources.Resources) {
	if s.leaves[domainID].tasUsage == nil {
		s.leaves[domainID].tasUsage = resources.Resources{}
	}
...
func (s *TASFlavorSnapshot) freeCapacityPerDomain() map[utiltas.TopologyDomainID]resources.Resources {
	freeCapacityPerDomain := make(map[utiltas.TopologyDomainID]resources.Resources, len(s.leaves))
...
func (s *TASFlavorSnapshot) tasUsagePerDomain() map[utiltas.TopologyDomainID]resources.Resources {
	tasUsagePerDomain := make(map[utiltas.TopologyDomainID]resources.Resources, len(s.leaves))
...
func (t *TASPodSetRequests) TotalRequests() resources.Resources {
...
func (s *TASFlavorSnapshot) FindTopologyAssignmentsForFlavor(flavorTASRequests FlavorTASRequests, simulateEmpty bool) TASAssignmentsResult {
	...
	assumedUsage := make(map[utiltas.TopologyDomainID]resources.Resources)
	for _, tr := range flavorTASRequests {
		...
		for _, domain := range assignment.Domains {
			...
			if assumedUsage[domainID] == nil {
				assumedUsage[domainID] = resources.Resources{}
			}
...
func (s *TASFlavorSnapshot) findTopologyAssignment(
	...
	assumedUsage map[utiltas.TopologyDomainID]resources.Resources,
	simulateEmpty bool) (*kueue.TopologyAssignment, string) {
	requests := tasPodSetRequests.SinglePodRequests.Clone()
	requests.Add(resources.Resources{corev1.ResourcePods: 1})
...
func (s *TASFlavorSnapshot) fillInCounts(requests resources.Resources,
	assumedUsage map[utiltas.TopologyDomainID]resources.Resources,
	simulateEmpty bool,
	tolerations []corev1.Toleration) {

...
func (s *TASFlavorSnapshot) FindTopologyAssignmentsForFlavor(flavorTASRequests FlavorTASRequests, options ...FindTopologyAssignmentsOption) TASAssignmentsResult {
	...
	for _, trs := range groupedTASRequests {
		if workload.HasNodeToReplace(opts.workload) {
			tr := trs[0] // TODO support node replacement
			...
			for podSetName, assignment := range assignments {
				result[podSetName] = tasPodSetAssignmentResult{TopologyAssignment: assignment, FailureReason: reason}
			}
			...
			for _, assignment := range assignments {
				addAssumedUsage(assumedUsage, assignment, &trs[0]) // TODO select tr from correct podset
			}
		}
	}
...
func (s *TASFlavorSnapshot) findReplacementAssignment(tr *TASPodSetRequests, existingAssignment *kueue.TopologyAssignment, wl *kueue.Workload, assumedUsage map[utiltas.TopologyDomainID]resources.Requests) (*kueue.TopologyAssignment, *kueue.TopologyAssignment, string) {
	...
	replacementAssignment, reason := s.findTopologyAssignment(*tr, tr, assumedUsage, false, requiredReplacementDomain) // TODO support leader and worker
	if reason != "" {
		return nil, nil, reason
	}
	newAssignment := s.mergeTopologyAssignments(replacementAssignment["worker"], existingAssignment) // TODO support leader and worker
	return newAssignment, replacementAssignment["worker"], ""                                        // TODO support leader and worker
}
...
func (s *TASFlavorSnapshot) findTopologyAssignment(
	...
	// TODO Assumptions
	// only two PodSets
	// the bigger one is "workers"
	// the smaller one is "leaders"
	// number of pods in "leaders" podset should be relatively small
	...
	var leaderAssignment *kueue.TopologyAssignment = nil
	if leaderTasPodSetRequests != nil && len(currFitDomain) > 0 {

		copiedDomain := *currFitDomain[0]
		copiedDomain.state = 1
		leaderDomains := []*domain{&copiedDomain}
		leaderAssignment = s.buildAssignment(leaderDomains)
		assignments[leaderTasPodSetRequests.PodSet.Name] = leaderAssignment
	}
...
func (s *TASFlavorSnapshot) findLevelWithFitDomains(levelIdx int, required bool, podSetSize int32, leaderPodSetSize int32, sliceSize int32, unconstrained bool) (int, []*domain, string) {
	...
	if topDomain.sliceStateWithLeader < sliceCount {
		if required {
...
func (s *TASFlavorSnapshot) updateSliceCountsToMinimum(domains []*domain, count int32, leaderCount int32, sliceSize int32, unconstrained bool) []*domain {
	...
	for i, domain := range domains {
		if remainingLeaderCount > 0 {
			...
			if domain.sliceStateWithLeader >= remainingSlices && domain.leaderState >= leaderCount {
				domain.state = remainingSlices * sliceSize
				domain.sliceState = remainingSlices
				domain.leaderState = leaderCount

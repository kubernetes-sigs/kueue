...
func resetResourceStatsFromResourceGroups(orig ResourceStats, resourceGroups []kueue.ResourceGroup) ResourceStats {
	ret := make(ResourceStats, len(resourceGroups))
	for _, rg := range resourceGroups {
		for _, res := range rg.CoveredResources {
			if oStats := orig[res]; oStats != nil {
				ret[res] = &QuotaStats{
					Usage: orig[res].Usage,
					// Reset Nominal and Lendable to be calculated in updateResourceGroups.
				}
			} else {
				ret[res] = &QuotaStats{}
			}
...
func (c *ClusterQueue) updateResourceGroups(in []kueue.ResourceGroup) {
	...
	c.ResourceStats = resetResourceStatsFromResourceGroups(c.ResourceStats, in)
	for i, rgIn := range in {

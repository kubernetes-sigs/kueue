...
func newSliceShape(tr *kueue.PodSetTopologyRequest, count, sliceSize int32) sliceShape {
	...
	if tr == nil || len(tr.PodsetSliceRequiredTopologyConstraints) > 1 {
		return shape
	}
	if len(tr.PodsetSliceRequiredTopologyConstraints) == 0 && (tr.PodSetSliceRequiredTopology == nil || tr.PodSetSliceSize == nil) {
		return shape
	}

...
type topologyAssignmentPodRequirements struct {
	...
	// podSetGroupCounts holds how many Workloads matching the PodSet group's
	// spreading selector already occupy each domain. Nil unless topology
	// spreading applies to the group.
...
func (s *TASFlavorSnapshot) findTopologyAssignment(
	...
	requirements := &topologyAssignmentPodRequirements{
		...
		podSetGroupCounts:         podSetGroupCounts,
	}
	...
	if features.Enabled(features.TASTopologySpreading) && podSetGroupCounts != nil {
...
func (s *TASFlavorSnapshot) HasRequiredSpreadingLevels(spec *utiltas.SpreadingSpec) bool {
	...
	for _, rule := range spec.Rules {
		if rule.EnforcementMode != kueue.TopologySpreadingEnforcementModeRequired {
			continue
		}
...
func (s *TASFlavorSnapshot) bannedBySpreading(d *domain, rules map[int]utiltas.SpreadingRule) bool {
	for ; d != nil; d = d.parent {
		...
		if ok && rule.EnforcementMode == kueue.TopologySpreadingEnforcementModeRequired && evaluateSpreadRule(rule, s.domainStateOf(d)) {
			return true
		}
	}
	return false
...
// Required rule survives either way, because filterBannedDomains has already

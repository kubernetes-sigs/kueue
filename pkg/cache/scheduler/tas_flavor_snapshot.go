...
// leafCapacity is the per-snapshot mutable state of a leaf domain, addressed by
// leafDomain.leafIdx.
...
// leafCapacityOf returns the snapshot's mutable capacity data of the given shared leaf.
...
// nodeExclusionType classifies the reason why a node was excluded from scheduling.
...
func (s *TASFlavorSnapshot) findReplacementAssignment(
	...
	if !s.assignmentSliceAligned(newAssignment, tr.PodSet.TopologyRequest, sliceSize) {
		...
		return nil, nil, fmt.Sprintf("cannot replace the node %v without splitting a PodSet slice", wl.Status.UnhealthyNodes[0].Name)
	}
...
func (s *TASFlavorSnapshot) requiredReplacementDomain(tr *TASPodSetRequests, ta *utiltas.TopologyAssignment) utiltas.TopologyDomainID {
	...
	if slicesRequested(tr.PodSet.TopologyRequest) && (tr.Count%sliceSize != 0) {
		...
		return s.findIncompleteSliceDomain(ta, tr.Count, sliceSize, s.sliceLevelKeyWithDefault(tr.PodSet.TopologyRequest, s.lowestLevel()))
	}
...
func (s *TASFlavorSnapshot) findTopologyAssignment(
	...
	if s.isLowestLevelNode {
		...
		if features.Enabled(features.TASCacheNodeMatchResults) && wl != nil && wl.UID != "" {
			requirements.matchKey = &podSetMatchKey{
				workloadUID: wl.UID,
				podSetName:  workersTasPodSetRequests.PodSet.Name,
			}
		}
	} else {
		...
		for _, domain := range currFitDomain {
			...
			if sliceSizeOnLevel > 1 {
				...
				for _, d := range sortedLowerDomains {
					dState := s.domainStateOf(d)
					dState.sliceCount = dState.podCount / sliceSizeOnLevel
					dState.sliceCountWithLeader = dState.podCountWithLeader / sliceSizeOnLevel
				}
			}
	...
	assignments[workerPodSetName] = s.buildAssignment(currFitDomain)
	...
	if state.tailSize > 0 {
		...
		if utiltas.CountPodsInAssignment(workerAssignment) != state.count || !s.assignmentSliceAligned(workerAssignment, topologyRequest, state.sliceSize) {
			...
			return nil, fmt.Sprintf("cannot place the partial slice of PodSet %s in a single topology domain", workerPodSetName)
		}
	}
...
func (s *TASFlavorSnapshot) findLevelWithFitDomains(
	...
	if useBestFitAlgorithm(state.unconstrained) && s.domainStateOf(topDomain).leaderCount >= state.leaderCount &&
		...
		if hasTail {
			topDomain = s.findBestFitDomainForSlicesWithTail(sortedDomain, sliceCount, state.leaderCount)
		} else {
			...
			if features.Enabled(features.TASRespectNodeAffinityPreferred) {
				for i := 1; i < len(sortedDomain); i++ {
					...
					if s.domainStateOf(d).fitsSlices(sliceCount, state.leaderCount, hasTail) {
						if hasTail {
							return searchLevelIdx, []*domain{s.findBestFitDomainForSlicesWithTail(sortedDomain[i:], sliceCount, state.leaderCount)}, ""
						}
...
func (s *TASFlavorSnapshot) updateCountsToMinimumGeneric(domains []*domain, count int32, leaderCount int32, shape sliceShape, unconstrained bool, distributeSlices bool) []*domain {
	...
	for i, dom := range domains {
		...
		if distributeSlices {
			...
			domState := s.domainStateOf(dom)
			domState.leaderCount = 0
			if domState.sliceCount >= remainingPrimary {
				domState.podCount = remainingPrimary * shape.size
				domState.sliceCount = remainingPrimary
				result = append(result, dom)
				return s.finishSliceDistribution(result, domains, shape, count, tailPending)
			}
			domState.podCount = domState.sliceCount * shape.size
			remainingPrimary -= domState.sliceCount
			result = append(result, dom)
		...
		domState := s.domainStateOf(dom)
		domState.leaderCount = 0
		if domState.podCount >= remainingPrimary {
			domState.podCount = remainingPrimary
			result = append(result, dom)
			return result
		}
		remainingPrimary -= domState.podCount
		result = append(result, dom)
	}
...
func (s *TASFlavorSnapshot) sortedDomainsWithLeader(domains []*domain, unconstrained bool) []*domain {
	...
	slices.SortFunc(result, func(a, b *domain) int {
		...
		if aState.leaderCount != bState.leaderCount {
			return cmp.Compare(bState.leaderCount, aState.leaderCount)
		}
		...
		if aState.sliceCountWithLeader != bState.sliceCountWithLeader {
			if isLeastFreeCapacity {
				// Start from the domain with the least amount of free resources.
				// Ascending order.
				return cmp.Compare(aState.sliceCountWithLeader, bState.sliceCountWithLeader)
			}
			return cmp.Compare(bState.sliceCountWithLeader, aState.sliceCountWithLeader)
		}

		if aState.podCountWithLeader != bState.podCountWithLeader {
			return cmp.Compare(aState.podCountWithLeader, bState.podCountWithLeader)
		}
...
func (s *TASFlavorSnapshot) sortedDomains(domains []*domain, unconstrained bool) []*domain {
	...
	slices.SortFunc(result, func(a, b *domain) int {
		...
		if aState.sliceCount != bState.sliceCount {
			if isLeastFreeCapacity {
				// Start from the domain with the least amount of free resources.
				// Ascending order.
				return cmp.Compare(aState.sliceCount, bState.sliceCount)
			}
			return cmp.Compare(bState.sliceCount, aState.sliceCount)
		}

		if aState.podCount != bState.podCount {
			return cmp.Compare(aState.podCount, bState.podCount)
		}
...
func (s *TASFlavorSnapshot) fillInCounts(requirements *topologyAssignmentPodRequirements, state *findTopologyAssignmentState) {
	...
	if features.Enabled(features.TASCacheNodeMatchResults) {
		...
		for _, ml := range matchingLeaves {
			s.domainStateOf(&ml.leaf.domain).affinityScore += ml.affinityScore
		...
		for _, leaf := range s.leaves {
			...
			if s.isLowestLevelNode {
				...
				s.domainStateOf(&leaf.domain).affinityScore += affinityScore
			}
...
func (s *TASFlavorSnapshot) getMatchingLeaves(requirements *topologyAssignmentPodRequirements) ([]matchedLeaf, *ExclusionStats) {
	if !s.isLowestLevelNode {
		stats := newExclusionStats()
		stats.TotalNodes += len(s.leaves)
		result := make([]matchedLeaf, 0, len(s.leaves))
		for _, leaf := range s.leaves {
			result = append(result, matchedLeaf{leaf: leaf})
		}
		return result, stats
	}
	...
	entry := &matchingLeavesCacheEntry{
		leaves: make([]matchedLeaf, 0, len(s.leaves)),
		stats:  newExclusionStats(),
	}

	for _, leaf := range s.leaves {
		entry.stats.TotalNodes++

		excluded, affinityScore, taint, exclusionType := s.matchNode(leaf, requirements)
		if excluded {
			entry.stats.recordExclusion(exclusionType, taint)
			continue
		}

		entry.leaves = append(entry.leaves, matchedLeaf{leaf: leaf, affinityScore: affinityScore})
	}
...
func (s *TASFlavorSnapshot) fillInCountsHelper(domain *domain, shape sliceShape, sliceLevelIdx int, level int, sliceSizeAtLevel map[int]int32, leaderRequired bool) {
	...
	if level == sliceLevelIdx {
		...
		sliceCapacity = domainState.podCount / shape.size

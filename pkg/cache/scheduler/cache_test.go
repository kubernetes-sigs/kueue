...
func TestAddOrUpdateDynamicResourceAllocationConfig(t *testing.T) {
	_, log := utiltesting.ContextWithLog(t)
	newCfg := func(resources ...kueuealpha.DynamicResource) *kueuealpha.DynamicResourceAllocationConfig {
		return &kueuealpha.DynamicResourceAllocationConfig{
			Spec: kueuealpha.DynamicResourceAllocationConfigSpec{Resources: resources},
		}
	}
	toRes := func(name string, dcs ...string) kueuealpha.DynamicResource {
		dcNames := make([]corev1.ResourceName, 0, len(dcs))
		for _, dc := range dcs {
			dcNames = append(dcNames, corev1.ResourceName(dc))
		}
		return kueuealpha.DynamicResource{Name: corev1.ResourceName(name), DeviceClassNames: dcNames}
	}

	type expect struct {
		mapped  map[corev1.ResourceName]corev1.ResourceName
		missing []corev1.ResourceName
		apply   []*kueuealpha.DynamicResourceAllocationConfig
	}
	...
	cases := map[string]expect{
		"build mapping": {
			apply: []*kueuealpha.DynamicResourceAllocationConfig{
				newCfg(
					toRes("whole-gpus", "example.com/foo", "example.com/bar"),
					toRes("shared-gpus", "example.com/baz"),
				),
			},
			mapped: map[corev1.ResourceName]corev1.ResourceName{
				"example.com/foo": "whole-gpus",
				"example.com/bar": "whole-gpus",
				"example.com/baz": "shared-gpus",
			},
			missing: []corev1.ResourceName{"example.com/unknown"},
		},
		"overwrite clears previous": {
			apply: []*kueuealpha.DynamicResourceAllocationConfig{
				newCfg(
					toRes("whole-gpus", "example.com/foo", "example.com/bar"),
				),
				newCfg(
					toRes("shared-gpus", "example.com/foo"),
				),
			},
			mapped: map[corev1.ResourceName]corev1.ResourceName{
				"example.com/foo": "shared-gpus",
			},
			missing: []corev1.ResourceName{"example.com/bar"},
		},
		"duplicate last wins": {
			apply: []*kueuealpha.DynamicResourceAllocationConfig{
				newCfg(
					toRes("whole-gpus", "example.com/foo"),
					toRes("shared-gpus", "example.com/foo"),
				),
			},
			mapped: map[corev1.ResourceName]corev1.ResourceName{
		...
		"empty clears all": {
			...
			mapped:  map[corev1.ResourceName]corev1.ResourceName{},
			missing: []corev1.ResourceName{"example.com/foo"},
		},
		"idempotent": {
			apply: []*kueuealpha.DynamicResourceAllocationConfig{
				newCfg(
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			c := New(utiltesting.NewFakeClient())
			for _, cfg := range tc.apply {
				c.AddOrUpdateDynamicResourceAllocationConfig(log, cfg)
			}

			for dc, want := range tc.mapped {
				if got, ok := c.GetResourceNameForDeviceClass(dc); !ok || got != want {
					t.Fatalf("mapping for %s got=%q ok=%v; want=%q true", dc, got, ok, want)
				}
			}
			for _, dc := range tc.missing {
				if _, ok := c.GetResourceNameForDeviceClass(dc); ok {
					t.Fatalf("expected %s to be missing", dc)
				}
			}
		})
	}

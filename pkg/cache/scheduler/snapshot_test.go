...
func TestSnapshot(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			// release-0.19's AddFlavor ignores label changes to a known flavor,
			// so relabel it in the cache directly; the usage recorded above stays
			// on the node the flavor no longer selects.

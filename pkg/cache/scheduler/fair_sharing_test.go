...
func TestDRS_IsBorrowing(t *testing.T) {
	cases := map[string]struct {
		...
		"borrowing with non-zero ratio": {
			drs:           DRS{fairWeight: 1, unweightedRatio: 100, borrowing: true},
			wantBorrowing: true,
			wantIsZero:    false,
		},
		"not borrowing with zero ratio": {
			drs:           DRS{fairWeight: 1, unweightedRatio: 0, borrowing: false},
			wantBorrowing: false,
			wantIsZero:    true,
		},
		"borrowing with zero ratio": {
			drs:           DRS{fairWeight: 1, unweightedRatio: 0, borrowing: true},
			wantBorrowing: true,
			wantIsZero:    true,
		},
	}
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			if got := tc.drs.IsBorrowing(); got != tc.wantBorrowing {
				t.Errorf("IsBorrowing() = %v, want %v", got, tc.wantBorrowing)
			}
			if got := tc.drs.IsZero(); got != tc.wantIsZero {

...
func TestSnapshotReuseAfterBalancedPlacement(t *testing.T) {
	...
	snapshot := fc.snapshot(log, nil)

	requests := treeTestBalancedRequests("workers")
	first := snapshot.FindTopologyAssignmentsForFlavor(log, requests)
	...
	second := snapshot.FindTopologyAssignmentsForFlavor(log, requests)
...
func TestSnapshotsSharingTreeCanAssignConcurrently(t *testing.T) {
	...
	for i := range snapshots {
		snapshots[i] = fc.snapshot(log, nil)
	}
	...
	for _, snapshot := range snapshots {
		go func(snapshot *TASFlavorSnapshot) {
			...
			for range 20 {
				result = snapshot.FindTopologyAssignmentsForFlavor(log, treeTestBalancedRequests("workers"))
	...
	tests := map[string]struct {
		...
		"allocatable change": {
			...
			validate: func(t *testing.T, snapshot *TASFlavorSnapshot) {
				n1State := snapshot.leafCapacityOf(snapshot.leaves[utiltas.TopologyDomainID("n1")])
...
func TestTopologyTreeStateIndexesWhenDomainIDsCollide(t *testing.T) {
	...
	snapshot := newTASFlavorSnapshot(log, "default", tree, nil)

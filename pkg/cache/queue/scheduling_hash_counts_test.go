...
func TestSchedulingHashCounts(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			gotActive, gotInadmissible := counts.pendingLens()
			if gotActive != tc.wantActive {
				t.Errorf("pendingLens() active = %d, want %d", gotActive, tc.wantActive)
			}
			if gotInadmissible != tc.wantInadmissible {
				t.Errorf("pendingLens() inadmissible = %d, want %d", gotInadmissible, tc.wantInadmissible)
			}
			if got := counts.pendingUnionLen(); got != tc.wantUnion {
...
// TestSchedulingHashCountsInadmissibleTransitions verifies that hash counts
// follow the single-bucket invariant: an AddFromLocalQueue resync must not
// double-track a workload already stored as inadmissible, and later
// transitions operate on the stored inadmissible copy.
func TestSchedulingHashCountsInadmissibleTransitions(t *testing.T) {
	features.SetFeatureGateDuringTest(t, features.SchedulingEquivalenceHashing, true)

	ctx, log := utiltesting.ContextWithLog(t)
	now := time.Now()
	tests := map[string]struct {
		transition             func(t *testing.T, cq *ClusterQueue, storedInfo, resyncInfo *workload.Info)
		wantActive             int
		wantInadmissible       int
		wantActiveCounts       map[workload.EquivalenceHash]int
		wantInadmissibleCounts map[workload.EquivalenceHash]int
		...
		"queue inadmissible workloads": {
			transition: func(t *testing.T, cq *ClusterQueue, _, _ *workload.Info) {
				...
				if moved := queueInadmissibleWorkloads(ctx, cq, utiltesting.NewFakeClient(utiltesting.MakeNamespace(defaultNamespace))); moved != 1 {
					t.Fatalf("queueInadmissibleWorkloads() = %d, want 1", moved)
				}
			},
			wantActive:       1,
			wantActiveCounts: map[workload.EquivalenceHash]int{"stored-hash": 1},
			wantCPU:          true,
		},
		"delete": {
			transition: func(t *testing.T, cq *ClusterQueue, storedInfo, _ *workload.Info) {
				cq.Delete(log, workloadKey(storedInfo))
			},
		},
		"immediate requeue": {
			transition: func(t *testing.T, cq *ClusterQueue, _, resyncInfo *workload.Info) {
				// The stored inadmissible copy moves to the heap, not the requeued one.
				if pushed := cq.requeueIfNotPresent(log, resyncInfo, true, RequeueReasonGeneric, ""); !pushed {
					t.Fatal("requeueIfNotPresent() = false, want true")
				}
			},
			wantActive:       1,
			wantActiveCounts: map[workload.EquivalenceHash]int{"stored-hash": 1},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			tc.transition(t, cq, storedInfo, resyncInfo)

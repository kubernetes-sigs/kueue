...
// RebuildLocalQueue reheapifies the pending workloads after a LocalQueue's
...
// the whole invariant is re-established in O(n) with heap.Init. lqName identifies
// the LocalQueue whose reconcile triggered the rebuild.
func (c *ClusterQueue) RebuildLocalQueue(log logr.Logger, lqName string) {
	c.rwm.Lock()
	defer c.rwm.Unlock()

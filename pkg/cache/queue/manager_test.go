...
// they order by creation time (a1, b1, a2, b2). Raising lq-a's weight halves its
// fair-sharing usage, so both lq-a workloads must move ahead of both lq-b ones.
// Fixing the moved entries one by one can leave the heap invariant broken and pop
// the wrong workload; a single whole-heap rebuild keeps every pop correct. Entry
// penalties are left at zero so Pop's penalty-driven rebuildAll cannot mask a
// missing reheapify in UpdateLocalQueueWeight.
func TestUpdateLocalQueueWeightReheapifiesMultipleWorkloads(t *testing.T) {
	...
	// Raise lq-a's weight so its usage (4/2) drops below lq-b's (4/1); both
	// lq-a workloads must now pop before both lq-b workloads.
	lqA.Spec.FairSharing.Weight = new(resource.MustParse("2"))
	if err := manager.UpdateLocalQueue(log, lqA); err != nil {
		t.Fatalf("Failed updating LocalQueue lq-a: %v", err)
	}

	got := popNamesFromCQ(manager.hm.ClusterQueue("cq"))
	want := []workload.Reference{"default/wl-a1", "default/wl-a2", "default/wl-b1", "default/wl-b2"}
	if diff := cmp.Diff(want, got); diff != "" {

	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			kClient := utiltesting.NewClientBuilder().
				WithObjects(makeWorkload(tc.clientGeneration), lq).
			...
			if err := manager.AddOrUpdateWorkload(ctx, log, makeWorkload(1), draCharges(1)); err != nil {
				t.Fatalf("Failed adding workload: %v", err)
			}
			// Check the workload out the way the scheduler does, so the Info
			// remembers which ClusterQueue it was borrowed from.
			head := manager.PopFrom("cq")
			if head == nil {
			...
			if tc.capturedGeneration != 0 {
				...
				if err := manager.AddOrUpdateWorkload(ctx, log, makeWorkload(tc.capturedGeneration), draCharges(tc.capturedGPUs)); err != nil {
			...
			wantRequests := []workload.PodSetResources{{
				...
				PerPodRequests: resources.NewRequestsFromMap(map[corev1.ResourceName]int64{"gpu": tc.wantGPUs}),
				Count:          1,
			}}
			if diff := gocmp.Diff(wantRequests, requeued.TotalRequests,
				cmpopts.IgnoreFields(workload.PodSetResources{}, "Flavors"),
				gocmp.Comparer(resources.Equal)); diff != "" {
			...
			if !tc.wantHashUnknown {
				...
				want := workload.NewInfo(log, requeued.Obj, workload.WithTotalRequestsFrom(requeued.TotalRequests))

...
// TestHeadsFairSharingLookAhead verifies that heads() pops up to
// FairSharingLookAheadDepth workloads per ClusterQueue when Fair Sharing is
// enabled and the FairSharingLookAhead gate is on, and exactly one per CQ
// otherwise.
func TestHeadsFairSharingLookAhead(t *testing.T) {
	now := time.Now().Truncate(time.Second)
	clusterQueues := []*kueue.ClusterQueue{
		utiltestingapi.MakeClusterQueue("active-fooCq").Obj(),
		utiltestingapi.MakeClusterQueue("active-barCq").Obj(),
	...
	cases := map[string]struct {
		...
		"look-ahead on: two heads from fooCq, one from barCq": {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			features.SetFeatureGateDuringTest(t, features.FairSharingLookAhead, tc.gateEnabled)
			ctx, log := utiltesting.ContextWithLog(t)
			ctx, cancel := context.WithTimeout(ctx, headsTimeout)
			defer cancel()
			fakeC := &fakeStatusChecker{}

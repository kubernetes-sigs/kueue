...
type schedulingHashCounts struct {
	...
	// inflight counts the hashes of the workloads in ClusterQueue.inflight.
	// It is folded into the counts at read time because Pop moves a workload
	// out of the heap while it is still being scheduled, and the scheduler may
...
// moveActiveToInflight moves wInfo's hash from the active to the inflight
// bucket, mirroring Pop handing the workload to the scheduler.
...
// removeInflight forgets wInfo's hash, mirroring its entry in
// ClusterQueue.inflight being cleared.

...
//nolint:staticcheck //SA1019: CredentialsProviders is validated for backward compatibility.
func convertClusterProfileCredentialsProviders(
	providers []configapi.ClusterProfileCredentialsProvider,
) []configapi.ClusterProfileAccessProvider {
	if len(providers) == 0 {
		return nil
	}
	converted := make([]configapi.ClusterProfileAccessProvider, len(providers))
	for i, provider := range providers {
		converted[i] = configapi.ClusterProfileAccessProvider(provider)
	}
	return converted
}

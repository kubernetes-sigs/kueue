...
func TestAmountFromQuantity(t *testing.T) {
	cases := map[string]struct {
		...
		"one milli past it downward": {name: corev1.ResourceCPU, qty: "-9223372036854775808m", want: "-9223372036854775808"},
...
// Adding a workload and removing it again leaves the ledger where it started,
...
func TestAmountAsApproximateFloat64(t *testing.T) {
	// No Quantity spelling reaches past float64, so the last case is doubled into it.
	...
	cases := map[string]struct {
		...
		"cpu at 1E":            {name: corev1.ResourceCPU, amount: AmountFromQuantity(corev1.ResourceCPU, resource.MustParse("1E")), want: 1e18},
		"cpu at 10P":           {name: corev1.ResourceCPU, amount: AmountFromQuantity(corev1.ResourceCPU, resource.MustParse("10P")), want: 1e16},
		"past float64 is +Inf": {name: "example.com/gpu", amount: huge, want: math.Inf(1)},
	}

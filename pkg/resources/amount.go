...
// converting milliCPU to CPU. An amount at or past math.MaxInt64 is reported
// as +Inf, as the unlimited sentinel was before amounts became exact.
func (a Amount) AsApproximateFloat64(name corev1.ResourceName) float64 {
	if a.CmpInt64(math.MaxInt64) >= 0 {
		return math.Inf(1)
	}
	f := float64(a.asSaturatedInt64())
	if name == corev1.ResourceCPU {
		return f / 1000
	}
	return f
...
// PerThousandOf returns a over b in thousandths as a float64, or 0 when b is
// zero. The division is taken on the exact values and only the result is
// rounded, so operands past the range float64 holds exactly do not move the
// answer.

...
func findSlices(ctx context.Context, clnt client.Client, jobObject client.Object, jobObjectGVK schema.GroupVersionKind, predicate predicate) ([]kueue.Workload, error) {
	...
	if err := clnt.List(ctx, list, client.InNamespace(jobObject.GetNamespace()), client.MatchingFields{indexer.WorkloadOwnerIndexKey(jobObjectGVK): jobObject.GetName()}); err != nil {
		return nil, err
	}

...
func NeedsSecondPass(w *kueue.Workload) bool {
	if workloadfinish.IsFinished(w) || workloadevict.IsEvicted(w) || !HasQuotaReservation(w) {
		return false
	}
...
func IsAdmissible(w *kueue.Workload) bool {
	return !HasAdmissionGate(w) && !workloadfinish.IsFinished(w) && IsActive(w) && !HasQuotaReservation(w)
}

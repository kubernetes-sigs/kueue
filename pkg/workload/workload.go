...
func FinalizeOrphanedWorkload(ctx context.Context, c client.Client, clk clock.Clock, wl *kueue.Workload, canFinish bool, tracker *roletracker.RoleTracker) error {
	...
	if features.Enabled(features.FinishOrphanedWorkloads) && wl.DeletionTimestamp.IsZero() && canFinish {
		...
		if err := Finish(ctx, c, wl, kueue.WorkloadFinishedReasonOwnerNotFound,
			"The workload's owner no longer exists", clk, tracker); err != nil {

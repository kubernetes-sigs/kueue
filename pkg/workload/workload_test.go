...
func TestUpdateWithTotalRequestsFromClones(t *testing.T) {
	...
	_, log := utiltesting.ContextWithLog(t)
	info := NewInfo(log, utiltestingapi.MakeWorkload("wl", "ns").Request("example.com/gpu", "2").Obj())
	info.Update(log, utiltestingapi.MakeWorkload("wl", "ns").Request("example.com/gpu", "2").Obj(),
		WithTotalRequestsFrom(source))

	source[0].Requests.Mul(4)
	source[0].PerPodRequests.Mul(4)

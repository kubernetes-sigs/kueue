...
func TestCompareWaitingForPreemption(t *testing.T) {
	waitingWl := utiltestingapi.MakeWorkload("waiting", "ns").
		Condition(metav1.Condition{
			Type:   kueue.WorkloadQuotaReserved,
			Status: metav1.ConditionFalse,
	...
	notWaitingWl := utiltestingapi.MakeWorkload("not-waiting", "ns").
		Condition(metav1.Condition{
			Type:   kueue.WorkloadQuotaReserved,
			Status: metav1.ConditionFalse,
			Reason: "Pending",
	...
	admittedWl := utiltestingapi.MakeWorkload("admitted", "ns").
		Condition(metav1.Condition{
			Type:   kueue.WorkloadQuotaReserved,
			Status: metav1.ConditionTrue,
		}).
	...
	cases := map[string]struct {
		...
		"a waiting, b not waiting": {
			a:    waitingWl,
			b:    notWaitingWl,
			want: -1,
		},
		"a not waiting, b waiting": {
			a:    notWaitingWl,
			b:    waitingWl,
			want: 1,
		},
		...
		"neither waiting (both not waiting)": {
			a:    notWaitingWl,
			b:    admittedWl,
			want: 0,
		},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			got := CompareWaitingForPreemption(tc.a, tc.b)
			if got != tc.want {
				t.Errorf("CompareWaitingForPreemption() = %d, want %d", got, tc.want)
			}
		})
	}
}

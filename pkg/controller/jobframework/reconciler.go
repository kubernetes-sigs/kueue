	...
	if evCond := apimeta.FindStatusCondition(wl.Status.Conditions, kueue.WorkloadEvicted); evCond != nil && evCond.Status == metav1.ConditionTrue {
		...
		if workload.HasQuotaReservation(wl) {
			if !job.IsActive() {
				...
				if err := r.clearAdmissionAfterEviction(ctx, wl, evCond); err != nil {
...
func (r *JobReconciler) finalizeWorkloads(ctx context.Context, key types.NamespacedName, job GenericJob) error {
	...
	for i := range workloads {
		...
		if hasLiveOwner {
			if evCond := apimeta.FindStatusCondition(wl.Status.Conditions, kueue.WorkloadEvicted); evCond != nil && evCond.Status == metav1.ConditionTrue && workload.HasQuotaReservation(wl) {
				if err := r.clearAdmissionAfterEviction(ctx, wl, evCond); err != nil {
...
func (r *JobReconciler) hasLiveNonPodOwner(ctx context.Context, wl *kueue.Workload) (bool, error) {
	...
	for i := range wl.OwnerReferences {
		...
		if ownerGVK == corev1.SchemeGroupVersion.WithKind("Pod") {
			continue
		}

		ownerRuntimeObject, err := r.client.Scheme().New(ownerGVK)
		...
		ownerObject, ok := ownerRuntimeObject.(client.Object)
		if !ok {
			return false, fmt.Errorf("owner %s is not a client object", ownerGVK)
		}

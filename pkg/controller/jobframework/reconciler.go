...
func (r *JobReconciler) ReconcileGenericJob(ctx context.Context, req ctrl.Request, job GenericJob) (result ctrl.Result, err error) {
	...
	if dropFinalizers {
		...
		if cJob, isComposable := job.(ComposableJob); isComposable {
			...
			if err := r.client.List(ctx, workloads, client.InNamespace(req.Namespace), indexer.WorkloadOwnerReferenceIndexFieldMatcher(job.GVK(), req.Name)); err != nil {
...
func FindMatchingWorkloads(ctx context.Context, c client.Client, job GenericJob) (match *kueue.Workload, toDelete []*kueue.Workload, err error) {
	...
	workloads := &kueue.WorkloadList{}
	if err := c.List(ctx, workloads, client.InNamespace(object.GetNamespace()), indexer.WorkloadOwnerReferenceIndexFieldMatcher(job.GVK(), object.GetName())); err != nil {
		return nil, nil, err
	}

...
// WorkloadName returns a deterministic name for a workload based on its owner's properties,
// including its generation.
//
// This function differs from GetWorkloadNameForOwnerWithGVK by incorporating the owner's generation
// into the suffix calculation.  This ensures that the generated workload name is unique even when
// the same owner object (e.g., a Job) is updated and its generation changes, which is important for
// scenarios where workloads need to be recreated or managed differently based on the owner's version.
func WorkloadName(owner client.Object, ownerGVK schema.GroupVersionKind) string {
	return workloadPrefix(uint(maxPrefixLength), ownerGVK.Kind, owner.GetName()) + nameDelimiter +
		workloadSuffix(hashLength, ownerGVK.Kind, ownerGVK.Group, owner.GetName(), string(owner.GetUID()),

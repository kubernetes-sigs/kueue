...
func TestValidateUpdateForQueueName(t *testing.T) {
	...
	for tcName, tc := range testCases {
		t.Run(tcName, func(t *testing.T) {
			...
			newGJ.EXPECT().Object().Return(tc.newJob).AnyTimes()
			newGJ.EXPECT().IsSuspended().Return(ptr.Deref(tc.newJob.Spec.Suspend, false)).AnyTimes()

			gotErr := jobframework.ValidateUpdateForQueueName(oldGJ, newGJ, func(string) bool { return tc.nsHasDefaultQueue })
			if diff := cmp.Diff(tc.wantErr, gotErr, cmpopts.IgnoreFields(field.Error{}, "BadValue", "Detail")); diff != "" {
				t.Errorf("Unexpected error (-want,+got):\n%s", diff)
			}
		})

...
func TestNotifyCRDAvailable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			manager.crdNotifiersMu.RLock()
			_, exists := manager.crdNotifiers[tt.wantGVK]
			manager.crdNotifiersMu.RUnlock()
			if exists {
				t.Errorf("Expected notifier entry for %v to be removed after signaling", tt.wantGVK)
			}
		})
	}
}

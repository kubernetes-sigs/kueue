...
func (w *wlReconciler) reconcileGroup(ctx context.Context, group *wlGroup) (reconcile.Result, error) {
	...
	// calculate nominated clusters
	// when time is up, there will be no nominated,

	// 5141: This is common part
	...
	// 5141: This is common part, just work on the single, subset or all of them...in fact just subset
	...
	// 5141: This is common part, it is first reserving out of subset
	// 3. get the first reserving
	hasReserving, reservingRemote := group.FirstReserving()
	if hasReserving {
		// 5141: same here bc it's removal of remaining part of the subset
...
func newWlReconciler(c client.Client, helper *multiKueueStoreHelper, cRec *clustersReconciler, origin string,
	...
	adapters map[string]jobframework.MultiKueueAdapter, dispatcherName string, dispatcherRoundTimeout time.Duration, opts ...Option) *wlReconciler {

...
type remoteClient struct {
	...
	// Guards rc.client so a concurrent getClient() never observes a torn value
	// while updateConfigAndRefreshWatchers swaps it on a reconnect. See #12557.
	mu sync.RWMutex
}
...
func (rc *remoteClient) getClient() client.WithWatch {
	rc.mu.RLock()
	defer rc.mu.RUnlock()
	return rc.client
}
...
func (rc *remoteClient) setClient(c client.WithWatch) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
	rc.client = c
}

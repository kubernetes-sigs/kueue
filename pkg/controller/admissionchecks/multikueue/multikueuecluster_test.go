...
func TestUpdateConfig(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			reconciler := newClustersReconciler(c, TestNamespace, 0, defaultOrigin, nil, adapters, tc.cpCreds, nil, nil)
...
func TestReconnectBackoff(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			adapters, _ := jobframework.GetMultiKueueAdapters(sets.New("batch/job"))
			reconciler := newClustersReconciler(c, TestNamespace, 0, defaultOrigin, nil, adapters, &testClusterProfileCreds{}, nil, nil)
			reconciler.rootContext = ctx
...
func TestUpdateStatusRefreshesObservedGeneration(t *testing.T) {
	...
	reconciler := newClustersReconciler(managerClient, TestNamespace, 0, defaultOrigin, nil, nil, nil, nil, nil)
...
func TestClustersReconcilerEventFilters(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			reconciler := newClustersReconciler(c, TestNamespace, 0, defaultOrigin, newKubeConfigFSWatcher(), nil, &NoOpClusterProfileCreds{}, nil, nil)
			reconciler.rootContext = ctx
...
func TestSetRemoteClientConfigDoesNotBlockOtherClusters(t *testing.T) {
	...
	reconciler := newClustersReconciler(localClient, TestNamespace, 0, defaultOrigin, nil, nil, &NoOpClusterProfileCreds{}, nil, nil)
	reconciler.rootContext = ctx
...
func TestClustersReconcilerWorkerClientConstruction(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			adapters, _ := jobframework.GetMultiKueueAdapters(sets.New("batch/job"))
			reconciler := newClustersReconciler(c, TestNamespace, 0, defaultOrigin, nil, adapters, &NoOpClusterProfileCreds{}, nil, tc.clientConn)
			reconciler.rootContext = ctx

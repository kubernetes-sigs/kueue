...
func (r *nodeFailureReconciler) listPodsForWorkload(ctx context.Context, wl *kueue.Workload) ([]corev1.Pod, error) {
	...
	indexKey, indexValue := indexer.PodIndexKeyValue(wl.Name, workloadSliceName)

	var podList corev1.PodList
	if err := r.client.List(ctx, &podList, client.InNamespace(wl.Namespace), client.MatchingFields{indexKey: indexValue}); err != nil {
		return nil, err
	}

...
const (
	...
	NodeHostnameKey               = "metadata.hostname"
...
func indexNodeHostname(o client.Object) []string {
	node, ok := o.(*corev1.Node)
	if !ok {
		return nil
	}
	return []string{utiltas.NodeHostname(node)}
}
...
func indexAdmittedWorkloadNodes(o client.Object) []string {
	...
	if !ok || workloadfinish.IsFinished(wl) || workloadevict.IsEvicted(wl) {
		return nil
	}
...
func SetupIndexes(ctx context.Context, indexer client.FieldIndexer) error {
	...
	if err := indexer.IndexField(ctx, &corev1.Node{}, NodeHostnameKey, indexNodeHostname); err != nil {
		return fmt.Errorf("setting index on %s for Node: %w", NodeHostnameKey, err)
	}

...
func (r *LocalQueueReconciler) Create(e event.CreateEvent) bool {
	...
	if should, _ := metrics.ShouldReportLocalMetrics(r.client, q); should {
...
func (r *LocalQueueReconciler) Update(e event.UpdateEvent) bool {
	...
	oldShould, _ := metrics.ShouldReportLocalMetrics(r.client, oldLq)
	newShould, _ := metrics.ShouldReportLocalMetrics(r.client, newLq)

	if oldShould && !newShould {
		metrics.ClearLocalQueueMetrics(metrics.LocalQueueReference{
			Name:      oldLq.Name,
			Namespace: oldLq.Namespace,
		})
	}
...
func (r *LocalQueueReconciler) UpdateStatusIfChanged(
	...
	if len(conditionStatus) != 0 && len(reason) != 0 && len(msg) != 0 {
		...
		if should, _ := metrics.ShouldReportLocalMetrics(r.client, queue); should {

...
func (r *WorkloadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if workload.IsActive(&wl) {
		if apimeta.IsStatusConditionTrue(wl.Status.Conditions, kueue.WorkloadDeactivationTarget) {
			...
			workload.ReportEvictionCompleted(&wl, wl.Spec.QueueName, kueue.WorkloadDeactivationTarget, "workload is deactivated", r.clock.Now())
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
	...
	case !lqExists:
		...
		if workload.UnsetQuotaReservationWithCondition(&wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("LocalQueue %s doesn't exist", wl.Spec.QueueName), r.clock.Now()) {
	...
	case !lqActive:
		...
		if workload.UnsetQuotaReservationWithCondition(&wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("LocalQueue %s is inactive", wl.Spec.QueueName), r.clock.Now()) {
			err := workload.ApplyAdmissionStatus(ctx, r.client, &wl, true, r.clock)
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
	case !cqOk:
		log.V(3).Info("Workload is inadmissible because of missing ClusterQueue", "clusterQueue", klog.KRef("", string(cqName)))
		if workload.UnsetQuotaReservationWithCondition(&wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("ClusterQueue %s doesn't exist", cqName), r.clock.Now()) {
			err := workload.ApplyAdmissionStatus(ctx, r.client, &wl, true, r.clock)
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
	case !r.cache.ClusterQueueActive(cqName):
		...
		if workload.UnsetQuotaReservationWithCondition(&wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("ClusterQueue %s is inactive", cqName), r.clock.Now()) {
			err := workload.ApplyAdmissionStatus(ctx, r.client, &wl, true, r.clock)
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
	}
...
func (r *WorkloadReconciler) reconcileOnLocalQueueActiveState(ctx context.Context, wl *kueue.Workload, lqExists bool, lq *kueue.LocalQueue) (bool, error) {
	...
	if !lqExists || !lq.DeletionTimestamp.IsZero() {
		...
		_ = workload.UnsetQuotaReservationWithCondition(wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("LocalQueue %s is terminating or missing", wl.Spec.QueueName), r.clock.Now())
	...
	if queueStopPolicy != kueue.None {
		...
		_ = workload.UnsetQuotaReservationWithCondition(wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("LocalQueue %s is stopped", wl.Spec.QueueName), r.clock.Now())
		return true, workload.ApplyAdmissionStatus(ctx, r.client, wl, true, r.clock)
	}
...
func (r *WorkloadReconciler) reconcileOnClusterQueueActiveState(ctx context.Context, wl *kueue.Workload, cqName kueue.ClusterQueueReference) (bool, error) {
	...
	if !cqExists || !cq.DeletionTimestamp.IsZero() {
		...
		_ = workload.UnsetQuotaReservationWithCondition(wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("ClusterQueue %s is terminating or missing", cqName), r.clock.Now())
		return true, workload.ApplyAdmissionStatus(ctx, r.client, wl, true, r.clock)
	}

	if queueStopPolicy != kueue.None {
		log.V(3).Info("Workload is inadmissible because the ClusterQueue is stopped", "clusterQueue", klog.KRef("", string(cqName)))
		_ = workload.UnsetQuotaReservationWithCondition(wl, wl.Spec.QueueName, kueue.WorkloadInadmissible, fmt.Sprintf("ClusterQueue %s is stopped", cqName), r.clock.Now())
		return true, workload.ApplyAdmissionStatus(ctx, r.client, wl, true, r.clock)
	}

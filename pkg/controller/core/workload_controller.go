...
func (r *WorkloadReconciler) reconcileNotReadyTimeout(ctx context.Context, req ctrl.Request, wl *kueue.Workload) (time.Duration, error) {
	...
	detailedReason, recheckAfter := r.admittedNotReadyWorkload(wl)
	if detailedReason == "" {
		return 0, nil
	}
	if recheckAfter > 0 {
	...
	workload.ResetChecksOnEviction(wl, r.clock.Now())
	reportWorkloadEvictedOnce := workload.WorkloadEvictionStateInc(wl, kueue.WorkloadEvictedByPodsReadyTimeout, detailedReason)
	err := workload.ApplyAdmissionStatus(ctx, r.client, wl, true, r.clock)
	if err == nil {
		cqName, _ := r.queues.ClusterQueueForWorkload(wl)
		workload.ReportEvictedWorkload(r.recorder, wl, cqName, kueue.WorkloadEvictedByPodsReadyTimeout, message)
		if reportWorkloadEvictedOnce {
			metrics.ReportEvictedWorkloadsOnce(cqName, kueue.WorkloadEvictedByPodsReadyTimeout, detailedReason)
		}
	}
	return 0, client.IgnoreNotFound(err)
}
...
//  1. A detailedReason that complements informarion carried by kueue.WorkloadEvictedByPodsReadyTimeout
//
// (e.g., StartupTimeoutReason, RecoveryTimeoutReason, or empty if not applicable).
//
...
// it returns an empty detailedReason and zero duration.
func (r *WorkloadReconciler) admittedNotReadyWorkload(wl *kueue.Workload) (string, time.Duration) {
	...
	if podsReadyCond == nil || podsReadyCond.Reason == kueue.WorkloadWaitForStart || podsReadyCond.Reason == kueue.WorkloadPodsReady {
		...
		return kueue.StartupTimeoutReason, max(r.waitForPodsReady.timeout-elapsedTime, 0)
		...
		return kueue.RecoveryTimeoutReason, max(*r.waitForPodsReady.recoveryTimeout-elapsedTime, 0)
	}
	return "", 0
}

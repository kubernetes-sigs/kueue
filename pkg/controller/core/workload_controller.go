...
func (r *WorkloadReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if workload.IsActive(&wl) {
		...
		if updated {
			return ctrl.Result{}, client.IgnoreNotFound(workload.PatchAdmissionStatus(ctx, r.client, wlOrig, true, r.clock, func() (*kueue.Workload, bool, error) {
				return &wl, true, nil
			}))
		}
	} else {
		...
		if updated {
			if evicted {
				...
				if err := workload.PatchAdmissionStatus(ctx, r.client, wlOrig, true, r.clock, func() (*kueue.Workload, bool, error) {
					return &wl, true, nil
				}); err != nil {
					if !apierrors.IsNotFound(err) {
	...
	case !lqExists:
		...
		if err := workload.PatchAdmissionStatus(ctx, r.client, &wl, true, r.clock, func() (*kueue.Workload, bool, error) {
			updated := workload.UnsetQuotaReservationWithCondition(&wl, kueue.WorkloadInadmissible, fmt.Sprintf("LocalQueue %s doesn't exist", wl.Spec.QueueName), r.clock.Now())
	...
	case !lqActive:
		...
		if err := workload.PatchAdmissionStatus(ctx, r.client, &wl, true, r.clock, func() (*kueue.Workload, bool, error) {
			updated := workload.UnsetQuotaReservationWithCondition(&wl, kueue.WorkloadInadmissible, fmt.Sprintf("LocalQueue %s is inactive", wl.Spec.QueueName), r.clock.Now())
			return &wl, updated, nil
		}); err != nil {
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
	case !cqOk:
		...
		if err := workload.PatchAdmissionStatus(ctx, r.client, &wl, true, r.clock, func() (*kueue.Workload, bool, error) {
			updated := workload.UnsetQuotaReservationWithCondition(&wl, kueue.WorkloadInadmissible, fmt.Sprintf("ClusterQueue %s doesn't exist", cqName), r.clock.Now())
			return &wl, updated, nil
		}); err != nil {
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
	case !r.cache.ClusterQueueActive(cqName):
		...
		if err := workload.PatchAdmissionStatus(ctx, r.client, &wl, true, r.clock, func() (*kueue.Workload, bool, error) {
			updated := workload.UnsetQuotaReservationWithCondition(&wl, kueue.WorkloadInadmissible, fmt.Sprintf("ClusterQueue %s is inactive", cqName), r.clock.Now())
			return &wl, updated, nil
		}); err != nil {
			return ctrl.Result{}, client.IgnoreNotFound(err)
		}
...
func (r *WorkloadReconciler) reconcileNotReadyTimeout(ctx context.Context, req ctrl.Request, wl *kueue.Workload) (time.Duration, error) {
	...
	err := workload.Evict(ctx, r.client, r.recorder, wlOrig, kueue.WorkloadEvictedByPodsReadyTimeout, underlyingCause, message, r.clock, workload.WithCustomPrepare(func() (*kueue.Workload, error) {
		return wl, nil
	}))
	return 0, err
}

...
func TestMultiKueueAdapter(t *testing.T) {
	...
	cases := map[string]struct {
		managersTrainJobs []kftrainerv2api.TrainJob
		workerTrainJobs   []kftrainerv2api.TrainJob

		operation func(ctx context.Context, adapter *multiKueueAdapter, managerClient, workerClient client.Client) error

		wantError             error
		wantManagersTrainJobs []kftrainerv2api.TrainJob
		wantWorkerTrainJobs   []kftrainerv2api.TrainJob
	}{
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			managerBuilder := utiltesting.NewClientBuilder(kftrainerv2api.AddToScheme).WithInterceptorFuncs(interceptor.Funcs{SubResourcePatch: utiltesting.TreatSSAAsStrategicMerge})
			managerBuilder = managerBuilder.WithLists(&kftrainerv2api.TrainJobList{Items: tc.managersTrainJobs})
			managerBuilder = managerBuilder.WithStatusSubresource(slices.Map(tc.managersTrainJobs, func(w *kftrainerv2api.TrainJob) client.Object { return w })...)
			managerClient := managerBuilder.Build()

			workerBuilder := utiltesting.NewClientBuilder(kftrainerv2api.AddToScheme).WithInterceptorFuncs(interceptor.Funcs{SubResourcePatch: utiltesting.TreatSSAAsStrategicMerge})
			workerBuilder = workerBuilder.WithLists(&kftrainerv2api.TrainJobList{Items: tc.workerTrainJobs})
			...
			gotManagersTrainJobs := &kftrainerv2api.TrainJobList{}
			...
			gotWorkerTrainJobs := &kftrainerv2api.TrainJobList{}

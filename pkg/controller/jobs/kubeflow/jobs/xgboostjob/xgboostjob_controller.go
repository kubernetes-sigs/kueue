...
func (h *parentWorkloadHandler) queueReconcileJobsWaitingForPrebuiltWorkload(ctx context.Context, object client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	log.V(5).Info("Queueing reconcile for prebuilt workload waiting xgboostjobs")

	var waitingJobs kftraining.XGBoostJobList
	if err := h.client.List(ctx, &waitingJobs, client.InNamespace(w.Namespace), client.MatchingFields{constants.PrebuiltWorkloadIndexName: w.Name}); err != nil {
		log.Error(err, "Unable to list waiting xgboostjobs")
		return
	}

	for _, waitingJob := range waitingJobs.Items {
		log.V(5).Info("Queueing reconcile for waiting xgboostjob", "xgboostjob", klog.KObj(&waitingJob))
...
func SetupIndexes(ctx context.Context, indexer client.FieldIndexer) error {
	if err := jobframework.SetupPrebuiltWorkloadIndex(ctx, indexer, &kftraining.XGBoostJob{}); err != nil {
		return err
	}

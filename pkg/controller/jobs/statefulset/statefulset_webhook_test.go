...
func TestValidateUpdate(t *testing.T) {
	baseStatefulSet := testingstatefulset.MakeStatefulSet(testStatefulSetName, testNamespaceName)
	baseStatefulSetWithQueue := baseStatefulSet.Clone().Queue(testQueueName)
	...
	testCases := map[string]struct {
		...
		"no changes": {
			oldObj: baseStatefulSetWithQueue.DeepCopy(),
			newObj: baseStatefulSetWithQueue.DeepCopy(),
		},
		"change in queue label": {
			oldObj: baseStatefulSetWithQueue.DeepCopy(),
			newObj: baseStatefulSetWithQueue.Clone().Queue(fmt.Sprintf("%s-new", testQueueName)).Obj(),
		},
		"change in queue label (ready replicas)": {
			oldObj: baseStatefulSetWithQueue.Clone().ReadyReplicas(1).Obj(),
			newObj: baseStatefulSetWithQueue.Clone().Queue(fmt.Sprintf("%s-new", testQueueName)).Obj(),
			wantErr: field.ErrorList{
				&field.Error{
					Type:  field.ErrorTypeInvalid,
		...
		"change in queue label (delete queue label)": {
			oldObj: baseStatefulSetWithQueue.Clone().ReadyReplicas(1).Obj(),
			newObj: baseStatefulSet.Clone().Obj(),
			wantErr: field.ErrorList{
				&field.Error{
					Type:  field.ErrorTypeInvalid,
					Field: queueNameLabelPath.String(),

...
func (r *Reconciler) syncQueueLabel(ctx context.Context, sts *appsv1.StatefulSet, pods []corev1.Pod) error {
	...
	return parallelize.Until(ctx, len(pods), func(i int) error {
		pod := &pods[i]
		if pod.Labels[controllerconstants.QueueLabel] == string(queueName) {
			return nil
		}
		return client.IgnoreNotFound(clientutil.Patch(ctx, r.client, pod, func() (bool, error) {
			if pod.Labels[controllerconstants.QueueLabel] != string(queueName) {
...
func NewReconciler(_ context.Context, client client.Client, _ client.FieldIndexer, recorder record.EventRecorder, opts ...jobframework.Option) (jobframework.JobReconcilerInterface, error) {

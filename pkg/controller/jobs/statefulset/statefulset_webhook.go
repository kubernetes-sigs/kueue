	...
	if suspend {
		...
		if oldReplicas == 0 && newReplicas > 0 {
			if newStatefulSet.Status.Replicas > 0 {
				...
				// Block if workload is still being deleted (exists without RequeueHeld).
				// If the workload has the RequeueHeld condition, it is intentionally
				// kept alive during scale-to-zero and will be re-admitted on scale-up.
				...
				if client.IgnoreNotFound(err) != nil {
					return nil, err
				} else if err == nil && !workload.IsRequeueHeld(&wl) {

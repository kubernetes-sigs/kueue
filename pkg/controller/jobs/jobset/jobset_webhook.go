...
func (w *JobSetWebhook) validateTopologyRequest(jobSet *JobSet) field.ErrorList {
	...
	for i := range jobSet.Spec.ReplicatedJobs {
		...
		allErrs = append(allErrs, jobframework.ValidateTASPodSetRequest(replicaMetaPath, &jobSet.Spec.ReplicatedJobs[i].Template.Spec.Template.ObjectMeta, podsCount(&jobSet.Spec.ReplicatedJobs[i]))...)
	}
	return allErrs
}

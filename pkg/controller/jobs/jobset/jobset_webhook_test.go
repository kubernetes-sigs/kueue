...
func TestValidateCreate(t *testing.T) {
	testcases := []struct {
		...
		},
		{
			...
			job: testingutil.MakeJobSet("jobset", "default").
				ReplicatedJobs(
					...
					testingutil.ReplicatedJobRequirements{
						Name: "job4", Replicas: 2, Parallelism: 1, Completions: 1, PodAnnotations: map[string]string{
							kueue.PodSetRequiredTopologyAnnotation: "cloud.com/block",
							kueue.PodSetGroupName:                  "3podsets",
						},
					},
					testingutil.ReplicatedJobRequirements{
						Name: "job4", Replicas: 4, Parallelism: 1, Completions: 1, PodAnnotations: map[string]string{
							kueue.PodSetRequiredTopologyAnnotation: "cloud.com/block",
						},
					},
				).
			...
			wantErr: field.ErrorList{
				...
				field.Invalid(field.NewPath("spec", "replicatedJobs[1]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-group-name"), "3podsets", "can only define groups of exactly 2 pod sets, got: 3 pod set(s)"),
				field.Invalid(field.NewPath("spec", "replicatedJobs[2]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-group-name"), "3podsets", "can only define groups of exactly 2 pod sets, got: 3 pod set(s)"),
				field.Invalid(field.NewPath("spec", "replicatedJobs[3]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-group-name"), "3podsets", "can only define groups of exactly 2 pod sets, got: 3 pod set(s)"),
			}.ToAggregate(),
			topologyAwareScheduling: true,
		},
		...
		},
		{
			...
			wantErr: field.ErrorList{
				field.Invalid(field.NewPath("spec", "replicatedJobs[0]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-required-topology"), "cloud.com/rack", "has to match 'spec.replicatedJobs[1].template.metadata.annotations[kueue.x-k8s.io/podset-required-topology]'"),
				field.Invalid(field.NewPath("spec", "replicatedJobs[1]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-required-topology"), "cloud.com/block", "has to match 'spec.replicatedJobs[0].template.metadata.annotations[kueue.x-k8s.io/podset-required-topology]'"),
			}.ToAggregate(),
			topologyAwareScheduling: true,
		},
		{
			...
			wantErr: field.ErrorList{
				field.Invalid(field.NewPath("spec", "replicatedJobs[0]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-preferred-topology"), "cloud.com/rack", "has to match 'spec.replicatedJobs[1].template.metadata.annotations[kueue.x-k8s.io/podset-preferred-topology]'"),
				field.Invalid(field.NewPath("spec", "replicatedJobs[1]", "template", "metadata", "annotations").
					Key("kueue.x-k8s.io/podset-preferred-topology"), "cloud.com/block", "has to match 'spec.replicatedJobs[0].template.metadata.annotations[kueue.x-k8s.io/podset-preferred-topology]'"),
			}.ToAggregate(),
			topologyAwareScheduling: true,
		},
	}

	...
	testcases := []struct {
		...
		},
		{
			...
			wantErr: field.ErrorList{field.Invalid(field.NewPath("spec.replicatedJobs[1].template.metadata.annotations"),
				field.OmitValueType{}, `both "kueue.x-k8s.io/podset-required-topology" and "kueue.x-k8s.io/podset-preferred-topology" specified for the replica`)}.ToAggregate(),
		},
	}
	...
	testcases := []struct {
		...
		},
		{
			...
			wantErr: field.ErrorList{field.Invalid(field.NewPath("spec.replicatedJobs[0].template.metadata.annotations"),
				field.OmitValueType{}, `both "kueue.x-k8s.io/podset-required-topology" and "kueue.x-k8s.io/podset-preferred-topology" specified for the replica`)},
		},
	}

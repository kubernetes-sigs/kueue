...
func TestValidateCreate(t *testing.T) {
	testCases := map[string]struct {
		...
		"invalid PodSet group name request - does not match required pattern": {
			lws: testingleaderworkerset.MakeLeaderWorkerSet("test-lws", "").
				Queue("test-queue").
				LeaderTemplate(corev1.PodTemplateSpec{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{
							kueuealpha.PodSetGroupName: "1leadername",
						},
					},
				}).
				WorkerTemplate(corev1.PodTemplateSpec{
					ObjectMeta: metav1.ObjectMeta{
						Annotations: map[string]string{
							kueuealpha.PodSetGroupName: "1workername",
						},
					},
				}).
				Obj(),
			wantErr: field.ErrorList{
				field.Invalid(field.NewPath("spec.leaderWorkerTemplate.leaderTemplate.metadata.annotations").
					Key("kueue.x-k8s.io/podset-group-name"), "1leadername", "must start with a lowercase letter and contain only lowercase letters and numbers"),
				field.Invalid(field.NewPath("spec.leaderWorkerTemplate.workerTemplate.metadata.annotations").
					Key("kueue.x-k8s.io/podset-group-name"), "1workername", "must start with a lowercase letter and contain only lowercase letters and numbers"),
			}.ToAggregate(),
			topologyAwareScheduling: true,
		},
	}

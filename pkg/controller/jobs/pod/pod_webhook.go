...
var (
	...
	podGroupSetsAnnotationPath     = annotationsPath.Key(PodGroupSetsAnnotation)
...
func validatePodGroupMetadata(p *Pod) field.ErrorList {
	...
	gtcStr, gtcExists := p.pod.GetAnnotations()[GroupTotalCountAnnotation]
	...
	pgsStr, pgsExists := p.pod.GetAnnotations()[PodGroupSetsAnnotation]
	if !pgsExists {
		return allErrs
	}
	...
	if totalCount != int32(gtc) {
		return append(allErrs, field.Invalid(
			podGroupSetsAnnotationPath,
			pgsStr,
			fmt.Sprintf("sum of count mismatched to '%s' annotation", GroupTotalCountAnnotation),
		))
	}

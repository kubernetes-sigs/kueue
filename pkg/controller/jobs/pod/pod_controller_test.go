	...
	testCases := map[string]struct {
		...
		"scheduling gate is removed and node selector is added if workload is admitted": {
			...
			wantPods: []corev1.Pod{*basePodWrapper.
				...
				Obj()},
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("unit-test", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
		...
		"non-matching admitted workload is marekd for deletion and pod is finalized": {
			pods: []corev1.Pod{*basePodWrapper.
				Clone().
				ManagedByKueueLabel().
				KueueFinalizer().
		...
		"the workload is created when queue name is set": {
			...
			wantPods: []corev1.Pod{*basePodWrapper.
				...
				Obj()},
			wantWorkloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload(GetWorkloadNameForPod(basePodWrapper.GetName(), basePodWrapper.GetUID()), "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
					PodSets(
						*utiltestingapi.MakePodSet(kueue.DefaultPodSetName, 1).
							Request(corev1.ResourceCPU, "1").
		...
		"pod is stopped when workload is evicted": {
			...
			wantPods: []corev1.Pod{*basePodWrapper.
				...
				Obj()},
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("job", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
					PodSets(*utiltestingapi.MakePodSet(kueue.DefaultPodSetName, 1).Request(corev1.ResourceCPU, "1").Obj()).
					Queue("test-queue").
					ControllerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod", "test-uid").
		...
		"workload is composed and created for the pod group": {
			...
			wantWorkloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
					PodSets(
		...
		"all pods in group should be removed if workload is marked for deletion": {
			pods: []corev1.Pod{
				*basePodWrapper.
					Clone().
					ManagedByKueueLabel().
		...
		"replacement pod should be started for pod group of size 1": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
					...
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod", "test-uid").
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod2", "test-uid").
					ReserveQuota(utiltestingapi.MakeAdmission("cq").PodSets(utiltestingapi.MakePodSetAssignment(kueue.NewPodSetReference(podUID)).Obj()).Obj()).
					Admitted(true).
					Condition(metav1.Condition{
						Type:    WorkloadWaitingForReplacementPods,
						Status:  metav1.ConditionTrue,
		...
		"replacement pod should be started for set of Running, Failed, Succeeded pods": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
					...
					Queue("user-queue").
					ReserveQuota(utiltestingapi.MakeAdmission("cq").PodSets(utiltestingapi.MakePodSetAssignment(kueue.NewPodSetReference(podUID)).Count(3).Obj()).Obj()).
					Admitted(true).
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod", "test-uid").
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod2", "test-uid").
		...
		"deleted pods in group should not be finalized if the workload doesn't match": {
			...
			workloads: []kueue.Workload{ // We overrride the safe-delete finalizer to simulate the wrokload getting fully deleted.
				*utiltestingapi.MakeWorkload("test-group", "ns").
		...
		"deleted unschedulable pods are finalized": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
					...
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod1", "test-uid").
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod2", "test-uid").
					ReserveQuota(utiltestingapi.MakeAdmission("cq").PodSets(utiltestingapi.MakePodSetAssignment(kueue.NewPodSetReference(podUID)).Count(2).Obj()).Obj()).
					Admitted(true).
					Obj(),
			},
			wantPods: []corev1.Pod{
...
func TestReconciler_ErrorFinalizingPod(t *testing.T) {
	...
	wl := *utiltestingapi.MakeWorkload("unit-test", "ns").WithFinalizers(kueue.ResourceInUseFinalizerName).
		PodSets(*utiltestingapi.MakePodSet(kueue.DefaultPodSetName, 1).Request(corev1.ResourceCPU, "1").Obj()).
		ReserveQuota(utiltestingapi.MakeAdmission("cq").PodSets(utiltestingapi.MakePodSetAssignment(kueue.DefaultPodSetName).Obj()).Obj()).
		Admitted(true).
		Obj()

		...
		"scheduling gate is removed and node selector is added if workload is admitted": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("unit-test", "ns").Finalizers(kueue.ResourceInUseFinalizerName).
					...
					ReserveQuotaAt(
						utiltestingapi.MakeAdmission(kueue.ClusterQueueReference("cq")).
		...
		"non-matching admitted workload is deleted and pod is finalized": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("unit-test", "ns").Finalizers(kueue.ResourceInUseFinalizerName).
					...
					ReserveQuotaAt(utiltestingapi.MakeAdmission(kueue.ClusterQueueReference("cq")).PodSets(utiltestingapi.MakePodSetAssignment(kueue.DefaultPodSetName).Obj()).Obj(), now).
					ControllerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod", "test-uid").
					AdmittedAt(true, now).
					Obj(),
			},
		...
		"replacement pod should be started for pod group of size 1": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").Finalizers(kueue.ResourceInUseFinalizerName).
					...
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod", "test-uid").
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod2", "test-uid").
					ReserveQuotaAt(utiltestingapi.MakeAdmission(kueue.ClusterQueueReference("cq")).PodSets(utiltestingapi.MakePodSetAssignment(kueue.NewPodSetReference(podUID)).Obj()).Obj(), now).
					AdmittedAt(true, now).
					Condition(metav1.Condition{
						Type:    WorkloadWaitingForReplacementPods,
						Status:  metav1.ConditionTrue,
		...
		"replacement pod should be started for set of Running, Failed, Succeeded pods": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").Finalizers(kueue.ResourceInUseFinalizerName).
					...
					Queue("user-queue").
					ReserveQuotaAt(utiltestingapi.MakeAdmission(kueue.ClusterQueueReference("cq")).PodSets(utiltestingapi.MakePodSetAssignment(kueue.NewPodSetReference(podUID)).Count(3).Obj()).Obj(), now).
					AdmittedAt(true, now).
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod", "test-uid").
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod2", "test-uid").
		...
		"deleted unschedulable pods are finalized": {
			...
			workloads: []kueue.Workload{
				*utiltestingapi.MakeWorkload("test-group", "ns").Finalizers(kueue.ResourceInUseFinalizerName).
					...
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod1", "test-uid").
					OwnerReference(corev1.SchemeGroupVersion.WithKind("Pod"), "pod2", "test-uid").
					ReserveQuotaAt(utiltestingapi.MakeAdmission(kueue.ClusterQueueReference("cq")).PodSets(utiltestingapi.MakePodSetAssignment(kueue.NewPodSetReference(podUID)).Count(2).Obj()).Obj(), now).
					AdmittedAt(true, now).
					Obj(),
			},
			wantPods: []corev1.Pod{

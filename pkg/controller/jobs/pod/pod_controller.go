...
const (
	...
	PodGroupSetsAnnotation = "kueue.x-k8s.io/pod-group-sets"
	PodGroupRoleLabel      = "kueue.x-k8s.io/pod-group-role"
)
...
// Returns an empty string if there's no such label.
func podGroupRole(p corev1.Pod) string {
	role := p.GetLabels()[PodGroupRoleLabel]
	if role == "" {
		role = kueue.DefaultPodSetName
	}
	return role
}
...
func (p *Pod) podGroupSets() ([]kueue.PodSet, error) {
	...
	podGroupSetsAnnotionRaw, ok := p.pod.GetAnnotations()[PodGroupSetsAnnotation]
	if !ok {
		return nil, fmt.Errorf("failed to extract '%s' annotation", PodGroupSetsAnnotation)
	}
	...
	if err := decoder.Decode(&podSets); err != nil {
		return nil, fmt.Errorf("failed to parse '%s' annotation: %w", PodGroupSetsAnnotation, err)
	}
	for i := range podSets {
...
func (p *Pod) Load(ctx context.Context, c client.Client, key *types.NamespacedName) (removeFinalizers bool, err error) {
	...
	for i, pod := range p.list.Items {
		if _, usePodGroupSet := pod.GetAnnotations()[PodGroupSetsAnnotation]; usePodGroupSet {
			targetPodIndex = i
			break
		}
	}
...
func (p *Pod) constructGroupPodSets() ([]kueue.PodSet, error) {
	...
	_, usePodGroupSets := p.pod.GetAnnotations()[PodGroupSetsAnnotation]
...
func constructGroupPodSetsFromPodGroupSets(p *Pod, groupTotalCount int) ([]kueue.PodSet, error) {
	...
	for i := range resultPodSets {
		...
		if err != nil {
			return nil, fmt.Errorf(
				...
				resultPodSets[i].Name, PodGroupSetsAnnotation,
				err,
			)
		}
		resultPodSets[i].Name = roleHash
	}
	...
	if totalCountInPodGroupSets != int32(groupTotalCount) {
		return nil, fmt.Errorf(
			...
			PodGroupSetsAnnotation, GroupTotalCountAnnotation,
		)
	}

...
func (w *RayJobWebhook) validateTopologyRequest(rayJob *rayv1.RayJob) field.ErrorList {
	...
	allErrs = append(allErrs, jobframework.ValidateTASPodSetRequestForJob((*RayJob)(rayJob), headGroupMetaPath, &rayJob.Spec.RayClusterSpec.HeadGroupSpec.Template.ObjectMeta)...)
	for i := range rayJob.Spec.RayClusterSpec.WorkerGroupSpecs {
		...
		allErrs = append(allErrs, jobframework.ValidateTASPodSetRequestForJob((*RayJob)(rayJob), workerGroupMetaPath, &rayJob.Spec.RayClusterSpec.WorkerGroupSpecs[i].Template.ObjectMeta)...)
	}
	return allErrs
}

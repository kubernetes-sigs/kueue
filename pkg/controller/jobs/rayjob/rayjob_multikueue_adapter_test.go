...
func TestApplyChildWorkerStateClampsToDeclaredBounds(t *testing.T) {
	// The manager RayJob declares a single worker group with MinReplicas=1,
	// MaxReplicas=5 (the wrapper default). Counts are reported by the child
	// RayCluster on the worker cluster, across a trust boundary.
	const group = "workers-group-0"
	groupRef := kueue.NewPodSetReference(group)
	...
	testCases := map[string]struct {
		...
		"count within bounds is reflected": {
			counts:         map[kueue.PodSetReference]int32{groupRef: 3},
			wantAnnotation: `[{"name":"workers-group-0","count":3}]`,
			wantChanged:    true,
		},
		"count above MaxReplicas is dropped": {
			counts:         map[kueue.PodSetReference]int32{groupRef: 99},
			wantAnnotation: `[]`,
			wantChanged:    true,
		},
		"count below MinReplicas is dropped": {
			counts:         map[kueue.PodSetReference]int32{groupRef: 0},
			wantAnnotation: `[]`,
			wantChanged:    true,
		},
		"count for an undeclared group is dropped": {
			counts:         map[kueue.PodSetReference]int32{kueue.NewPodSetReference("ghost-group"): 2},
			wantAnnotation: `[]`,
			wantChanged:    true,
		},
	}
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			localJob := utiltestingrayjob.MakeJob("rayjob", TestNamespace).Obj()

			changed := applyChildWorkerState(localJob, tc.counts, "rev-1")

			if changed != tc.wantChanged {
				t.Errorf("applyChildWorkerState() changed = %v, want %v", changed, tc.wantChanged)
			}
			gotAnnotation := localJob.GetAnnotations()[raycluster.MultiKueueRuntimePodSetReplicaSizesAnnotation]
			if gotAnnotation != tc.wantAnnotation {
				t.Errorf("runtime replica-sizes annotation = %q, want %q", gotAnnotation, tc.wantAnnotation)
			}
		})
	}
}

...
func TestValidateCreate(t *testing.T) {
	...
	bigWorkerGroup := []rayv1.WorkerGroupSpec{worker, worker, worker, worker, worker, worker, worker, worker, worker, worker, worker, worker, worker, worker, worker, worker}
	...
	testcases := map[string]struct {
		...
		"invalid managed - too many worker groups": {
			...
			job: testingrayutil.MakeJob("job", "ns").Queue("queue").
				...
				Obj(),
			wantErr: field.ErrorList{
				field.TooMany(field.NewPath("spec", "rayClusterSpec", "workerGroupSpecs"), 16, 15),
			}.ToAggregate(),
		},

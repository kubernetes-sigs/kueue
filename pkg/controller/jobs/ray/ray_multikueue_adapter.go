...
type FetchResult struct {
	// Counts holds the effective per-worker-group pod counts keyed by PodSet
	// reference.
...
type RuntimeReplicaSync[PtrT any] struct {
	// Fetch reads the runtime worker state via the remote client. A nil result
	// means the runtime object does not exist yet (or its state is not the
	// autoscaler's, e.g. while suspended) and the reflection is skipped.

...
var (
	...
	// +metricsdoc:labels=result="possible values are `success` or `inadmissible`",replica_role="replica role (leader or follower)"
...
- 'inadmissible' means that no workload was admitted.`,
	...
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",replica_role="replica role (leader or follower)"
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",status="status label (varies by metric)",replica_role="replica role (leader or follower)"
...
- "inadmissible" means there was a failed admission attempt for these workloads and they won't be retried until cluster conditions, which could make this workload admissible, change`,
	...
	// +metricsdoc:labels=name="the name of the LocalQueue",namespace="the namespace of the LocalQueue",status="status label (varies by metric)",replica_role="replica role (leader or follower)"
...
- "inadmissible" means there was a failed admission attempt for these workloads and they won't be retried until cluster conditions, which could make this workload admissible, change`,
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",priority_class="the priority class name",replica_role="replica role (leader or follower)"
	...
	// +metricsdoc:group=localqueue
	// +metricsdoc:labels=name="the name of the LocalQueue",namespace="the namespace of the LocalQueue",priority_class="the priority class name",replica_role="replica role (leader or follower)"
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",reason="eviction or preemption reason",underlying_cause="root cause for eviction",replica_role="replica role (leader or follower)"
...
- "RequeuingLimitExceeded" means that the workload was evicted by Kueue due to requeuing limit exceeded.`,
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",reason="eviction or preemption reason",underlying_cause="root cause for eviction",priority_class="the priority class name",replica_role="replica role (leader or follower)"
...
- "RequeuingLimitExceeded" means that the workload was evicted by Kueue due to requeuing limit exceeded.`,
	...
	// +metricsdoc:group=localqueue
	// +metricsdoc:labels=name="the name of the LocalQueue",namespace="the namespace of the LocalQueue",reason="eviction or preemption reason",underlying_cause="root cause for eviction",priority_class="the priority class name",replica_role="replica role (leader or follower)"
...
- "RequeuingLimitExceeded" means that the workload was evicted by Kueue due to requeuing limit exceeded.`,
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",reason="eviction or preemption reason",detailed_reason="finer-grained eviction cause",priority_class="the priority class name",replica_role="replica role (leader or follower)"
...
- "RequeuingLimitExceeded" means that the workload was evicted by Kueue due to requeuing limit exceeded.`,
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=preempting_cluster_queue="the ClusterQueue executing preemption",reason="eviction or preemption reason",replica_role="replica role (leader or follower)"
...
- "InCohortReclaimWhileBorrowing" means that the workload was preempted by a workload in the same cohort due to reclamation of nominal quota while borrowing.`,
	...
	// +metricsdoc:group=localqueue
	// +metricsdoc:labels=name="the name of the LocalQueue",namespace="the namespace of the LocalQueue",replica_role="replica role (leader or follower)"
	...
	// +metricsdoc:group=clusterqueue
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",status="one of `pending`, `active`, or `terminated`",replica_role="replica role (leader or follower)"
...
For a ClusterQueue, the metric only reports a value of 1 for one of the statuses.`,
	...
	// +metricsdoc:group=localqueue
	// +metricsdoc:labels=name="the name of the LocalQueue",namespace="the namespace of the LocalQueue",active="one of `True`, `False`, or `Unknown`",replica_role="replica role (leader or follower)"
...
For a LocalQueue, the metric only reports a value of 1 for one of the statuses.`,
	...
	// +metricsdoc:labels=cohort="the name of the Cohort",cluster_queue="the name of the ClusterQueue",flavor="the resource flavor name",resource="the resource name",replica_role="replica role (leader or follower)"
	...
	// +metricsdoc:group=localqueue
	// +metricsdoc:labels=name="the name of the LocalQueue",namespace="the namespace of the LocalQueue",flavor="the resource flavor name",resource="the resource name",replica_role="replica role (leader or follower)"
	...
	// +metricsdoc:group=optional_clusterqueue_resources
	// +metricsdoc:labels=cluster_queue="the name of the ClusterQueue",cohort="the name of the Cohort",replica_role="replica role (leader or follower)"
...
If the ClusterQueue has a weight of zero and is borrowing, this will return NaN.`,
	...
	// +metricsdoc:labels=cohort="the name of the Cohort",replica_role="replica role (leader or follower)"
...
func ReportAdmissionChecksWaitTime(cqName kueue.ClusterQueueReference, priorityClass string, waitTime time.Duration, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	AdmissionChecksWaitTime.WithLabelValues(string(cqName), priorityClass, role).Observe(waitTime.Seconds())
}
...
func ReportLocalQueueAdmissionChecksWaitTime(lq LocalQueueReference, priorityClass string, waitTime time.Duration, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	LocalQueueAdmissionChecksWaitTime.WithLabelValues(string(lq.Name), lq.Namespace, priorityClass, role).Observe(waitTime.Seconds())
}
...
func ReadyWaitTime(cqName kueue.ClusterQueueReference, priorityClass string, waitTime time.Duration, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	QueuedUntilReadyWaitTime.WithLabelValues(string(cqName), priorityClass, role).Observe(waitTime.Seconds())
}
...
func LocalQueueReadyWaitTime(lq LocalQueueReference, priorityClass string, waitTime time.Duration, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	LocalQueueQueuedUntilReadyWaitTime.WithLabelValues(string(lq.Name), lq.Namespace, priorityClass, role).Observe(waitTime.Seconds())
}
...
func ReportAdmittedUntilReadyWaitTime(cqName kueue.ClusterQueueReference, priorityClass string, waitTime time.Duration, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	AdmittedUntilReadyWaitTime.WithLabelValues(string(cqName), priorityClass, role).Observe(waitTime.Seconds())
}
...
func ReportLocalQueueAdmittedUntilReadyWaitTime(lq LocalQueueReference, priorityClass string, waitTime time.Duration, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	LocalQueueAdmittedUntilReadyWaitTime.WithLabelValues(string(lq.Name), lq.Namespace, priorityClass, role).Observe(waitTime.Seconds())
}
...
func ReportEvictedWorkloads(cqName kueue.ClusterQueueReference, evictionReason, underlyingCause, priorityClass string, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	EvictedWorkloadsTotal.WithLabelValues(string(cqName), evictionReason, underlyingCause, priorityClass, role).Inc()
}

func ReportReplacedWorkloadSlices(cqName kueue.ClusterQueueReference, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	ReplacedWorkloadSlicesTotal.WithLabelValues(string(cqName), role).Inc()
}
...
func ReportLocalQueueEvictedWorkloads(lq LocalQueueReference, reason, underlyingCause string, priorityClass string, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	LocalQueueEvictedWorkloadsTotal.WithLabelValues(string(lq.Name), lq.Namespace, reason, underlyingCause, priorityClass, role).Inc()
}
...
func ReportEvictedWorkloadsOnce(cqName kueue.ClusterQueueReference, reason, underlyingCause, priorityClass string, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	EvictedWorkloadsOnceTotal.WithLabelValues(string(cqName), reason, underlyingCause, priorityClass, role).Inc()
}
...
func ReportPreemption(preemptingCqName kueue.ClusterQueueReference, preemptingReason string, targetCqName kueue.ClusterQueueReference, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	PreemptedWorkloadsTotal.WithLabelValues(string(preemptingCqName), preemptingReason, role).Inc()
}
...
func ClearClusterQueueMetrics(cqName string) {
	// Use DeletePartialMatch to delete metrics for all replica roles
...
func ReportClusterQueueStatus(cqName kueue.ClusterQueueReference, cqStatus ClusterQueueStatus, tracker *roletracker.RoleTracker) {
	...
	for _, status := range CQStatuses {
		...
		ClusterQueueByStatus.WithLabelValues(string(cqName), string(status), role).Set(v)
	}
}
...
func ReportLocalQueueStatus(lq LocalQueueReference, conditionStatus metav1.ConditionStatus, tracker *roletracker.RoleTracker) {
	...
	for _, status := range ConditionStatusValues {
		...
		LocalQueueByStatus.WithLabelValues(string(lq.Name), lq.Namespace, string(status), role).Set(v)
	}
}
...
func ReportClusterQueueResourceReservations(cohort kueue.CohortReference, queue, flavor, resource string, usage float64, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	ClusterQueueResourceReservations.WithLabelValues(string(cohort), queue, flavor, resource, role).Set(usage)
}
...
func ReportLocalQueueResourceReservations(lq LocalQueueReference, flavor, resource string, usage float64, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	LocalQueueResourceReservations.WithLabelValues(string(lq.Name), lq.Namespace, flavor, resource, role).Set(usage)
}
...
func ReportClusterQueueResourceUsage(cohort kueue.CohortReference, queue, flavor, resource string, usage float64, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	ClusterQueueResourceUsage.WithLabelValues(string(cohort), queue, flavor, resource, role).Set(usage)
}
...
func ReportLocalQueueResourceUsage(lq LocalQueueReference, flavor, resource string, usage float64, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	LocalQueueResourceUsage.WithLabelValues(string(lq.Name), lq.Namespace, flavor, resource, role).Set(usage)
}

func ReportClusterQueueWeightedShare(cq, cohort string, weightedShare float64, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	ClusterQueueWeightedShare.WithLabelValues(cq, cohort, role).Set(weightedShare)
}

func ReportCohortWeightedShare(cohort string, weightedShare float64, tracker *roletracker.RoleTracker) {
	role := roletracker.GetMetricsRole(tracker)
	CohortWeightedShare.WithLabelValues(cohort, role).Set(weightedShare)
}

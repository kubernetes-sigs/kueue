...
func ReportEvictedWorkloads(cqName kueue.ClusterQueueReference, evictionReason string, durationToPreemption *time.Duration) {
	...
	if durationToPreemption != nil {
		WorkloadsWithPodsReadyToEvictedTimeSeconds.WithLabelValues(string(cqName), evictionReason).Observe(durationToPreemption.Seconds())
	}
}
...
func ClearClusterQueueMetrics(cqName string) {
	...
	WorkloadsWithPodsReadyToEvictedTimeSeconds.DeleteLabelValues(cqName)
...
func Register() {
	metrics.Registry.MustRegister(
		...
		WorkloadsWithPodsReadyToEvictedTimeSeconds,

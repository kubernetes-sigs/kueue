...
func (j *JAXJobWrapper) JAXReplicaSpecs(replicaSpecs ...JAXReplicaSpecRequirement) *JAXJobWrapper {
	...
	for _, rs := range replicaSpecs {
		...
		if rs.Annotations != nil {
			j.Spec.JAXReplicaSpecs[rs.ReplicaType].Template.ObjectMeta.Annotations = rs.Annotations
		}
	}
...
func (j *JAXJobWrapper) Parallelism(p int32) *JAXJobWrapper {
	j.Spec.JAXReplicaSpecs[kftraining.JAXJobReplicaTypeWorker].Replicas = ptr.To(p)
	return j
}

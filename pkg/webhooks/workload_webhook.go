...
func ValidateWorkload(obj, oldObj *kueue.Workload) field.ErrorList {
	...
	resizeElastic := workload.IsResizeElastic(obj)
	...
	for i := range obj.Spec.PodSets {
		...
		if resizeElastic && ps.TopologyRequest != nil {
			allErrs = append(allErrs, field.Invalid(
				specPath.Child("podSets").Index(i).Child("topologyRequest"),
				ps.TopologyRequest,
				"topology-aware scheduling is not supported for an elastic (resize) workload",
			))
		}
	}
...
func ValidateWorkloadUpdate(newObj, oldObj *kueue.Workload) field.ErrorList {
	...
	resizeElastic := workload.IsResizeElastic(oldObj) && workload.IsResizeElastic(newObj)

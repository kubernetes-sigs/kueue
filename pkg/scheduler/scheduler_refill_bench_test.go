...
func (h *drainHarness) queueArrival(tb testing.TB, sc *drainScenario) {
	...
	if err := h.queues.AddOrUpdateWorkload(h.ctx, h.log, wl); err != nil {
...
func setupDrain(tb testing.TB, sc *drainScenario, budget int, log logr.Logger) (*drainHarness, *drainRun) {
	...
	cl := utiltesting.NewClientBuilder().
		...
		WithInterceptorFuncs(interceptor.Funcs{
			SubResourceApply: func(ctx context.Context, client client.Client, subResourceName string, applyConf apiruntime.ApplyConfiguration, opts ...client.SubResourceApplyOption) error {
	...
	for i := range sc.occupants {
		if !cqCache.AddOrUpdateWorkload(ctx, log, sc.occupants[i].DeepCopy()) {

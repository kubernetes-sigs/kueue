...
func TestScheduleForTASPreemption(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"workload gets scheduled after preempting a lower-priority TAS workload": {
			nodes:           defaultSingleNode,
			topologies:      []kueuealpha.Topology{defaultSingleLevelTopology},
			resourceFlavors: []kueue.ResourceFlavor{defaultTASFlavor},
			...
			wantPreempted: sets.New("default/bar-admitted"),
			wantLeft: map[string][]string{
				"tas-main": {"default/foo"},
			},
			wantEvents: []utiltesting.EventRecord{
				{
					Key:       types.NamespacedName{Namespace: "default", Name: "bar-admitted"},
					EventType: "Normal",
					Reason:    "Preempted",

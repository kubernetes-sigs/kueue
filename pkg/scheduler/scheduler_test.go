	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			clientBuilder := utiltesting.NewClientBuilder().
				...
				WithObjects(append(
					[]client.Object{
						utiltesting.MakeNamespace("eng-alpha").Label("dep", "eng").Obj(),
						utiltesting.MakeNamespace("eng-beta").Label("dep", "eng").Obj(),
						utiltesting.MakeNamespace("eng-gamma").Label("dep", "eng").Obj(),
						utiltesting.MakeNamespace("sales").Label("dep", "sales").Obj(),
						utiltesting.MakeNamespace("lend").Label("dep", "lend").Obj(),
					}, tc.objects...,
				)...)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			clientBuilder := utiltesting.NewClientBuilder().
				...
				WithObjects(
					utiltesting.MakeDefaultNamespace("default"),
				)
			cl := clientBuilder.Build()
...
func TestRequeueAndUpdate(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			updates := 0
			objs := []client.Object{w1, q1, utiltesting.MakeDefaultNamespace("ns1")}

		...
		for _, enabled := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s WorkloadRequestUseMergePatch enabled: %t", name, enabled), func(t *testing.T) {
				...
				for cqName, want := range tc.wantSkippedPreemptions {
					val, err := testutil.GetGaugeMetricValue(metrics.AdmissionCyclePreemptionSkips.WithLabelValues(cqName, roletracker.GetMetricsRole(roletracker.NewFakeRoleTracker(roletracker.RoleStandalone))))
					if err != nil {

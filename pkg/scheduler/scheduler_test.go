...
func TestSchedule(t *testing.T) {
	...
	cases := map[string]scheduleTestCase{
		...
		"ignore preemption gates when MultiKueueOrchestratedPreemption is disabled": {
			...
			wantWorkloads: []kueue.Workload{
				...
				*utiltestingapi.MakeWorkload("preemptor", "eng-beta").
					...
					Condition(metav1.Condition{
						...
						Status:             metav1.ConditionFalse,
						Reason:             kueue.WorkloadQuotaReservedReasonWaitingForPreemptedWorkloads,
						Message:            "couldn't assign flavors to pod set main: insufficient unused quota for example.com/gpu in flavor model-a, 20 more needed. Pending the preemption of 1 workload(s)",
						LastTransitionTime: metav1.NewTime(now),
					}).
					Condition(metav1.Condition{
						Type:               kueue.WorkloadAdmitted,

...
func (s *Scheduler) schedule(ctx context.Context) wait.SpeedSignal {
	...
	iterator := makeIterator(entries, s.workloadOrdering, s.fairSharing.Enable)
...
func makeIterator(entries []entry, workloadOrdering workload.Ordering, enableFairSharing bool) entryIterator {
	if enableFairSharing {
		return makeFairSharingIterator(entries, workloadOrdering)
	}

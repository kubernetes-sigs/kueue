...
func (s *Scheduler) reserveCapacityForUnreclaimablePreempt(log logr.Logger, e *entry, snapshot *schdcache.Snapshot, cq *schdcache.ClusterQueueSnapshot) {
	...
	if !preemption.CanAlwaysReclaim(cq) || (features.Enabled(features.PrioritizePreemptorWorkloads) && e.IsPreemptor) {
		snapshot.AddUsage(cq.Name, resourcesToReserve(log, e, cq))
	}
}
...
func (s *Scheduler) getInitialAssignments(
...
) (flavorassigner.Assignment, []*preemption.Target, error) {
	cq := simCtx.ClusterQueue(wl.ClusterQueue)
	...
	fullAssignment, terminalError := flvAssigner.Assign(ctx, nil)
	...
	if arm == flavorassigner.Preempt {
		...
		faPreemptionTargets, terminalError = s.preemptor.GetTargets(ctx, simCtx, *wl, fullAssignment)
		if terminalError != nil {
			return flavorassigner.Assignment{}, nil, terminalError
		}

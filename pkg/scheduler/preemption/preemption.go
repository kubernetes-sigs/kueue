...
type preemptionCtx struct {
	...
	tasRequests       schedulercache.WorkloadTASRequests
...
func (p *Preemptor) GetTargets(log logr.Logger, wl workload.Info, assignment flavorassigner.Assignment, snapshot *schedulercache.Snapshot) []*Target {
...
func restoreSnapshot(snapshot *schedulercache.Snapshot, targets []*Target) {
...
func (p *Preemptor) findCandidates(wl *kueue.Workload, cq *schedulercache.ClusterQueueSnapshot, frsNeedPreemption sets.Set[resources.FlavorResource]) []*workload.Info {
...
func cqIsBorrowing(cq *schedulercache.ClusterQueueSnapshot, frsNeedPreemption sets.Set[resources.FlavorResource]) bool {
	if !cq.HasParent() {
		return false
	}

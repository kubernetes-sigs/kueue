...
func TestCandidatesOrderingPartialPreemption(t *testing.T) {
	...
	partialWl := func(name string, prio int32, reserved time.Time) *workload.Info {
		wl := utiltesting.MakeWorkload(name, "ns").
			...
			Obj()
		return workload.NewInfo(logr.Discard(), wl)
	}
	...
	t.Run("gate on: workload with usage above reclaim target sorts first", func(t *testing.T) {
		features.SetFeatureGateDuringTest(t, features.PartialPreemption, true)
		a := partialWl("a-partial", 0, now.Add(time.Minute))
		b := partialWl("b-partial", 0, now)
		b.Obj.Status.Admission.PodSetAssignments[0].ReclaimTargetCount = ptr.To[int32](3)

		if got := CandidatesOrdering(log, false, a, b, preemptorCQ, now); got <= 0 {
			t.Errorf("CandidatesOrdering(noTarget, inFlightTarget) = %d, want > 0", got)
		}
		if got := CandidatesOrdering(log, false, b, a, preemptorCQ, now); got >= 0 {
			t.Errorf("CandidatesOrdering(inFlightTarget, noTarget) = %d, want < 0", got)
		}
	})

	t.Run("gate on: satisfied reclaim target does not sort first", func(t *testing.T) {
		features.SetFeatureGateDuringTest(t, features.PartialPreemption, true)
		a := partialWl("a-partial", 0, now.Add(time.Minute))
		b := partialWl("b-partial", 0, now)
		b.Obj.Status.Admission.PodSetAssignments[0].ReclaimTargetCount = ptr.To[int32](5)

		if got := CandidatesOrdering(log, false, a, b, preemptorCQ, now); got >= 0 {
			t.Errorf("CandidatesOrdering(noTarget, satisfiedTarget) = %d, want < 0 (recency decides)", got)
		}
		if got := CandidatesOrdering(log, false, b, a, preemptorCQ, now); got <= 0 {
			t.Errorf("CandidatesOrdering(satisfiedTarget, noTarget) = %d, want > 0 (recency decides)", got)
		}
	})

	t.Run("gate on: recency decides when partial candidates have equal effective usage", func(t *testing.T) {
		features.SetFeatureGateDuringTest(t, features.PartialPreemption, true)
		a := partialWl("a-partial", 0, now)
		b := partialWl("b-partial", 0, now.Add(time.Minute))
		a.Obj.Status.Admission.PodSetAssignments[0].ReclaimTargetCount = ptr.To[int32](3)
		b.Obj.Status.Admission.PodSetAssignments[0].ReclaimTargetCount = ptr.To[int32](3)

		if got := CandidatesOrdering(log, false, a, b, preemptorCQ, now); got <= 0 {
			t.Errorf("CandidatesOrdering(older, newer) = %d, want > 0", got)
		}
		if got := CandidatesOrdering(log, false, b, a, preemptorCQ, now); got >= 0 {
			t.Errorf("CandidatesOrdering(newer, older) = %d, want < 0", got)
		}
	})

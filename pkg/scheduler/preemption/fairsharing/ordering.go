...
type TargetClusterQueueOrdering struct {
	preemptorCq *schedulercache.ClusterQueueSnapshot
	// ancestor Cohorts of the preemptor ClusterQueue.
	preemptorAncestors sets.Set[*schedulercache.CohortSnapshot]
	...
	prunedClusterQueues sets.Set[*schedulercache.ClusterQueueSnapshot]
	prunedCohorts       sets.Set[*schedulercache.CohortSnapshot]
}

func MakeClusterQueueOrdering(cq *schedulercache.ClusterQueueSnapshot, candidates []*workload.Info) TargetClusterQueueOrdering {
	t := TargetClusterQueueOrdering{
		preemptorCq:        cq,
		preemptorAncestors: sets.New[*schedulercache.CohortSnapshot](),

		clusterQueueToTarget: make(map[kueue.ClusterQueueReference][]*workload.Info),

		prunedClusterQueues: sets.New[*schedulercache.ClusterQueueSnapshot](),
		prunedCohorts:       sets.New[*schedulercache.CohortSnapshot](),
	}
...
func (t *TargetClusterQueueOrdering) onPathFromRootToPreemptorCQ(cohort *schedulercache.CohortSnapshot) bool {
	return t.preemptorAncestors.Has(cohort)
}

func (t *TargetClusterQueueOrdering) hasWorkload(cq *schedulercache.ClusterQueueSnapshot) bool {
...
func (t *TargetClusterQueueOrdering) nextTarget(cohort *schedulercache.CohortSnapshot) *TargetClusterQueue {
	var highestCq *schedulercache.ClusterQueueSnapshot = nil
	highestCqDrs := -1
	...
	var highestCohort *schedulercache.CohortSnapshot = nil
	highestCohortDrs := -1

...
type HierarchicalPreemptionCtx struct {
	Log               logr.Logger
	Wl                *kueue.Workload
	Cq                *schedulercache.ClusterQueueSnapshot
	FrsNeedPreemption sets.Set[resources.FlavorResource]
	Requests          resources.FlavorResourceQuantities
	WorkloadOrdering  workload.Ordering
}

func IsBorrowingWithinCohortForbidden(cq *schedulercache.ClusterQueueSnapshot) (bool, *int32) {
...
func getCandidatesFromCQ(cq *schedulercache.ClusterQueueSnapshot, lca *schedulercache.CohortSnapshot, ctx *HierarchicalPreemptionCtx, hasHiearchicalAdvantage bool) []*candidateElem {
...
func collectCandidatesForHierarchicalReclaim(ctx *HierarchicalPreemptionCtx) ([]*candidateElem, []*candidateElem) {
	...
	var previousSubtreeRoot *schedulercache.CohortSnapshot
	var candidateList *[]*candidateElem
	var fits bool
	hasHierarchicalAdvantage, remainingRequests := schedulercache.QuantitiesFitInQuota(ctx.Cq, ctx.Requests)
	for currentSubtreeRoot := range ctx.Cq.PathParentToRoot() {
		...
		fits, remainingRequests = schedulercache.QuantitiesFitInQuota(currentSubtreeRoot, remainingRequests)
...
func collectCandidatesInSubtree(ctx *HierarchicalPreemptionCtx, currentCohort *schedulercache.CohortSnapshot, subtreeRoot *schedulercache.CohortSnapshot, skipSubtree *schedulercache.CohortSnapshot, hasHierarchicalAdvantage bool, result *[]*candidateElem) {
	for _, childCohort := range currentCohort.ChildCohorts() {
		...
		if schedulercache.IsWithinNominalInResources(childCohort, ctx.FrsNeedPreemption) {
			continue
		}
	...
	for _, childCq := range currentCohort.ChildCQs() {
		...
		if !schedulercache.IsWithinNominalInResources(childCq, ctx.FrsNeedPreemption) {
...
func getNodeHeight(node *schedulercache.CohortSnapshot) int {
...
func FindHeightOfLowestSubtreeThatFits(c *schedulercache.ClusterQueueSnapshot, fr resources.FlavorResource, val int64) (int, bool) {
	if !c.BorrowingWith(fr, val) || !c.HasParent() {
		return 0, c.HasParent()
	}
	remaining := val - schedulercache.LocalAvailable(c, fr)
	for trackingNode := range c.PathParentToRoot() {
		...
		remaining -= schedulercache.LocalAvailable(trackingNode, fr)
	}
	// no fit found

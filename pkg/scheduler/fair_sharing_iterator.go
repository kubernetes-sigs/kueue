...
type fairSharingIterator struct {
	...
	cqToEntry     map[*scheduler.ClusterQueueSnapshot]*entry
...
func makeFairSharingIterator(ctx context.Context, entries []entry, workloadOrdering workload.Ordering) *fairSharingIterator {
	f := fairSharingIterator{
		cqToEntry: make(map[*scheduler.ClusterQueueSnapshot]*entry, len(entries)),
		entryComparer: entryComparer{
...
func (f *fairSharingIterator) getCq() *scheduler.ClusterQueueSnapshot {
...
func runTournament(cohort *scheduler.CohortSnapshot, ec entryComparer, cqToEntry map[*scheduler.ClusterQueueSnapshot]*entry) *entry {
...
func (e *entryComparer) computeDRS(rootCohort *scheduler.CohortSnapshot, cqToEntry map[*scheduler.ClusterQueueSnapshot]*entry) {

...
type fairSharingIterator struct {
	...
	cqToEntry     map[*schedulercache.ClusterQueueSnapshot]*entry
...
func makeFairSharingIterator(ctx context.Context, entries []entry, workloadOrdering workload.Ordering) *fairSharingIterator {
	f := fairSharingIterator{
		cqToEntry: make(map[*schedulercache.ClusterQueueSnapshot]*entry, len(entries)),
		entryComparer: entryComparer{
...
func (f *fairSharingIterator) getCq() *schedulercache.ClusterQueueSnapshot {
...
func runTournament(cohort *schedulercache.CohortSnapshot, ec entryComparer, cqToEntry map[*schedulercache.ClusterQueueSnapshot]*entry) *entry {
...
func (e *entryComparer) computeDRS(rootCohort *schedulercache.CohortSnapshot, cqToEntry map[*schedulercache.ClusterQueueSnapshot]*entry) {

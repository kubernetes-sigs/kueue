...
func TestScheduleForFairSharingRefill(t *testing.T) {
	...
	cases := map[string]scheduleTestCase{
		...
		// Control for the reservation case: the deferral above is caused by the
		// reservation, not by the fixture.
		...
		// Fit-only rule with genuine scarcity: fitonly-next has a real
		// lower-priority candidate. The victim keeps its admission, and
		// fitonly-next may preempt next cycle as its ClusterQueue's head.
...
func TestRefillReleasesWorkloadAlreadyAccountedInCache(t *testing.T) {
	...
	admittedStale := staleWrapper.Clone().
		...
		Obj()
	if !cqCache.AddOrUpdateWorkload(ctx, log, admittedStale) {
		t.Fatal("Failed to account the workload in the scheduler cache")
	}
	if !cqCache.IsAdded(*workload.NewInfo(log, admittedStale)) {
	...
	if err := qManager.AddOrUpdateWorkload(ctx, log, stale.DeepCopy()); err != nil {
...
// TestRefillFitOnlyDeferralIsPerCycle guards that the Fit-only rule's state is
// scoped to the entry, not to the Scheduler: a deferral signal outliving the
// cycle would suppress the next cycle's refill. Cycle one defers the refilled
// workload behind a mid-cycle reservation; cycle two -- where the reserving head
// is parked as inadmissible and the reservation is gone with its snapshot --
// must let both the deferred workload and a fresh refill pop through.
//
// Not expressible as a scheduleTestCase: the harness runs a single schedule().
func TestRefillFitOnlyDeferralIsPerCycle(t *testing.T) {
	...
	if !cqCache.AddOrUpdateWorkload(ctx, log, richActive.DeepCopy()) {
...
// TestRefillDeferralClearsFlavorScanProgress guards the deferral's FlavorScanState
// clearing against FlavorFungibilityPreserveScanProgress: with whenCanPreempt:
// MayStopSearch the refilled flv-next's first-cycle scan records f1, where it has
// a genuine preemption candidate. In the second cycle flv-next is the head and may
// preempt; only a cleared assignment rescans f1 and evicts that candidate, while
// preserved progress resumes at f2, where the equal-priority filler leaves none,
// and parks the workload.
//
// Not expressible as a scheduleTestCase: the harness runs a single schedule().
		...
		t.Run(name, func(t *testing.T) {
			...
			if tc.full {
				occupant := utiltestingapi.MakeWorkload("occupant", "default").Queue("stop-lq").
					...
					Obj()
				if !cqCache.AddOrUpdateWorkload(ctx, log, occupant) {
...
// touched.
//
// Not expressible as a scheduleTestCase: the harness leaves pods-ready tracking off.
...
// TestRefillPopKeepsMidCycleRequeueSignal covers the mid-cycle pop's effect on the
// epoch counter end to end: when a requeue event arrives after the cycle's heads
// were popped, a workload that cannot be admitted goes back to the active heap
// rather than being parked. The event is edge-triggered, so a refilled pop that
// hides it makes the workload wait for the next event, not the next cycle.
// TestPopMidCycleDoesNotConsumeRequeueSignal pins this on the queue primitive;
// nothing pinned the scheduler end.
//
// The namespaceSelector turns the refilled workload away because the Fit-only rule
// leaves no other way in: every refilled entry that got an assignment is requeued
// with a reason that never reads the counter, so only a failed nomination reaches
// it.
//
// Not expressible as a scheduleTestCase: one schedule() call, no mid-cycle event.

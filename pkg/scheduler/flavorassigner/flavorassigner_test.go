...
func flexAssertConsidered(t *testing.T, want, got Assignment) {
	...
	for name, wantPS := range wantByName {
		...
		if len(wantPS.Considered) == 0 {
			continue
		}

		assertPodSetConsideredFlexible(t, name, wantPS.Considered, gotPS.Considered)
	}
}

func assertPodSetConsideredFlexible(t *testing.T, podSetName string, want, got []FlavorAttempt) {
	wantByFlavor := make(map[kueue.ResourceFlavorReference]FlavorAttempt, len(want))
	for _, wa := range want {
		wantByFlavor[wa.Flavor] = wa
	}

	gotByFlavor := make(map[kueue.ResourceFlavorReference]FlavorAttempt, len(got))
	hasFit := false
	for _, ga := range got {
	...
	if hasFit {
		for flavor, ga := range gotByFlavor {
			...
			if !ok {
				t.Errorf("podset %q: unexpected flavor %q in Considered (fit case), got=%#v", podSetName, flavor, ga)
				continue
			}
	...
	for flavor, wa := range wantByFlavor {
		...
		if !ok {
			t.Errorf("podset %q: expected flavor %q in Considered (no-fit case)", podSetName, flavor)
			continue
		}
		if diff := cmp.Diff(wa, ga); diff != "" {
...
func isDoesNotProvideOnly(at FlavorAttempt) bool {
	if at.Mode != NoFit {
		return false
	}
	if len(at.Reasons) == 0 {
				...
				PodSets: []PodSetAssignment{
					{
						...
						Count:      1,
						Considered: []FlavorAttempt{{Flavor: "default", Mode: Fit}},
					},
				},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.wantAssignment, assignment,
				...
				cmpopts.IgnoreFields(PodSetAssignment{}, "Considered"),
			); diff != "" {

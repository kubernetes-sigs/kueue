...
func (f *testOracle) SimulatePreemption(log logr.Logger, cq *schedulercache.ClusterQueueSnapshot, wl workload.Info, fr resources.FlavorResource, quantity int64) (preemptioncommon.PreemptionPossibility, int) {
...
func TestAssignFlavors(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			cache := schedulercache.New(utiltesting.NewFakeClient())
...
func TestWorkloadsTopologyRequests_ErrorBranches(t *testing.T) {
	cases := map[string]struct {
		...
		"workload requires Topology, but there is no TAS cache information": {
			cq: schedulercache.ClusterQueueSnapshot{
				TASFlavors: map[kueue.ResourceFlavorReference]*schedulercache.TASFlavorSnapshot{},
			},
			assignment: Assignment{
				PodSets: []PodSetAssignment{{
					Name: kueue.DefaultPodSetName,
		...
		"workload requires Topology, but there is no TAS cache information for the assigned flavor": {
			cq: schedulercache.ClusterQueueSnapshot{
				TASFlavors: map[kueue.ResourceFlavorReference]*schedulercache.TASFlavorSnapshot{"tas": nil},
			},
			assignment: Assignment{
				PodSets: []PodSetAssignment{{
					Name: kueue.DefaultPodSetName,

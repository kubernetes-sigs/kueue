...
func (a *Assignment) UpdateForTASResult(result schedulercache.TASAssignmentsResult) {
...
type preemptionOracle interface {
	SimulatePreemption(log logr.Logger, cq *schedulercache.ClusterQueueSnapshot, wl workload.Info, fr resources.FlavorResource, quantity int64) (preemptioncommon.PreemptionPossibility, int)
}
...
func New(wl *workload.Info, cq *schedulercache.ClusterQueueSnapshot, resourceFlavors map[kueue.ResourceFlavorReference]*kueue.ResourceFlavor, enableFairSharing bool, oracle preemptionOracle, preemptWorkloadSlice *workload.Info) *FlavorAssigner {
...
func lastAssignmentOutdated(wl *workload.Info, cq *schedulercache.ClusterQueueSnapshot) bool {
...
func (a *FlavorAssigner) assignFlavors(log logr.Logger, counts []int32) Assignment {
	...
	if features.Enabled(features.TopologyAwareScheduling) {
		...
		if assignment.RepresentativeMode() == Fit {
			result := a.cq.FindTopologyAssignmentsForWorkload(tasRequests, schedulercache.WithWorkload(a.wl.Obj))
		...
		if assignment.RepresentativeMode() == Preempt && !workload.HasNodeToReplace(a.wl.Obj) {
			...
			result := a.cq.FindTopologyAssignmentsForWorkload(tasRequests, schedulercache.WithSimulateEmpty(true))
			if failure := result.Failure(); failure != nil {
...
func (a *FlavorAssigner) fitsResourceQuota(log logr.Logger, fr resources.FlavorResource, val int64, rQuota schedulercache.ResourceQuota) (preemptionMode, int, *Status) {
	var status Status

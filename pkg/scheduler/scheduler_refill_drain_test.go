...
func TestRefillDrainScenarios(t *testing.T) {
	for _, sc := range refillDrainScenarios() {
		t.Run(sc.name, func(t *testing.T) {
			...
			if sc.wantFullStops {
				if census[true].stops[string(refillStopNoFreeCapacity)] == 0 {
					t.Errorf("refill never ran into a full cohort, so the fixture does not exercise the capacity check")
				}
				if census[true].deferred != 0 {
					t.Errorf("%d refilled workloads were deferred; the capacity check should have stopped those pops", census[true].deferred)
				}
			}
			if sc.arrival == nil {
				return

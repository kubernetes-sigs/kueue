...
prepare-release-branch: yq kustomize ## Prepare the release branch with the release version.
	...
	$(SED) -r 's/--version="v?[0-9]+\.[0-9]+\.[0-9]+/--version="$(APP_VERSION)/g' -i charts/kueue/README.md -i cmd/kueueviz/INSTALL.md
	$(SED) -r 's/KUEUE_VERSION="[0-9]+\.[0-9]+\.[0-9]+/KUEUE_VERSION="$(APP_VERSION)/g' -i cmd/kueueviz/INSTALL.md

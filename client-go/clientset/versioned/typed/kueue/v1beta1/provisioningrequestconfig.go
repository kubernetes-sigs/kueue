...
func (c *provisioningRequestConfigs) List(ctx context.Context, opts v1.ListOptions) (*v1beta1.ProvisioningRequestConfigList, error) {
	if watchListOptions, hasWatchListOptionsPrepared, watchListOptionsErr := watchlist.PrepareWatchListOptionsFromListOptions(opts); watchListOptionsErr != nil {
		klog.Warningf("Failed preparing watchlist options for provisioningrequestconfigs, falling back to the standard LIST semantics, err = %v", watchListOptionsErr)
	} else if hasWatchListOptionsPrepared {
		result, err := c.watchList(ctx, watchListOptions)
		if err == nil {
			consistencydetector.CheckWatchListFromCacheDataConsistencyIfRequested(ctx, "watchlist request for provisioningrequestconfigs", c.list, opts, result)
			return result, nil
		}
		klog.Warningf("The watchlist request for provisioningrequestconfigs ended with an error, falling back to the standard LIST semantics, err = %v", err)
	}
	result, err := c.list(ctx, opts)
	if err == nil {
		consistencydetector.CheckListFromCacheDataConsistencyIfRequested(ctx, "list request for provisioningrequestconfigs", c.list, opts, result)
	}
	return result, err
}
...
// watchList establishes a watch stream with the server and returns the list of ProvisioningRequestConfigs
func (c *provisioningRequestConfigs) watchList(ctx context.Context, opts v1.ListOptions) (result *v1beta1.ProvisioningRequestConfigList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}

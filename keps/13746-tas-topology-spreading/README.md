...
`[{"key": "kueue.x-k8s.io/job-uid", "operator": "In", "values": ["<job-uid>"]}]`,
where `<job-uid>` is the value of the `kueue.x-k8s.io/job-uid` label on the
Workload. The `kueue.x-k8s.io/job-uid` label is set by the job integrations on
the Workloads they manage, so it requires no additional configuration. This
default is appropriate for integrations where all spreading-group members share
a common parent object (e.g., all groups created from one LWS object).
...
7. Workloads with a valid annotation are parsed and rules applied correctly.
8. Only admitted workloads whose labels match the effective selector are counted
   (workloads that do not match are ignored).
9. Only PodSet groups with a topology assignment at the rule's `topologyKey` level contribute
   to the count; groups without that topology level are skipped.
10. Only PodSets in the same namespace are counted (cross-namespace spreading is not
    supported).
11. Correct per-domain PodSet group count computation for:
...
12. Banned domain set correctly derived for `Required` rules.
13. Over-threshold domain set correctly derived for `Preferred` rules.
14. Multiple rules (different topology level keys) in a single annotation all applied.
15. Correct counts after a workload matching the label selector is admitted within
    the same scheduling cycle.
16. Correct counts after a workload matching the label selector is evicted within

...
For assignments within the per-slice domain limit, Kueue first creates one compact slice and retains it when the
serialized `TopologyAssignment` fits within a conservative size budget. Assignments above the per-slice domain limit
skip this attempt. This preserves the existing encoding for assignments that already fit and avoids additional prefix
analysis and slice metadata.

When the single-slice encoding does not fit and the lowest topology level is `kubernetes.io/hostname`, Kueue:
...
When the lowest topology level is not
`kubernetes.io/hostname`, no prefix is reusable, or no prefix selection fits the slice limit, Kueue forms slices
using only domain-count chunking before applying the normal compaction.

For example, within a larger assignment that failed the single-slice check, the consecutive domains

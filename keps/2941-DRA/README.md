...
- Support for alpha DRA features like DRADeviceTaints, DRAAdminAccess, DRAPrioritizedLists and
  DRAPartitionableDevices will not be included.
...
- Alpha DRA features like DRADeviceTaints, DRAAdminAccess, DRAPrioritizedLists, and
  DRAPartitionableDevices are not supported.
- **Extended Resources** requires DeviceClasses to have `spec.extendedResourceName` set.
  ...
  The DeviceClass must be included in Kueue's `deviceClassMappings` configuration.
...
4. For Extended Resources: if a DeviceClass is created or updated between Kueue admitting a
   ...
   - **Scheduling failure**: the scheduler cannot allocate devices. `waitForPodsReady` catches
     this — the pod stays Pending, times out, and the workload is evicted and re-queued.
   ...
   - **Quota drift**: the scheduler allocates from a different DeviceClass than Kueue charged
     ...
     If both DeviceClasses are in `deviceClassMappings` under the same logical resource name,
     quota accounting remains correct at the logical level, though not at the physical
     DeviceClass level. Drift only occurs when the DeviceClasses map to different logical
     resources or one is not in `deviceClassMappings`.
   To mitigate:
   - Kueue uses a controller-runtime field indexer on `DeviceClass` by `spec.extendedResourceName`,
     filtered through `deviceClassMappings`, to resolve DeviceClasses deterministically.
   - Per KEP-5004, admins should ensure one `extendedResourceName` maps to at most one
     DeviceClass. During transitions, both DeviceClasses should be in `deviceClassMappings`
     under the same logical resource name.
...
3. If no matching DeviceClass is found, the workload is marked inadmissible with an error
   describing the missing DeviceClass. The workload may be re-evaluated on a subsequent
   reconciliation cycle (e.g., triggered by other workload or queue events).
...
dependencies on non-staging k8s repos. The indexer returns all DeviceClasses matching a given
`extendedResourceName`; Kueue filters the results against `deviceClassMappings` to determine
which DeviceClass applies for quota accounting.

#### DeviceClass Lifecycle Scenarios

1. Two DeviceClasses with the same `extendedResourceName`: `deviceClassMappings` determines
   which DeviceClass Kueue uses for quota. Per KEP-5004, admins should ensure one
   `extendedResourceName` maps to at most one DeviceClass.

2. New DeviceClass created after Kueue admits a workload: a TOCTOU gap exists where Kueue
   charged quota against one DeviceClass while the scheduler picks the newer one. If both
   DeviceClasses are in `deviceClassMappings` under the same logical resource, quota is
   correct at the logical level. `waitForPodsReady` handles scheduling failures. See
   [Risks and Mitigations](#risks-and-mitigations) for the full breakdown.
...
For Alpha, if a DeviceClass does not exist when a workload is created, the workload is marked
inadmissible with an error. A subsequent reconciliation cycle (e.g., triggered by other
workload or queue events) re-evaluates the workload after the DeviceClass is created.
Event-driven re-reconciliation on DeviceClass creation will be evaluated as a Beta

...
- The mechanism that records a verdict is out of scope here. The condition and reason a refusal is
  written to, the fingerprint encoding, the builder signature that carries a verdict out of the
  build, and the queue invalidation that retires a stale entry belong to `KueueDRAIntegration` and
...
- A Workload carrying an ordinary request, `spec.overhead`, or a resource transformation output on a
  ...
  request shape, and it is lifted by the prerequisite work rather than by a change here.
...
needs is not a particular route but two properties of whichever one an implementation keeps. The
resources, the keys they replace and the provenance behind them arrive as one payload rather than a
map per concern, since they go out of step otherwise. And the payload reaches the checked build
required [below](#where-the-verdict-is-allowed-to-arrive) rather than being optional to it.

Where that payload type is declared is constrained rather than chosen: `pkg/dra` already imports
`pkg/workload`, so declaring it in `pkg/dra` and importing it back would close a cycle.
...
- a checked build that returns the verdict as a verdict, run before the queue manager is asked to
  hold anything, and unbypassable rather than one of two constructors a caller may pick;
...
- a commit that installs only while what it was built from is still current, and refuses rather than
...
came from. The merge into `resources.Requests` saturates the same way, since `MapRequests.Add` goes
through `SaturatingAdd`, so nothing wraps there and nothing carries the bound either. A
...
`Value()` through `SafeMilliValue` and `SafeValue`, which clamp out-of-range quantities onto the
sentinel rather than reporting them, so following it is not on its own enough: a quantity that does
...
total, which a transformation output can produce, is as much a rejection as one past `MaxInt64`.
...
names, and check the merged value of what remains. The builder is where both happen because it is
...
since the two move independently. A fingerprint over the totals, the envelope-touched names,
whether any `firstAvailable` was reached, and the template revisions behind it is enough, and it
does not all have to reach the scheduling hash; what it has to do is wake a Workload whose charge
is unchanged and whose shape is not.

The encoding that makes a fingerprint unambiguous rather than merely deterministic, and the
commit discipline that keeps an entry's fields from being assembled out of two revisions, are
`KueueDRAIntegration` mechanism and are designed with the rest of the outcome protocol.
The invariant an implementation has to be checkable against:

```text
For a Workload revision, exactly one of these is externally observable:

1. a valid queue entry whose object, requests, provenance and fingerprint all
   belong to that revision; or
...
- an entry carries what it was built from, and no irreversible effect is issued for it unless that is
  still current;
...
above does. The `Exactly` walker reads the raw fields today, so an unset mode falls through to
unsupported there while a subrequest with the same gap is charged one device. Objects that went
through the apiserver carry the defaults either way, so this is a test and consistency problem
rather than a live undercharge, but the two readings have to come from one pair of helpers,
`effectiveAllocationMode` and `effectiveCount`, used by both walkers, with the omitted forms
covered on each. Claim-level `constraints` and `config` do not change the device count, and `pkg/dra`
does not read them.
...
IgnoreUndeclared`, from [KEP-7513](../7513-quota-check-strategy/README.md), otherwise omits
undeclared dimensions from enforcement, which for an envelope would admit a DRA request with its
device quota dropped rather than counted. The initial Alpha refuses that Workload instead, so the
bound never rests on a resource outside the queue's accounting. The builder decides it, against the
target ClusterQueue's declared resources, and declaring the resource there lifts it without a
manager restart. Since every alternative of a request resolves to the same logical resource, the
refusal is of the request as a whole rather than of one alternative. That KEP is provisional, so the interaction is worth confirming with its authors before
this one graduates.
...
- the configuration around it, cleared by a request or template change or by a manager restart: an
  ...
  to change;
- what else the Workload carries on a resource an envelope is charged on, cleared by a request or a
  template change: any non-DRA contribution the merge reads on that resource, which is an ordinary
  request, a LimitRange default, `spec.overhead`, RuntimeClass overhead or a resource transformation
  output, and a source-backed `Exactly` request anywhere in the same Workload unless it carries
  `adminAccess`, which is classified first and charges nothing. These are compositions rather than
  request shapes. The initial Alpha refuses them rather than charging them through shared accounting
  whose defects are still open in the first prerequisite table, and each becomes supported as those
  close, without the envelope itself changing. Two things on that resource are not contributions of
  this kind: an `Exactly` charge, which is charged and checked against the same total, and a
...
- Repairable dependency or configuration: the request is well formed and something outside it is
  ...
  it before admission, or comparing the entry's fingerprint against the Workload's current revision
  on pop. Alpha ships the watch and states the bound, and the entry-level guarantee is an Alpha
  requirement listed with the criteria below; which of the three closes it is the parent path's
...
- Waiting for a dependency: the claim template the request names does not exist. Nothing about the
  ...
  compile verdict with no provenance recording which revision was compiled and no watch to notice.

  If class selectors are wanted on this path later, the cost is the whole set and not part of it: a
  DeviceClass UID or generation in the provenance, an index from a claim template to the
...
admitted usage or AFS either. A `firstAvailable` envelope is the exception: the initial Alpha refuses a Workload whose
envelope resource the queue does not declare rather than admitting it unaccounted, so an envelope never reaches AFS
undeclared. This means
...
The Alpha composition limit in [Scope](#scope) takes the rows below off that critical path. Each
is a shared accounting defect that can only reach an envelope through a contribution this Alpha
refuses, so it is tracked against the Beta that lifts the limit rather than gating the Alpha that
keeps it. Removing the limit without them is what would put them back.
...
- Prioritized list arithmetic at the unit boundary: a fractional non-CPU charge and a CPU charge
  below a milli are refused rather than rounded up, an ordinary CPU request and a charge on a
  logical resource named `cpu` sum in milli rather than either being read as absolute units, and
  the largest exactly representable total is charged where the first one past it is refused, with
  both taken from the representation rather than spelled as constants
- Prioritized list composition on a charged key: `spec.overhead`, a transformation output, or an
  ordinary request landing on the resource an envelope is charged on makes the Workload inadmissible
  in this Alpha rather than merging into the envelope. Driven with a positive operand and with the
...
- Prioritized list under a relaxed LimitRange: the requeue
  ...
  rebuilds the charge and the provenance that belongs to it, rather than carrying an earlier
  revision's total, which preserving now refuses to do without a matching revision
- Prioritized list template lifecycle: a Workload refused for a template that does not exist is
  ...
  Workloads built on it, and no admission or preemption afterwards uses the charge computed from the
  template that is gone; replacement of the entry is what is eventual. A template deleted and
...
- Prioritized list identity is semantic: reordering `spec.podSets` or a PodSet's `resourceClaims`
  without changing any of them produces the same charge and the same provenance, so nothing this
  gate computes reads the reorder as a new revision
- Prioritized list provenance carries the origin: an unrepresentable total whose operands all came
  from the PodSet, and the same total reached with a `resourceTransformations` contribution, are
  distinguishable in the provenance the pass produces, and the same for a negative operand. What a
  refusal is reported as is the parent path's
- Prioritized list reference multiplicity: a template referenced twice in one PodSet, the same
  template moved to a different PodSet, and two equal-total templates exchanged each produce
  different provenance, because it records one entry per reference and never deduplicates them
...
- Prioritized list lifecycle: a Workload that becomes unrepresentable on update has its earlier
  ...
  charge with the provenance that belongs to it survives a backoff requeue, an inflight requeue
  that sees a newer generation, and the gate being turned on and then off again
...
- Prioritized list under `quotaCheckStrategy: IgnoreUndeclared`: a Workload whose envelope resource
  the target ClusterQueue does not declare is refused rather than admitted with the device quota
  dropped, while an `Exactly` DRA request on an undeclared resource is still left out of enforcement
  under the same strategy. The Alpha restriction is on the envelope rather than on DRA as a whole,
  and a run that waives both would pass a build that never looked at the envelope
...
- a rejection carrying the class that says what clears it: a request rejection re-evaluated when
  ...
  capability rejection lifted once the manager runs with the changed configuration or once the
  ClusterQueue declares the resource; a dependency
  rejection lifted when the object it names appears; and a read failure retried rather than
...
`KueueDRAIntegration` provides them is its own, and this KEP does not choose a fingerprint
encoding, a condition schema, a constructor signature, a field manager, or an invalidation
primitive.
...
- a Workload revision that changes what is charged invalidates the earlier result rather than
  being acted on with it ([#14535](https://github.com/kubernetes-sigs/kueue/issues/14535))

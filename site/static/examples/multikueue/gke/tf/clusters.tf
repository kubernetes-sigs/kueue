...
resource "google_container_node_pool" "manager_nodepool" {
  name       = "${var.cluster_manager_name_prefix}-nodepool"
  location   = var.location_manager
  cluster    = google_container_cluster.manager_cluster.name
...
resource "null_resource" "update_kubeconfig" {
  ...
  depends_on = [google_container_cluster.worker_cluster, google_container_node_pool.worker_nodepool, google_container_node_pool.worker_default_nodepool] # Ensure clusters are created first
}
...
resource "null_resource" "update_manager_kubeconfig" {
  ...
  depends_on = [google_container_cluster.manager_cluster, google_container_node_pool.manager_nodepool, google_container_node_pool.manager_default_nodepool]
}

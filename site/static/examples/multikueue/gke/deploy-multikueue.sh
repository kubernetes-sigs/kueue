...
function patch_manager_deployment() {
    kubectl patch deployment kueue-controller-manager -n kueue-system --patch \
        '{"spec":{"template":{"spec":{"containers":[{"name":"manager","args":["--config=/controller_manager_config.yaml","--zap-log-level=8","--feature-gates=MultiKueueBatchJobWithManagedBy=true", "--feature-gates=TopologyAwareScheduling=true"]}]}}}}'
}

...
The intended audience for this page are [batch administrators](/v0.17/docs/tasks#batch-administrator).
...
- [Kueue is installed](/v0.17/docs/installation).

## Single ClusterQueue and single ResourceFlavor setup

In the following steps, you will create a queuing system with a single
ClusterQueue and a single [ResourceFlavor](/v0.17/docs/concepts/cluster_queue#resourceflavor-object)
...
### 1. Create a [ClusterQueue](/v0.17/docs/concepts/cluster_queue)
...
`cpu` and `memory`. Each resource type has a single [resource flavor](/v0.17/docs/concepts/cluster_queue#resourceflavor-object),
named `default` with a nominal quota.

The empty `namespaceSelector` allows any namespace to use these resources.

### 2. Create a [ResourceFlavor](/v0.17/docs/concepts/cluster_queue#resourceflavor-object)
...
### 3. Create [LocalQueues](/v0.17/docs/concepts/local_queue)

Users cannot directly send [workloads](/v0.17/docs/concepts/workload) to
...
You can define quotas for different [resource flavors](/v0.17/docs/concepts/cluster_queue#resourceflavor-object).
...
the same [cohort](/v0.17/docs/concepts/cluster_queue#cohort).
...
Note that the ClusterQueue `team-a-cq` also defines [borrowingLimit](/v0.17/docs/concepts/cluster_queue#borrowingLimit).
...
A ClusterQueue can borrow resources from the [cohort](/v0.17/docs/concepts/cluster_queue#cohort)
...
Follow the [installation instructions for using a custom configuration](/v0.17/docs/installation#install-a-custom-configured-released-version) 
...
For a practical example of using resource transformations with vGPU resources, see [Using HAMi](/v0.17/docs/tasks/run/using_hami).

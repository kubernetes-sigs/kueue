...
2. For Kueue v0.15 and earlier, learn how to [install Kueue with a custom manager configuration](/v0.17/docs/installation/#install-a-custom-configured-released-version)
...
3. Kueue will run webhooks for all created pods if the pod integration is enabled. The webhook namespaceSelector could be
   ...
   When you [install Kueue via Helm](/v0.17/docs/installation/#install-via-helm), the webhook namespace selector 
...
5. Check [Administer cluster quotas](/v0.17/docs/tasks/manage/administer_cluster_quotas) for details on the initial Kueue setup.
...
The target [local queue](/v0.17/docs/concepts/local_queue) should be specified in the `metadata.labels` section of the Pod configuration.
...
{{< include "v0.17/examples/pods-kueue/kueue-pod.yaml" "yaml" >}}
...
[preemption](/v0.17/docs/concepts/cluster_queue/#preemption).
...
JobSet, MPIJob, RayJob (see more [here](/v0.17/docs/tasks/#batch-user)).
{{% /alert %}}
...
{{< include "v0.17/examples/pods-kueue/kueue-pod-group.yaml" "yaml" >}}

...
The intended audience for this page are [batch users](/v0.17/docs/tasks#batch-user).
...
- The cluster has [quotas configured](/v0.17/docs/tasks/manage/administer_cluster_quotas).
...
The [ClusterQueue](/v0.17/docs/concepts/cluster_queue) defines the quotas for the
Queue.
...
You can also skip setting the "queue name" label if you use [LocalQueue defaulting](/v0.17/docs/tasks/manage/enforce_job_management/setup_default_local_queue).

You should also:

- Specify [resource requests or limits](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/) for each Job Pod. If you specify only limits, Kueue will treat the limit values as requests. See [how Kueue uses resource requests](/v0.17/docs/concepts/workload#resource-requests) for details.
...
{{< include "v0.17/examples/jobs/sample-cronjob.yaml" "yaml" >}}
...
Internally, Kueue will create a corresponding [Workload](/v0.17/docs/concepts/workload)
...
You can also [Monitoring Status of the Workload](/v0.17/docs/tasks/run/jobs#3-optional-monitor-the-status-of-the-workload).
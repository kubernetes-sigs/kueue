...
2. Check [Administer cluster quotas](/v0.17/docs/tasks/manage/administer_cluster_quotas) for details on the initial Kueue setup.
...
The target [local queue](/v0.17/docs/concepts/local_queue) should be specified in the `metadata.labels` section of the RayJob configuration.
...
{{< include "v0.17/examples/jobs/ray-job-code-sample.yaml" "yaml" >}}

The RayJob looks like the following:

{{< include "v0.17/examples/jobs/ray-job-sample.yaml" "yaml" >}}
...
1. Enable feature gate for [Elastic Workloads (Workload Slices)](/v0.17/docs/concepts/elastic_workload)
...
{{< include "v0.17/examples/jobs/ray-job-autoscaling-code-sample.yaml" "yaml" >}}

The RayJob looks like the following:

{{< include "v0.17/examples/jobs/ray-job-autoscaling-sample.yaml" "yaml" >}}

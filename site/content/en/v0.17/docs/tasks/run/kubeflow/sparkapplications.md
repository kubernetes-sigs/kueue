...
You can enable it by editing the `SparkApplicationIntegration` feature gate. Check the [Installation](/v0.17/docs/installation/#change-the-feature-gates-configuration) guide for details on feature gate configuration.
{{% /alert %}}
...
You can [modify kueue configurations from installed releases](/v0.17/docs/installation#install-a-custom-configured-released-version) to include SparkApplication as an allowed workload.
...
The target [local queue](/v0.17/docs/concepts/local_queue) should be specified in the `metadata.labels` section of the SparkApplication configuration.
...
{{< include "v0.17/examples/jobs/sample-sparkapplication.yaml" "yaml" >}}

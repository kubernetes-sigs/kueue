...
complete a task. A workload is the unit of [admission](/v0.17/docs/concepts#admission) in Kueue.
...
You can stop or resume a running workload by setting the [Active](/v0.17/docs/reference/kueue.v1beta1#kueue-x-k8s-io-v1beta1-WorkloadSpec) field. The active field determines if a workload can be admitted into a queue or continue running, if already admitted.
Changing `.spec.Active` from true to false will cause a running workload to be evicted and not be requeued.

## Queue name

To indicate in which [LocalQueue](/v0.17/docs/concepts/local_queue) you want your Workload to be
...
In addition to the usual resource naming restrictions, you cannot use the `pods` resource name in a Pod spec, as it is reserved for internal Kueue use. You can use the `pods` resource name in a [ClusterQueue](/v0.17/docs/concepts/cluster_queue#resources) to set quotas on the maximum number of pods.

## Priority

Workloads have a priority that influences the [order in which they are admitted by a ClusterQueue](/v0.17/docs/concepts/cluster_queue#queueing-strategy).
...
By using [`WorkloadPriority`](/v0.17/docs/concepts/workload_priority_class),
...
Please refer to the [All-or-nothing with ready Pods](/v0.17/docs/tasks/manage/setup_wait_for_pods_ready/) for more details.
...
- Learn about [workload priority class](/v0.17/docs/concepts/workload_priority_class).
- Learn how to [run jobs](/v0.17/docs/tasks/run/jobs)
- Read the [API reference](/v0.17/docs/reference/kueue.v1beta1/#kueue-x-k8s-io-v1beta1-Workload) for `Workload`

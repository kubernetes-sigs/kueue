...
A Workload can preempt one or more Workloads if it is admitted in a [ClusterQueue with preemption enabled](/v0.17/docs/concepts/cluster_queue/#preemption)
and any of the following events happen:
- The preemptee belongs to the same [ClusterQueue](/v0.17/docs/concepts/cluster_queue) as the preemptor and the preemptee has a lower priority.
- The preemptee belongs to the same [cohort](/v0.17/docs/concepts/cluster_queue#cohort) as the preemptor and the preemptee's ClusterQueue has a usage above
  the [nominal quota](/v0.17/docs/concepts/cluster_queue#resources) for at least one resource that the preemptee and preemptor require.

The configured settings for preemption in the [Kueue Configuration](/v0.17/docs/reference/kueue-config.v1beta1#FairSharing)
and in the [ClusterQueue](/v0.17/docs/concepts/cluster_queue#preemption) can limit whether a Workload can preempt others, in addition
...
To enable Fair Sharing, [use a Kueue Configuration](/v0.17/docs/installation#install-a-custom-configured-release-version) similar to the following:
...
the [`kueue_cluster_queue_weighted_share` metric](/v0.17/docs/reference/metrics#optional-metrics).

...
The Workload is admitted once all [AdmissionCheckStates](/v0.17/docs/concepts/admission_check/#admissioncheckstates) are in the `Ready` state.
...
- For temporary issues (e.g., cloud capacity shortages):
  ...
  - It triggers exponential backoff retries via [`retryStrategy`](/v0.17/docs/concepts/admission_check/provisioning_request/#retry-strategy) in [ProvisioningRequestConfig](/v0.17/docs/concepts/admission_check/provisioning_request/#provisioningrequestconfig).
...
## What's Next?
  ...
  You can read the [Concepts](/v0.17/docs/concepts) section to learn how [Admission Checks](/v0.17/docs/concepts/admission_check/) influence admission.

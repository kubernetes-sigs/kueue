...
A Workload can preempt one or more Workloads if it is admitted in a [ClusterQueue with preemption enabled](/v0.19/docs/concepts/cluster_queue/#preemption)
and any of the following events happen:
- The preemptee belongs to the same [ClusterQueue](/v0.19/docs/concepts/cluster_queue) as the preemptor and the preemptee has a lower priority.
...
To enable Fair Sharing, [use a Kueue Configuration](/v0.19/docs/installation#install-a-custom-configured-release-version) similar to the following:
...
the [`kueue_cluster_queue_weighted_share` metric](/v0.19/docs/reference/metrics#optional-metrics).

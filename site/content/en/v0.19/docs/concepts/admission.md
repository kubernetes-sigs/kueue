...
Admission may also be subject to the [admission gate mechanism](/v0.19/docs/reference/labels-and-annotations/#kueuex-k8sioadmission-gated-by) which is in beta and enabled by default.
...
2. **Admission Checks:** Await for [AdmissionChecks](/v0.19/docs/concepts/admission_check) configured in the ClusterQueue. Checks can be either built-in such as [MultiKueue](/v0.19/docs/concepts/multikueue/) or [ProvisioningRequest](/v0.19/docs/concepts/admission_check/provisioning_request/), or user-created plugins.

The Workload is admitted once all [AdmissionCheckStates](/v0.19/docs/concepts/admission_check/#admissioncheckstates) are in the `Ready` state.
...
- For temporary issues (e.g., cloud capacity shortages):
  ...
  - It triggers exponential backoff retries via [`retryStrategy`](/v0.19/docs/concepts/admission_check/provisioning_request/#retry-strategy) in [ProvisioningRequestConfig](/v0.19/docs/concepts/admission_check/provisioning_request/#provisioningrequestconfig).
...
## What's Next?
  ...
  You can read the [Concepts](/v0.19/docs/concepts) section to learn how [Admission Checks](/v0.19/docs/concepts/admission_check/) influence admission.

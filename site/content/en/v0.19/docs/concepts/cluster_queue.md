...
- The quotas for the [resource _flavors_](/v0.19/docs/concepts/resource_flavor) that the ClusterQueue manages,
  with usage limits and order of consumption.
- Fair Sharing rules across the multiple ClusterQueues in the cluster.

Only [batch administrators](/v0.19/docs/tasks#batch-administrator) should create `ClusterQueue` objects.
...
This ClusterQueue admits [Workloads](/v0.19/docs/concepts/workload) if and only if:
...
models). You can define how a flavor maps to a group of nodes using a [ResourceFlavor object](/v0.19/docs/concepts/resource_flavor).
...
In a process called [admission](/v0.19/docs/concepts#admission), Kueue assigns to the
[Workload pod sets](/v0.19/docs/concepts/workload#pod-sets) a flavor for each resource the pod set
requests.
...
When [Concurrent Admission](/v0.19/docs/concepts/concurrent_admission) is enabled for
...
The resource name `pods` is [reserved](/v0.19/docs/concepts/workload/#reserved-resource-names) and cannot be specified in
...
[admission](/v0.19/docs/concepts#admission) attempt.

The following are the supported queueing strategies:

- `StrictFIFO`: Workloads are ordered first by [priority](/v0.19/docs/concepts/workload#priority)
...
By default, if multiple workloads require `borrowing`, Kueue will try to schedule workloads with higher [priority](/v0.19/docs/concepts/workload#priority) first.
...
by setting a [`flavorFungibility`](/v0.19/docs/concepts/cluster_queue#flavorfungibility) in ClusterQueue.
...
Read [Preemption](/v0.19/docs/concepts/preemption) to learn more about
...
StopPolicy allows a cluster administrator to temporary stop the admission of workloads within a ClusterQueue by setting its value in the [spec](/v0.19/docs/reference/kueue.v1beta1/#kueue-x-k8s-io-v1beta1-ClusterQueueSpec) like:
...
For an example ClusterQueue configuration using admission checks, see [Admission Checks](/v0.19/docs/concepts/admission_check#usage).

## What's next?

- Create [local queues](/v0.19/docs/concepts/local_queue)
- Create [resource flavors](/v0.19/docs/concepts/resource_flavor) if you haven't already done so.
- Learn how to [administer cluster quotas](/v0.19/docs/tasks/manage/administer_cluster_quotas).
- Read the [API reference](/v0.19/docs/reference/kueue.v1beta1/#kueue-x-k8s-io-v1beta1-ClusterQueue) for `ClusterQueue`

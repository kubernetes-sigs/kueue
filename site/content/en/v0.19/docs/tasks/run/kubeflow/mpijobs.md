...
You can [modify kueue configurations from installed releases](/v0.19/docs/installation#install-a-custom-configured-released-version) to include MPIJobs as an allowed workload.
...
The target [local queue](/v0.19/docs/concepts/local_queue) should be specified in the `metadata.labels` section of the MPIJob configuration.
...
{{< include "v0.19/examples/jobs/sample-mpijob.yaml" "yaml" >}}

For equivalent instructions for doing this in Python, see [Run Python Jobs](/v0.19/docs/tasks/run/python_jobs/#mpi-operator-job).

...
We demonstrate how to support scheduling Tekton Pipelines Tasks in Kueue based on the [Plain Pod](/v0.19/docs/tasks/run/plain_pods) integration, where every Pod from a Pipeline is represented as a single independent Plain Pod.
...
1. Check [Administrator cluster quotas](/v0.19/docs/tasks/manage/administer_cluster_quotas/) for details on the initial Kueue step.
...
{{< include "v0.19/examples/pod-based-workloads/tekton-sleep-task.yaml" "yaml" >}}

{{< include "v0.19/examples/pod-based-workloads/tekton-hello-task.yaml" "yaml" >}}

A pipeline composes these tasks.

{{< include "v0.19/examples/pod-based-workloads/tekton-pipeline.yaml" "yaml" >}}

## a. Targeting a single LocalQueue

If you want every task to target a single [local queue](/v0.19/docs/concepts/local_queue),
it should be specified in the `metadata.label` section of the PipelineRun configuration.

{{< include "v0.19/examples/pod-based-workloads/tekton-pipeline-run.yaml" "yaml" >}}

...
2. The Pod belongs to a namespace that don't satisfy the requirements of
   the [`managedJobsNamespaceSelector`](/v0.19/docs/reference/kueue-config.v1beta1/#Configuration).
3. The Pod is owned by a Job or equivalent CRD that is managed by Kueue.
4. The Pod doesn't have a `kueue.x-k8s.io/queue-name` label and [`manageJobsWithoutQueueName`](/v0.19/docs/reference/kueue-config.v1beta1/#Configuration)
   is set to `false`.

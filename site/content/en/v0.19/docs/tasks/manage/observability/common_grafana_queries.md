...
- Kueue Prometheus metrics are enabled (see [Setup Prometheus](/v0.19/docs/tasks/manage/observability/setup_prometheus)).

## Quota utilization

{{% alert title="Note" color="primary" %}}
The queries in this section require `metrics.enableClusterQueueResources: true`
in the Kueue configuration. See [Installation](/v0.19/docs/installation/#install-a-custom-configured-released-version) for details.
{{% /alert %}}
...
See [Prometheus Metrics](/v0.19/docs/reference/metrics) for the full list of `reason` label values.
...
- See [Pending Workloads in Grafana](/v0.19/docs/tasks/manage/monitor_pending_workloads/pending_workloads_in_grafana) for visibility dashboards using the on-demand API.

...
Below we demonstrate how to support this with the [resource transformation for quota management](/v0.18/docs/tasks/manage/administer_cluster_quotas/#transform-resources-for-quota-management) in Kueue.
...
2. Follow the [installation instructions for using a custom configuration](/v0.18/docs/installation#install-a-custom-configured-released-version) to configure Kueue with ResourceTransformation.
...
The target [local queue](/v0.18/docs/concepts/local_queue) should be specified in the `metadata.labels` section of the Pod configuration.
...
{{< include "v0.18/examples/serving-workloads/sample-hami.yaml" "yaml" >}}

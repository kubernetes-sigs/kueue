...
This integration is based on the [Plain Pod Group](https://kueue.sigs.k8s.io/v0.18/docs/tasks/run/plain_pods/) integration.

This guide is for [serving users](/v0.18/docs/tasks#serving-user) that have a basic understanding of Kueue.
For more information, see [Kueue's overview](/v0.18/docs/overview).

## Before you begin

1. The `leaderworkerset.x-k8s.io/leaderworkerset` integration is enabled by default.

2. For Kueue v0.15 and earlier, learn how to [install Kueue with a custom manager configuration](/v0.18/docs/installation/#install-a-custom-configured-released-version)
   ...
   See [Run Plain Pods](/v0.18/docs/tasks/run/plain_pods/#before-you-begin) for configuration details.
   {{% /alert %}}

3. Check [Administer cluster quotas](/v0.18/docs/tasks/manage/administer_cluster_quotas) for details on the initial Kueue setup.
...
The target [local queue](/v0.18/docs/concepts/local_queue) should be specified in the `metadata.labels` section of the LeaderWorkerSet configuration.
...
{{< include "v0.18/examples/serving-workloads/sample-leaderworkerset.yaml" "yaml" >}}
...
- [Configure the cluster for Topology Aware Scheduling](/v0.18/docs/concepts/topology_aware_scheduling).
...
{{< include "v0.18/examples/serving-workloads/sample-leaderworkerset-tas.yaml" "yaml" >}}
...
## Multikueue
Check [MultiKueue](/v0.18/docs/tasks/run/multikueue/leaderworkerset) for details on running LeaderWorkerSets in MultiKueue environment.

## Troubleshooting

For general troubleshooting guidance, see the [Kueue troubleshooting guide](/v0.18/docs/tasks/troubleshooting).
...
Enable the alpha
[`WorkloadIdentifierAnnotations`](/v0.18/docs/getting-started/installation/#feature-gates-for-alpha-and-beta-features)

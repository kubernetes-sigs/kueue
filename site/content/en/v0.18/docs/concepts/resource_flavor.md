...
To guarantee that the Pods in the [Workload](/v0.18/docs/concepts/workload) run on the nodes associated to the flavor that Kueue selected, Kueue performs the following steps:

1. When admitting a Workload, Kueue evaluates the [`.nodeSelector`](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#nodeselector) and [`.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution`](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#node-affinity) fields in the PodSpecs of your [Workload](/v0.18/docs/concepts/workload) against the ResourceFlavor labels.
...
{{< include "v0.18/examples/admin/resource-flavor-tolerations.yaml" "yaml" >}}

When defining a ResourceFlavor as above, you should set the following values:
- The `.metadata.name` field, which is required to reference a ResourceFlavor from a [ClusterQueue](/v0.18/docs/concepts/cluster_queue) in the `.spec.resourceGroups[*].flavors[*].name` field.
...
For Kueue to [admit](/v0.18/docs/concepts#admission) a Workload to use the ResourceFlavor, the PodSpecs in the Workload should have a toleration for it.
On the other hand, when the ResourceFlavor has also set the matching tolerations in `.spec.tolerations`, 
then the taints are not considered during [admission](/v0.18/docs/concepts#admission).
...
{{< include "v0.18/examples/admin/resource-flavor-taints.yaml" "yaml" >}}
...
{{< include "v0.18/examples/admin/resource-flavor-empty.yaml" "yaml" >}}

## What's next?

- Learn about [cluster queues](/v0.18/docs/concepts/cluster_queue).
- Read the [API reference](/v0.18/docs/reference/kueue.v1beta1/#kueue-x-k8s-io-v1beta1-ResourceFlavor) for `ResourceFlavor`

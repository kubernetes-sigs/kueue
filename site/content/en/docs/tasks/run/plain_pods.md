...
Kueue then stops counting the marked Pod as an active group member. Only the
exact annotation value `"true"` has this effect; any other value leaves the Pod
active. The Workload's PodSet count, admission, and quota reservation are kept
unchanged, so the replacement Pod can take over the same group slot. Kueue
does not create the replacement Pod; the external controller remains
responsible for deleting the old Pod and creating the replacement.

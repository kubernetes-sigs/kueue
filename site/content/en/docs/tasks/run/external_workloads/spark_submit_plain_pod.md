...
[SparkApplication integration](/docs/tasks/run/kubeflow/sparkapplications/) instead.

When using `spark-submit` in cluster mode, Spark creates a driver Pod that in turn creates the
executor Pods. Kueue can manage these Pods through the [Plain Pod](/docs/tasks/run/plain_pods)
...
- the `kueue.x-k8s.io/queue-name` label, to select the target [local queue](/docs/concepts/local_queue);

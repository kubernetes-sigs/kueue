...
- **Assign different costs to different flavors**: Configure transformations that generate
  different amounts of credits based on the input resource. For example, spot instances
  could cost fewer credits than on-demand.

- **Track costs across resource types**: Generate credits from multiple input resources
  (CPU, memory, GPU) to enforce a combined budget.

- **Implement monetary budgets**: Use credits to approximate the cost of cloud resources
  and enforce spending limits per team.

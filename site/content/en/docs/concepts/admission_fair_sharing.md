...
You can disable it by editing the `AdmissionFairSharing` feature gate. Since Kueue v0.20 you have to disable `AdmissionFairSharingAnchorAtQuotaReservation` in the same configuration, because it depends on `AdmissionFairSharing` and is enabled by default; setting `AdmissionFairSharing` to false on its own is rejected at startup. Check the [Installation](/docs/installation/#change-the-feature-gates-configuration) guide for details on feature gate configuration.
{{% /alert %}}
...
other ClusterQueue accounts from admission. The reported `consumedResources` does not update
immediately, since it is a decaying average: it closes half of the remaining gap every
`usageHalfLifeTime`.

{{% alert title="Note" color="primary" %}}
`AdmissionFairSharingAnchorAtQuotaReservation` is currently a beta feature and is enabled by default.

You can disable it, which moves the anchor back to admission for every ClusterQueue, by editing the `AdmissionFairSharingAnchorAtQuotaReservation` feature gate. Check the [Installation](/docs/installation/#change-the-feature-gates-configuration) guide for details on feature gate configuration.
{{% /alert %}}

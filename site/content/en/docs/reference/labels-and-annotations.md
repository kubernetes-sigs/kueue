...
Example: `kueue.x-k8s.io/workload-group-index: "0"`

Used on: [Workload](/docs/concepts/workload/).

The annotation stores the numeric index for workloads created by multi-workload jobs
(e.g., LeaderWorkerSet creates one workload per replica). Used by MultiKueue to determine
primary workload ordering when dispatching workload group members to worker clusters atomically.

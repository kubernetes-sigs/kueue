...
对于使用 TLS 保护的指标端点，请参阅[使用 TLS 配置 Prometheus](/v0.19/docs/tasks/manage/productization/prometheus)。

本页面适用于[批处理管理员](/v0.19/docs/tasks#batch-administrator)。
...
参阅 [Prometheus 指标](/zh-cn/v0.19/docs/reference/metrics#optional-metrics)获取可选指标的完整列表。

## 接下来是什么

- 参阅[常用 Grafana 查询](/v0.19/docs/tasks/manage/observability/common_grafana_queries)
  获取用于在 Grafana 中监控 Kueue 的 PromQL 查询。
- 参阅[使用 TLS 配置 Prometheus](/v0.19/docs/tasks/manage/productization/prometheus)
  获取使用 cert-manager 进行高级 TLS 配置的信息。
- 参阅[设置开发监控](/v0.19/docs/tasks/dev/setup_dev_monitoring)获取带有

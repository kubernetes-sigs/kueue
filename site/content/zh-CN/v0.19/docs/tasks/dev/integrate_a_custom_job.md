...
- 通过将自定义 Job 实例包装在 AppWrapper 中来利用内置的 AppWrapper 集成。
  详情请参阅[运行包装的自定义工作负载](/v0.19/docs/tasks/run/external_workloads/wrapped_custom_workload)。
- 作为 Kueue 仓库的一部分构建新的集成。
- 作为外部控制器构建新的集成。

本指南面向[平台开发者](/v0.19/docs/tasks#platform-developer)，描述如何构建新的集成。
...
[controller_manager_config.yaml](https://kueue.sigs.k8s.io/v0.19/docs/installation/#install-a-custom-configured-released-version)

...
要启用指标抓取，请参阅[设置 Prometheus](/v0.19/docs/tasks/manage/observability/setup_prometheus)。
{{% /alert %}}

Kueue 暴露了 [prometheus](https://prometheus.io) 指标来监控系统的健康状况和
[ClusterQueues](/v0.19/docs/concepts/cluster_queue)
以及 [LocalQueues](/v0.19/docs/concepts/local_queue) 的状态。
...
详情请参阅[安装](/v0.19/docs/installation/)的[更改特性门控配置](/v0.19/docs/installation/#change-the-feature-gates-configuration)部分。
...
以下指标仅在[管理器配置](/v0.19/docs/installation/#install-a-custom-configured-released-version)中启用了
...
以下指标仅在[管理器配置](/v0.19/docs/installation/#install-a-custom-configured-released-version)中启用了 `waitForPodsReady` 时可用。
更多详情[见](/v0.19/docs/tasks/manage/setup_wait_for_pods_ready)。

...
为了保证 [Workload](/v0.19/docs/concepts/workload) 中的 Pod 运行在 Kueue 选定的 flavor 所关联的节点上，Kueue 会执行以下步骤：
...
{{< include "v0.19/examples/admin/resource-flavor-tolerations.yaml" "yaml" >}}

定义如上 ResourceFlavor 时，应设置以下值：
- `.metadata.name` 字段，用于在 [ClusterQueue](/v0.19/docs/concepts/cluster_queue) 的 `.spec.resourceGroups[*].flavors[*].name` 字段中引用 ResourceFlavor。
...
Kueue 要[接纳](/v0.19/docs/concepts#admission) Workload 使用 ResourceFlavor，Workload 的 PodSpec 必须包含相应的容忍度。
另一方面，如果 ResourceFlavor 的 `.spec.tolerations` 字段也设置了匹配的容忍度，
则在[接纳](/v0.19/docs/concepts#admission)期间不会考虑污点。
与 [ResourceFlavor 容忍度实现自动调度](#ResourceFlavor-容忍度实现自动调度)不同，
Kueue 不会为 flavor 污点自动添加容忍度。

此类型的 ResourceFlavor 示例如下：

{{< include "v0.19/examples/admin/resource-flavor-taints.yaml" "yaml" >}}
...
{{< include "v0.19/examples/admin/resource-flavor-empty.yaml" "yaml" >}}

## 下一步？

- 了解[集群队列（cluster queues）](/v0.19/docs/concepts/cluster_queue)。
- 阅读 `ResourceFlavor` 的 [API 参考](/zh-cn/v0.19/docs/reference/kueue.v1beta2/#kueue-x-k8s-io-v1beta2-ResourceFlavor)。

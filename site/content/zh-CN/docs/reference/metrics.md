...
| `kueue_admission_cycle_preemption_skips_total` | 计数器 | 在 ClusterQueue 中工作负载获得抢占候选但由于其他 ClusterQueues 在同一周期需要相同资源而被跳过的总次数。<br>标签 'reason' 可能的值如下:<br>- "OverlappingPreemptionTargets" 表示该工作负载的抢占目标与同一调度周期中已处理的另一个工作负载重叠。<br>- "NoLongerFits" 表示在同一调度周期中另一个工作负载占用容量后，该工作负载不再适配。 | `cluster_queue`: ClusterQueue 的名称<br> `reason`: 抢占被跳过的原因，`OverlappingPreemptionTargets` 或 `NoLongerFits` 其中之一<br> `replica_role`: `leader`, `follower` 或 `standalone` 其中之一 |

...
[准入(admission)](/v0.18/docs/concepts#admission)单位。
...
当工作负载所在的 ClusterQueue 启用[并发准入](/v0.18/docs/concepts/concurrent_admission)时，
...
您可以通过设置 [Active](/zh-cn/v0.18/docs/reference/kueue.v1beta2#kueue-x-k8s-io-v1beta2-WorkloadSpec)
...
要指示您希望将工作负载排入哪个[本地队列(LocalQueue)](/v0.18/docs/concepts/local_queue)，
...
Kueue 内部使用而保留的。您可以在[集群队列(ClusterQueue)](/v0.18/docs/concepts/cluster_queue#resources)
中使用 `pods` 资源名称来设置 Pod 最大数量的配额。

## 优先级 {#priority}

工作负载具有影响[集群队列准入顺序](/v0.18/docs/concepts/cluster_queue#queueing-strategy)的
优先级。有两种设置工作负载优先级的方法：
...
- **WorkloadPriority**：有时开发人员希望控制工作负载的优先级而不影响 Pod 的优先级。
  通过使用 [`WorkloadPriority`](/v0.18/docs/concepts/workload_priority_class)，
...
请参考[全有或全无与就绪 Pod](/v0.18/docs/tasks/manage/setup_wait_for_pods_ready/)
了解更多详情。
...
- 了解[工作负载优先级类](/v0.18/docs/concepts/workload_priority_class)。
- 了解如何[运行作业](/v0.18/docs/tasks/run/jobs)
- 阅读 `Workload` 的 [API 参考](/zh-cn/v0.18/docs/reference/kueue.v1beta2/#kueue-x-k8s-io-v1beta2-Workload)

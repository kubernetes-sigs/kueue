...
集成而设计的准入检查控制器。其主要功能是为持有[配额预留](/v0.18/docs/concepts/#quota-reservation)的工作负载创建
...
并保持 [AdmissionCheckState](/v0.18/docs/concepts/admission_check/#admissioncheckstate) 的同步。

该控制器是 Kueue 的一部分，默认启用。您可以通过编辑 `ProvisioningACC` 特性门控来禁用它。
有关特性门控配置的详细信息，请查看[安装](/v0.18/docs/installation/#change-the-feature-gates-configuration)指南。
...
要使用 Provisioning 准入检查，需要创建一个 [AdmissionCheck](/v0.18/docs/concepts/admission_check)，
将 `kueue.x-k8s.io/provisioning-request` 作为 `.spec.controllerName`，
并使用 `ProvisioningRequestConfig` 对象创建 ProvisioningRequest 配置。

接下来，您需要从 ClusterQueue 引用 AdmissionCheck，详见[准入检查使用](/v0.18/docs/concepts/admission_check#usage)。
...
如此示例：[`sample-pytorchjob.yaml`](/v0.18/docs/tasks/run/kubeflow/pytorchjobs/#sample-pytorchjob)。
{{% /alert %}}
...
{{< include "v0.18/examples/provisioning/provisioning-setup.yaml" "yaml" >}}

### 使用 ProvisioningRequest 的作业 {#job-using-a-provisioningrequest}

{{< include "v0.18/examples/provisioning/sample-job.yaml" "yaml" >}}

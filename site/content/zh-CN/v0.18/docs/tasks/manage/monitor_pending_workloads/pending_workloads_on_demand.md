...
The intended audience for this page are [batch administrators](/v0.18/docs/tasks#batch-administrator) for [Cluster Queue visibility section](#cluster-queue-visibility-via-kubectl), and [batch users](/v0.18/docs/tasks#batch-user) for [Local Queue Visibility section](#local-queue-visibility-via-kubectl).
...
- [Kueue is installed](/v0.18/docs/installation) in version v0.6.0 or later.
...
```shell
kubectl apply --server-side -f https://github.com/kubernetes-sigs/kueue/releases/download/v0.18.3/visibility-apf.yaml
```
...
{{< include "v0.18/examples/visibility/cluster-role-and-binding.yaml" "yaml" >}}
...
You can disable it by setting the `VisibilityOnDemand` feature gate. Check the [Installation](/v0.18/docs/installation/#change-the-feature-gates-configuration) guide for details on feature gate configuration.
...
{{< include "v0.18/examples/jobs/sample-job.yaml" "yaml" >}}

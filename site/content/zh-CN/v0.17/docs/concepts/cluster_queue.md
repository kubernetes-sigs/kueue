...
- ClusterQueue 管理的[资源规格](/v0.17/docs/concepts/resource_flavor)的配额，包括使用上限和消耗顺序。
- 集群中多个 ClusterQueue 之间的公平共享规则。

只有[批处理管理员](/v0.17/docs/tasks#batch-administrator)才应创建 `ClusterQueue` 对象。
...
只有在以下条件全部满足时，该 ClusterQueue 才会接纳[工作负载](/v0.17/docs/concepts/workload)：
...
规格代表某种资源的不同变体（例如，不同型号的 GPU）。您可以通过 [ResourceFlavor 对象](/v0.17/docs/concepts/resource_flavor)定义规格与节点组的映射关系。
...
资源名 `pods` 是[保留名](/v0.17/docs/concepts/workload/#reserved-resource-names)，不能在 Pod 的 requests 字段中指定。
...
[admission](/v0.17/docs/concepts#admission) 尝试后如何重新排队。

以下是支持的排队策略：

- `StrictFIFO`：工作负载首先按 [优先级](/v0.17/docs/concepts/workload#priority)排序，
...
[priority](/v0.17/docs/concepts/workload#priority) 首先。
如果 feature gate `PrioritySortingWithinCohort=false` 设置，Kueue 将尝试安排最早的 `.metadata.creationTimestamp` 的工作负载。
{{% /alert %}}

你可以设置一个 [`flavorFungibility`](/v0.17/docs/concepts/cluster_queue#flavorfungibility) 来影响一些规格选择和借用的语义。
...
阅读 [Preemption](/v0.17/docs/concepts/preemption) 以了解 Kueue 实现以预留尽可能少的工作负载的启发式方法。
...
StopPolicy 允许集群管理员通过在 [spec](/v0.17/docs/reference/kueue.v1beta1/#kueue-x-k8s-io-v1beta1-ClusterQueueSpec) 中设置其值来临时停止 ClusterQueue 中工作负载的接纳，如下所示：
...
例如，使用 admission checks 的 ClusterQueue 配置，请参阅 [Admission Checks](/v0.17/docs/concepts/admission_check#usage)。

## 下一步是什么？ {#what-next}

- 创建 [local queues](/v0.17/docs/concepts/local_queue)
- 如果你还没有，请创建 [resource flavors](/v0.17/docs/concepts/resource_flavor)
- 学习如何 [administer cluster quotas](/v0.17/docs/tasks/manage/administer_cluster_quotas)
- 阅读 [API reference](/v0.17/docs/reference/kueue.v1beta1/#kueue-x-k8s-io-v1beta1-ClusterQueue) for `ClusterQueue`

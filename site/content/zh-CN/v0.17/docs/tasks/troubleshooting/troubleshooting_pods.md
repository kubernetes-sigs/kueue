...
本文档介绍如何排除由 Kueue 直接管理的[普通 Pod](/v0.17/docs/tasks/run/plain_pods/) 的故障，
...
1. [Pod 集成被禁用](/v0.17/docs/tasks/run/plain_pods/#before-you-begin)。
2. Pod 所属的命名空间不满足
   [`managedJobsNamespaceSelector`](/v0.17/docs/reference/kueue-config.v1beta1/#Configuration)
   的要求。
3. Pod 由 Kueue 管理的 Job 或等效 CRD 拥有。
4. Pod 没有 `kueue.x-k8s.io/queue-name` 标签，并且
   [`manageJobsWithoutQueueName`](/v0.17/docs/reference/kueue-config.v1beta1/#Configuration)
   设置为 `false`。
...
当使用 [Pod 组](/v0.17/docs/tasks/run/plain_pods/#running-a-group-of-pods-to-be-admitted-together)时，
Workload 的名称与标签 `kueue.x-k8s.io/pod-group-name` 的值匹配。

当使用[单个 Pod](/v0.17/docs/tasks/run/plain_pods/#running-a-single-pod-admitted-by-kueue)时，
...
当你启用[抢占](/v0.17/docs/concepts/cluster_queue/#preemption)时，
...
- 组满足[终止](/v0.17/docs/tasks/run/plain_pods/#termination)条件，例如，
  当所有 Pod 成功终止时。

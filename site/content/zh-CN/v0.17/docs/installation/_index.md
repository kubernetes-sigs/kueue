...
Kueue 发布[指标](/v0.17/docs/reference/metrics)以监控其控制器组件。
...
[cert-manager](https://github.com/cert-manager/cert-manager)，请遵循[证书管理指南](/v0.17/docs/tasks/manage/productization/cert_manager)。
...
```shell
kubectl apply --server-side -f https://github.com/kubernetes-sigs/kueue/releases/download/v0.17.7/manifests.yaml
```
...
```shell
helm install kueue https://github.com/kubernetes-sigs/kueue/releases/download/v0.17.7/kueue-0.17.7.tgz \
  --namespace kueue-system \
  --create-namespace \
  --wait --timeout 300s
```
...
```shell
kubectl apply --server-side -f https://github.com/kubernetes-sigs/kueue/releases/download/v0.17.7/prometheus.yaml
```

### 为可见性 API 添加 API 优先级和公平性配置 {#add-api-priority-and-fairness-configuration-for-the-visibility-api}

有关更多详细信息，请参阅[配置 API 优先级和公平性](/v0.17/docs/tasks/manage/monitor_pending_workloads/pending_workloads_on_demand/#configure-api-priority-and-fairness)。
...
```shell
kubectl delete -f https://github.com/kubernetes-sigs/kueue/releases/download/v0.17.7/manifests.yaml
```
...
```shell
wget https://github.com/kubernetes-sigs/kueue/releases/download/v0.17.7/manifests.yaml
```

2. 使用你喜欢的编辑器打开 `manifests.yaml`。
3. 在 `kueue-manager-config` ConfigMap 清单中，编辑
   `controller_manager_config.yaml` 数据条目。该条目代表默认的
   [KueueConfiguration](/v0.17/docs/reference/kueue-config.v1beta1)。
...
{{% alert title="Note" color="primary" %}}
请参阅 [All-or-nothing 与就绪 Pod](/v0.17/docs/tasks/manage/setup_wait_for_pods_ready) 以了解
...
- 阅读 [`Configuration` 的 API 参考](/v0.17/docs/reference/kueue-config.v1beta1/#Configuration)

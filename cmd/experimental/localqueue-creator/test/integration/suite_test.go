...
func TestAPIs(t *testing.T) {
	...
	ginkgo.RunSpecs(t,
		"defaultlocalqueue-creator Controllers Suite",
	)
}
...
func managerAndControllerSetup(controllersCfg *config.Configuration) framework.ManagerSetup {
	return func(ctx context.Context, mgr manager.Manager) {
		...
		opts := []controller.LocalQueueCreatorReconcilerOption{
			controller.WithLocalQueueName("default-lq"),
		}
		if controllersCfg.ManagedJobsNamespaceSelector != nil {
			selector, err := metav1.LabelSelectorAsSelector(controllersCfg.ManagedJobsNamespaceSelector)
			gomega.Expect(err).ToNot(gomega.HaveOccurred())
			opts = append(opts, controller.WithNamespaceSelector(selector))
		}

		reconciler := controller.NewLocalQueueCreatorReconciler(
			mgr.GetClient(),

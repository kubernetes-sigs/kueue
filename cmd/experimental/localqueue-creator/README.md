...
This will create the executable `bin/manager` in the current directory (`cmd/experimental/localqueue-creator/bin`).
...
The `localqueue-creator` can be deployed to a Kubernetes cluster using the Kustomize manifests located in the `config/` directory.

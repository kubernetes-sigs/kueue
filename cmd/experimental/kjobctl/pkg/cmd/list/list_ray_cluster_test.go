...
func TestRayClusterCmd(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"should print only kjobctl ray clusters": {
			args: []string{},
			objs: []runtime.Object{
				wrappers.MakeRayCluster("rc1", metav1.NamespaceDefault).
					...
					Obj(),
				wrappers.MakeRayCluster("rc2", "ns1").
					LocalQueue("localqueue1").
					DesiredWorkerReplicas(2).
					AvailableWorkerReplicas(3).
					DesiredCPU(resource.MustParse("5")).
			...
			wantOut: `NAME   PROFILE    LOCAL QUEUE   DESIRED WORKERS   AVAILABLE WORKERS   CPUS   MEMORY   GPUS   STATUS   AGE
rc1    profile1   localqueue1   2                 3                   5      10Gi     10     ready    120m
`,
		},
		"should print ray cluster list with namespace filter": {
			args: []string{},
			ns:   "ns1",
			objs: []runtime.Object{
				wrappers.MakeRayCluster("rc1", "ns1").
					Profile("profile1").
					LocalQueue("localqueue1").
					DesiredWorkerReplicas(2).
					AvailableWorkerReplicas(3).
					...
					CreationTimestamp(testStartTime.Add(-2 * time.Hour)).
					Obj(),
				wrappers.MakeRayCluster("rc2", "ns2").
					Profile("profile1").
					LocalQueue("localqueue1").
					DesiredWorkerReplicas(2).
					AvailableWorkerReplicas(3).
...
`,
		...
		"should print ray cluster list with profile filter": {
...
`,
		...
		"should print ray cluster list with profile filter (short flag)": {
...
`,
		...
		"should print ray cluster list with localqueue filter": {
...
`,
		...
		"should print ray cluster list with localqueue filter (short filter)": {
...
`,
		...
		"should print ray cluster list with label selector filter": {
...
`,
		...
		"should print ray cluster list with label selector filter (short flag)": {
...
`,
		...
		"should print ray cluster list with field selector filter": {
			args: []string{"--field-selector", "metadata.name=rc1"},
			objs: []runtime.Object{
				wrappers.MakeRayCluster("rc1", metav1.NamespaceDefault).
					Profile("profile1").
					LocalQueue("localqueue1").
...
`,
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			clientset.PrependReactor("list", "rayclusters", func(action kubetesting.Action) (handled bool, ret runtime.Object, err error) {
				...
				rayClusterList := obj.(*rayv1.RayClusterList)

				filtered := make([]rayv1.RayCluster, 0, len(rayClusterList.Items))
				for _, item := range rayClusterList.Items {
				...
				rayClusterList.Items = filtered
				return true, rayClusterList, err
			})
			...
			cmd := NewRayClusterCmd(tcg, streams, testingclock.NewFakeClock(testStartTime))
			cmd.SetArgs(tc.args)

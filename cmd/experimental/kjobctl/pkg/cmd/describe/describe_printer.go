...
func describeJob(job *batchv1.Job) (string, error) {
	return tabbedString(func(out io.Writer) error {
		...
		if job.Spec.CompletionMode != nil {
			w.Write(IndentLevelZero, "Completion Mode:\t%s\n", *job.Spec.CompletionMode)
		}
		if job.Spec.Suspend != nil {
			w.Write(IndentLevelZero, "Suspend:\t%v\n", *job.Spec.Suspend)
		}
		if job.Spec.BackoffLimit != nil {
			w.Write(IndentLevelZero, "Backoff Limit:\t%v\n", *job.Spec.BackoffLimit)
		}
		if job.Spec.TTLSecondsAfterFinished != nil {
			w.Write(IndentLevelZero, "TTL Seconds After Finished:\t%v\n", *job.Spec.TTLSecondsAfterFinished)
		}
		...
		if job.Spec.ActiveDeadlineSeconds != nil {
			w.Write(IndentLevelZero, "Active Deadline Seconds:\t%ds\n", *job.Spec.ActiveDeadlineSeconds)
		}

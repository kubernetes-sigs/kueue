...
func TestKueuePopulatorReconciler(t *testing.T) {
	cases := map[string]struct {
		...
		"should use ClusterQueue name as LocalQueue name when mode is AsClusterQueue": {
			clusterQueues: []*kueue.ClusterQueue{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "my-cq"},
					Spec: kueue.ClusterQueueSpec{
						NamespaceSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"dep": "eng"}},
					},
				},
			...
			localQueueNameMode: kueuepopulatorconfig.LocalQueueNameModeAsClusterQueue,
			req:                reconcile.Request{NamespacedName: types.NamespacedName{Name: "ns"}},
			wantLQs: []kueue.LocalQueue{
				{
					ObjectMeta: metav1.ObjectMeta{
			...
			wantEvents: []Event{
				{
					EventType: corev1.EventTypeNormal,
					Reason:    "LocalQueueCreated",
					Message:   "Created LocalQueue my-cq in namespace ns",
				},
			},
		},
		"should create separate LocalQueues for multiple ClusterQueues in AsClusterQueue mode": {
			clusterQueues: []*kueue.ClusterQueue{
				{
					ObjectMeta: metav1.ObjectMeta{Name: "cq-alpha"},
					Spec: kueue.ClusterQueueSpec{
						NamespaceSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"dep": "eng"}},
					},
				},
				{
					ObjectMeta: metav1.ObjectMeta{Name: "cq-beta"},
					Spec: kueue.ClusterQueueSpec{
						NamespaceSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"dep": "eng"}},
					},
				},
			...
			wantEvents: []Event{
				{
					EventType: corev1.EventTypeNormal,
					Reason:    "LocalQueueCreated",
					Message:   "Created LocalQueue cq-alpha in namespace ns",
				},
				{
					EventType: corev1.EventTypeNormal,
					Reason:    "LocalQueueCreated",
					Message:   "Created LocalQueue cq-beta in namespace ns",
				},
			},
		},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.localQueueNameMode != "" {
				opts = append(opts, WithLocalQueueNameMode(tc.localQueueNameMode))
			}
			...
			sortLQs := cmpopts.SortSlices(func(a, b kueue.LocalQueue) bool {
				return a.Name < b.Name
			})
			if diff := cmp.Diff(tc.wantLQs, lqList.Items, cmpopts.IgnoreFields(metav1.ObjectMeta{}, "ResourceVersion"), sortLQs); diff != "" {
			...
			slices.SortFunc(tc.wantEvents, func(a, b Event) int {
				if a.Message < b.Message {
					return -1
				}
				if a.Message > b.Message {

...
func TestClusterQueueNameCompletionFunc(t *testing.T) {
	testCases := map[string]struct {
		...
		"should return active cluster queue names": {
			objs: []runtime.Object{
				utiltesting.MakeClusterQueue("cq1").StopPolicy(v1beta2.None).Obj(),
				utiltesting.MakeClusterQueue("cq2").StopPolicy(v1beta2.Hold).Obj(),
				utiltesting.MakeClusterQueue("cq3").StopPolicy(v1beta2.HoldAndDrain).Obj(),
			},
...
func TestLocalQueueNameCompletionFunc(t *testing.T) {
	testCases := map[string]struct {
		...
		"should return local queue names": {
			objs: []runtime.Object{
				utiltesting.MakeLocalQueue("lq1", metav1.NamespaceDefault).StopPolicy(v1beta2.None).Obj(),
				utiltesting.MakeLocalQueue("lq2", metav1.NamespaceDefault).StopPolicy(v1beta2.Hold).Obj(),
				utiltesting.MakeLocalQueue("lq3", metav1.NamespaceDefault).StopPolicy(v1beta2.HoldAndDrain).Obj(),
			},

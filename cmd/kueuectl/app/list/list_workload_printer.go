...
type listWorkloadResources struct {
	localQueues      map[string]*v1beta2.LocalQueue
...
func newListWorkloadResources() *listWorkloadResources {
	return &listWorkloadResources{
		localQueues:      make(map[string]*v1beta2.LocalQueue),
...
func (p *listWorkloadPrinter) PrintObj(obj runtime.Object, out io.Writer) error {
	...
	list, ok := obj.(*v1beta2.WorkloadList)
	if !ok {
...
func (p *listWorkloadPrinter) printWorkloadList(list *v1beta2.WorkloadList) []metav1.TableRow {
...
func (p *listWorkloadPrinter) printWorkload(wl *v1beta2.Workload) metav1.TableRow {
	row := metav1.TableRow{
	...
	var execTime string
	if admittedCond := apimeta.FindStatusCondition(wl.Status.Conditions, v1beta2.WorkloadAdmitted); admittedCond != nil &&
		admittedCond.Status == metav1.ConditionTrue {
		finishedTime := p.clock.Now()

		if finishedCond := apimeta.FindStatusCondition(wl.Status.Conditions, v1beta2.WorkloadFinished); finishedCond != nil &&
...
func (p *listWorkloadPrinter) crdTypes(wl *v1beta2.Workload) []string {
...
func (p *listWorkloadPrinter) crdNames(wl *v1beta2.Workload) []string {

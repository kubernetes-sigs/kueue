...
func NamespacesWebSocketHandler(client Client) gin.HandlerFunc {
	return GenericWebSocketHandler(func(ctx context.Context) (any, error) {
		return fetchNamespaces(ctx, client)
	})
}

// Fetch namespaces that have LocalQueues (Kueue-related namespaces)
func fetchNamespaces(ctx context.Context, cl Client) (any, error) {

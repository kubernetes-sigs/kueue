...
func (h *Handlers) NamespacesWebSocketHandler() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		h.GenericWebSocketHandler(func(ctx context.Context) (any, error) {
			return h.fetchNamespaces(ctx, user, groups)
		}, LocalQueuesGVK())(c)
	}
}

// Fetch namespaces that have LocalQueues (Kueue-related namespaces)
func (h *Handlers) fetchNamespaces(ctx context.Context, user string, groups []string) (any, error) {
	...
	for _, lq := range lql.Items {
		...
		if !namespaceSet.Has(ns) {
			if h.authorizer != nil {
				allowed, err := h.authorizer.Authorize(ctx, user, groups, middleware.ResourceAccess("list", LocalQueuesGVR(), ns, ""))
				if err != nil || !allowed {
					continue
				}
			}
	...
	if h.authorizer != nil {
		allowed, err := h.authorizer.Authorize(ctx, user, groups, middleware.ResourceAccess("list", LocalQueuesGVR(), "", ""))
		if err != nil || !allowed {

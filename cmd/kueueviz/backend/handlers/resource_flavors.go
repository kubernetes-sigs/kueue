...
func ResourceFlavorsWebSocketHandler(client Client) gin.HandlerFunc {
	return GenericWebSocketHandler(func(ctx context.Context) (any, error) {
		return fetchResourceFlavors(ctx, client)
	})
}

// ResourceFlavorDetailsWebSocketHandler streams details for a specific resource flavor
func ResourceFlavorDetailsWebSocketHandler(client Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		flavorName := c.Param("flavor_name")
		GenericWebSocketHandler(func(ctx context.Context) (any, error) {
			return fetchResourceFlavorDetails(ctx, client, flavorName)
		})(c)
	}
}

// Fetch all resource flavors
func fetchResourceFlavors(ctx context.Context, client Client) (any, error) {
	rfl := &kueueapi.ResourceFlavorList{}
	err := client.List(ctx, rfl)
	if err != nil {
...
func fetchResourceFlavorDetails(ctx context.Context, client Client, flavorName string) (map[string]any, error) {
	// Fetch the specified resource flavor details
	flavor := &kueueapi.ResourceFlavor{}
	err := client.Get(ctx, ctrlclient.ObjectKey{Name: flavorName}, flavor)
	if err != nil {
	...
	matchingNodes, _ := getNodesForFlavor(ctx, client, flavorName)
...
func getNodesForFlavor(ctx context.Context, client Client, flavorName string) ([]map[string]any, error) {
	rf := &kueueapi.ResourceFlavor{}
	err := client.Get(ctx, ctrlclient.ObjectKey{Name: flavorName}, rf)
	if err != nil {
		return nil, fmt.Errorf("error fetching resource flavor %s: %v", flavorName, err)
	}

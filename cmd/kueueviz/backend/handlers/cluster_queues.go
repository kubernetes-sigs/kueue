...
func fetchClusterQueueDetails(ctx context.Context, client Client, name string) (any, error) {
	// Fetch the specific ClusterQueue
	cq := &kueueapi.ClusterQueue{}
	err := client.Get(ctx, ctrlclient.ObjectKey{Name: name}, cq)
	if err != nil {

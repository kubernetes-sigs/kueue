...
func WorkloadDetailsWebSocketHandler(client Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		namespace := c.Param("namespace")
		workloadName := c.Param("workload_name")
		GenericWebSocketHandler(func(ctx context.Context) (any, error) {
			return fetchWorkloadDetails(ctx, client, namespace, workloadName)
		})(c)
	}
}

func fetchWorkloads(ctx context.Context, client Client, namespace string) (any, error) {
	wl := &kueueapi.WorkloadList{}
	err := client.List(ctx, wl, ctrlclient.InNamespace(namespace))
...
func fetchWorkloadDetails(ctx context.Context, client Client, namespace, workloadName string) (any, error) {
	// Fetch the workload details
	w := &kueueapi.Workload{}
	err := client.Get(ctx, ctrlclient.ObjectKey{Namespace: namespace, Name: workloadName}, w)
	if err != nil {
	...
	if localQueueName != "" {
		...
		err = client.Get(ctx, ctrlclient.ObjectKey{Namespace: namespace, Name: localQueueName}, lq)
		if err != nil {
...
func WorkloadEventsWebSocketHandler(client Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		namespace := c.Param("namespace")
		workloadName := c.Param("workload_name")

		GenericWebSocketHandler(func(ctx context.Context) (any, error) {
			return fetchWorkloadEvents(ctx, client, namespace, workloadName)
		})(c)
	}
}

func fetchWorkloadEvents(ctx context.Context, client Client, namespace, workloadName string) (any, error) {
	el := &corev1.EventList{}
	err := client.List(ctx, el, &ctrlclient.ListOptions{
		Namespace: namespace,

...
func (h *Handlers) LocalQueuesWebSocketHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		h.GenericWebSocketHandler(func(ctx context.Context) (any, error) {
			return h.fetchLocalQueues(ctx, namespace, user, groups)
		}, LocalQueuesGVK())(c)
	}
}
...
func (h *Handlers) fetchLocalQueues(ctx context.Context, namespace, user string, groups []string) (any, error) {
	...
	items := lql.Items
	if namespace == "" {
		items = filterAllowedItems(ctx, h, user, groups, LocalQueuesGVR(), items, func(lq kueueapi.LocalQueue) string {
			return lq.GetNamespace()
		})
	}

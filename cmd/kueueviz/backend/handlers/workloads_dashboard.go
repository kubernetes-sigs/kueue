...
func WorkloadsDashboardWebSocketHandler(client Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		dataFetcher := func(ctx context.Context) (any, error) {
			return fetchDashboardData(ctx, client, namespace)
		}
...
func fetchDashboardData(ctx context.Context, client Client, namespace string) (map[string]any, error) {
	resourceFlavors, _ := fetchResourceFlavors(ctx, client)
	clusterQueues, _ := fetchClusterQueues(ctx, client)
	localQueues, _ := fetchLocalQueues(ctx, client)
	workloads := fetchWorkloadsDashboardData(ctx, client, namespace)
...
func fetchWorkloadsDashboardData(ctx context.Context, client Client, namespace string) any {
	// Filter workloads by namespace if provided, otherwise fetch all
	wql := &kueueapi.WorkloadList{}
	err := client.List(ctx, wql, ctrlclient.InNamespace(namespace))
	...
	for _, workload := range wql.Items {
		...
		pl := &v1.PodList{}
		err = client.List(ctx, pl, ctrlclient.InNamespace(namespace))
		if err != nil {

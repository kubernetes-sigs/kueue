...
```
kubectl create -f https://github.com/kubernetes-sigs/kueue/releases/download/v0.18.10/kueueviz.yaml
```
...
First, patch the frontend ConfigMap to point at the port-forwarded backend.
The ConfigMap name includes a hash suffix generated by kustomize, so find it first:

```
kubectl -n kueue-system get configmap -l app.kubernetes.io/name=kueue | grep frontend-env
```

Then edit it (replace `<configmap-name>` with the actual name from the output above):

```
kubectl -n kueue-system edit configmap <configmap-name>
```
...
Restart the frontend to pick up the ConfigMap change:

```
kubectl -n kueue-system rollout restart deployment/kueue-kueueviz-frontend
```

Then start port-forwarding:

```
kubectl -n kueue-system port-forward svc/kueue-kueueviz-frontend 8080 &
kubectl -n kueue-system port-forward svc/kueue-kueueviz-backend 8081:8080
```

`KueueViz` will then be reachable on your browser at: http://localhost:8080
...
helm install kueue oci://us-central1-docker.pkg.dev/k8s-staging-images/charts/kueue \
            --version="0.18.10" --create-namespace --namespace=kueue-system
```
